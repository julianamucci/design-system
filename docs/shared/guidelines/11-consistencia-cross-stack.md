...
**Uma especificação, quatro implementações, um resultado visual.** O usuário final não deve perceber em qual framework o componente foi renderizado. As classes `.nds-*`, os tokens CSS e o comportamento interativo devem ser idênticos.
...
2. **Classes `.nds-*` compostas no `cva()`** — devem ser idênticas em todas as stacks
...
// O comportamento visual vem do CSS .nds-*; o cva() só seleciona a classe base
// e mapeia variante/tamanho/estado para data-attributes.
const base = 'nds-button';

const variants = {
  default: 'nds-button', // + data-variant="default" (o CSS .nds-button[data-variant=...] resolve as cores)
  // ... mesma classe/atributos em todas as stacks
};
```
...
|----------|-------|---------|
| Cor diferente | Classes `.nds-*` ou `data-*` divergentes no `cva()` | Copiar do React |
| Espaçamento diferente | Variante/tamanho (`data-size`) mapeado de forma divergente | Copiar do `cva()` React |
| Border-radius diferente | `.nds-rounded-md` vs `.nds-rounded-lg` | Padronizar |
| Focus ring diferente | Estilo `:focus-visible` do CSS `.nds-*` divergente | Alinhar ao CSS compartilhado |
| Hover inconsistente | Estado de hover no CSS `.nds-*` divergente | Alinhar ao CSS compartilhado |

...
import { createComposerVoice, type ComposerVoiceOptions } from './composer-voice';
import {
  SAMPLE_ELAPSED,
  SAMPLE_ELAPSED_DONE,
  SAMPLE_LEVEL,
...
// captar, não está disponível. Um arquivo de variantes com estados dentro
// diria que há uma escolha de forma onde não há, e a próxima pessoa procuraria
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      description: {
        component:
          'Os três pontos do ditado, mais o caso em que ditar não está disponível.',
      },
    },
  },
};
...
const mount = (options: Omit<ComposerVoiceOptions, 'labels'>) =>
  createComposerVoice({ labels: voiceLabels(), onToggle, ...options });
...
export const Idle: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O alternador espera, com o nome do que vai fazer', async () => {
      ...
      await expect(toggle).toHaveAccessibleName(rotulos.start);
    ...
    await step('E o estado chega em PALAVRA, que é o que se ouve', async () => {
      ...
      await expect(statusOf(canvasElement)).toHaveTextContent(rotulos.status.idle);
    });
  },
};
...
export const Recording: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O MESMO botão fica pressionado e passa a se chamar parar', async () => {
      ...
      await expect(toggle).toHaveAccessibleName(rotulos.stop);
    });
    ...
    await step('O medidor aparece, e está FORA do que é lido em voz', async () => {
      ...
      // para ouvir. O nível entra por custom property — valor de runtime nunca
      // vai para um `style` de desenho.
    ...
    await step('O tempo decorrido está na TELA e fora do que se anuncia', async () => {
      ...
      await expect(status.textContent).toContain(rotulos.status.recording);
    });
    ...
    await step('E a palavra do estado chega junto do NOME do botão', async () => {
      ...
      const toggle = canvas.getByRole('button');
      await expect(toggle).toHaveAccessibleDescription(rotulos.status.recording);
    });
  },
};
...
export const Transcribing: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E o MOTIVO está escrito ao lado, e não só no cinza', async () => {
      ...
      await expect(statusOf(canvasElement)).toHaveTextContent(rotulos.status.transcribing);
    });
  },
};

...
import {
  ...
  SITE_PLACEHOLDER,
  SITE_PREFIX,
  type BuildInputGroupOptions,
} from './input-group.fixtures';
import { inputGroupSource } from './input-group.source';
import { createInputGroupDocs } from '@/components/docs/InputGroupDocs';
 ...
 *
 * `class` e `align` entram sem control de propósito: a aba sai SÓ do `argTypes`
 * nesta stack — não há componente de framework para introspectar —, então prop
 * que não aparece aqui não aparece em lugar nenhum da página de story.
 */
type InputGroupArgs = {
  'aria-label'?: string;
  placeholder: string;
...
const meta: Meta<InputGroupArgs> = {
  ...
  parameters: {
    docs: {
      page: withAutoDocsTab(createInputGroupDocs),
  ...
  argTypes: {
    ...
    align: {
      control: false,
      description:
        "createInputGroupAddon: posição do addon — 'inline-start', 'inline-end', 'block-start' ou 'block-end'. As duas em bloco fazem o grupo empilhar.",
...
const canonical = (args: InputGroupArgs): BuildInputGroupOptions => ({
  ...
  addons: [
    ...
    { align: 'inline-end', button: { label: PASTE_LABEL } },
  ],
});
...
export const Playground: Story = {
  ...
  render: (args) => buildInputGroup(canonical(args)),
  play: async ({ canvasElement, args, step }) => {
    ...
    await step('Nada no grupo é região viva', async () => {
      ...
      await expect(group.querySelectorAll('[role="status"], [role="alert"], [role="log"]'))
        .toHaveLength(0);
    ...
    await step('A altura é RESULTADO, e cresce com a fonte do navegador', async () => {
      ...
      // medição é essa mesma — dobra a base de tipografia e confere que a
      // moldura cresceu junto. Com altura cravada em qualquer peça, a segunda
      // medida sairia igual à primeira e este passo reprova.
      //

...
export const Playground: Story = {
  ...
  render: (args) => {
    ...
    const select = createSelect({
      ...
      items: [...STATES],
      onValueChange: (value) => args.onValueChange?.(value),
    });
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('O campo é um combobox nomeado e nasce fechado', async () => {
      await expect(trigger).toHaveAccessibleName(args.labelText);
    ...
    await step('Digitar a inicial salta para a opção correspondente', async () => {
      ...
      await waitFor(async () => {
        ...
        await expect(trigger).toHaveAttribute('aria-activedescendant', minas.id);
      });
    });
    ...
    await step('Escape fecha sem trocar a escolha e devolve o foco', async () => {
      ...
      await expect(trigger.textContent).toBe(textBefore);
      await expect(trigger).toHaveAttribute('aria-expanded', 'false');
    ...
    await step('Enter, Espaço e seta para baixo abrem a lista', async () => {
      for (const tecla of ['{Enter}', ' ', '{ArrowDown}']) {
        ...
        await expect(trigger, `tecla ${tecla} não abriu a lista`).toHaveAttribute(
          'aria-expanded',
          'true',
        );
      }
    ...
    await step('Seta para cima, Home e End abrem já no extremo certo', async () => {
      ...
      for (const [tecla, ponta] of casos) {
        ...
        const options = within(listbox).getAllByRole('option');
        const target = ponta === 'primeira' ? options[0] : options[options.length - 1];
        await expect(target, `tecla ${tecla} não destacou a opção ${ponta}`).toHaveAttribute(
          'data-highlighted',
        );
      }
    });
    ...
    await step('Sem espaço embaixo, a lista abre para cima', async () => {
      await close();
      const block = trigger.closest<HTMLElement>('.nds-stack')!;
      const margemBefore = block.style.marginBlockStart;
      ...
      block.style.marginBlockStart = `${Math.max(window.innerHeight - 20 - box.bottom, 0)}px`;
      try {
        await userEvent.click(trigger);
        const listbox = await waitForPortal('listbox');
        ...
        block.style.marginBlockStart = margemBefore;
      }
    });
  },
};

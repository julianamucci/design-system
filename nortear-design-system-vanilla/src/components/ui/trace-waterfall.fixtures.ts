 ...
 * da camada que rola, a frase da régua, o molde da leitura e a palavra de cada
 * estado. Os TRECHOS e o EIXO saem de
 * `@shared/primitives/trace-waterfall-examples`, porque não são idioma: a
 * posição das barras é a mesma nos três, e escrever milissegundos diferentes por
 * idioma faria as fotos mostrarem cascatas diferentes.
 *
 * Como no grafo, NADA do andaime é `HTMLElement`: a peça não tem espaço de quem
 * consome, então tudo o que a demonstração precisa é dado, e dado inteiro cabe
 * no compartilhado.
 */
...
import { createTraceWaterfall, type TraceWaterfallLabels } from './trace-waterfall';

const { t } = createTranslation(traceWaterfallTranslations as Record<string, unknown>);
 ...
 * voltando depois: é o recuo, e não o número de linhas, que faz a peça passar da
 * conversa — a coluna do nome cresce com o degrau, e a do eixo tem largura
 * mínima própria. Gerado, e não escrito à mão, porque o que a story fotografa é
 * a rolagem.
 */
 ...
 *
 * Existe para a story da customização, e a razão é mecânica: a coluna do nome é
 * `max-content` com um PISO, e o piso só decide a largura quando o conteúdo é
 * mais estreito que ele. Com rótulos longos, apertar o piso não muda pixel
 ...
 * ele, e os trechos das pontas são recortados. Não é erro — é o desenho de quem
 * mostra um pedaço —, e a linha recortada avisa em palavras que o trecho
 * continua fora.
 */
 ...
 * para um id inexistente depois de uma varredura de renomeação, e a story passou
 * a LANÇAR em vez de reprovar — defeito que nenhum build alcança, porque mora
 ...
 * story sem trecho não teria elemento nenhum a que a asserção pudesse apontar.
 */
export function mountTraceWaterfall(options: {
  ...
  const piece = createTraceWaterfall({
    spans: options.spans,

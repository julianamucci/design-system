...
import { createContextDisplay, CONTEXT_DISPLAY_FORMS, type ContextDisplayForm } from './context-display';
...
import { createContextDisplayDocs } from '@/components/docs/ContextDisplayDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(createContextDisplayDocs),
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('E a palavra do nível está na linha, ao lado da cor', async () => {
      // Sem teto não há nível, e a linha inteira sai — o caso tem story
      // própria em `States`. Aqui a guarda impede que um control em zero
      // derrube o arquivo com um erro de tipo em vez de uma asserção.
      const level = budgetLevel(usage);
      if (level === null) {
        await expect(root.dataset.level).toBeUndefined();
        await expect(root.querySelector('[data-slot="context-display-level"]')).toBeNull();
        return;
      }

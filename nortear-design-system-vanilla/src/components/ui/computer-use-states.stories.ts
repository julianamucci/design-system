...
import {
  computerUseClampedSnippet,
  computerUseEveryStatusSnippet,
  computerUseFinishedSnippet,
  computerUseFirstStepSnippet,
  computerUseRunningSnippet,
  computerUseWithoutStepsSnippet,
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: computerUseEveryStatusSnippet },
      description: {
        component:
...
const mount = (status: RunStatus, activeIndex: number, steps = COMPUTER_STEPS_LOGIN) =>
  createComputerUse({
    url: COMPUTER_URL,
 ...
 *
 * A lista sai de `RUN_STATUSES`, e não de cinco chamadas escritas à mão: estado
...
export const EveryStatus: Story = {
  parameters: {
    ...
    docs: { source: { transform: computerUseEveryStatusSnippet } },
  },
  render: () => {
    const stack = document.createElement('div');
    stack.className = 'nds-stack nds-max-w-md';
    stack.dataset.spacing = 'lg';
    for (const status of RUN_STATUSES) stack.appendChild(mount(status, 3));
    return stack;
  },
...
export const Running: Story = {
  parameters: {
    ...
    docs: { source: { transform: computerUseRunningSnippet } },
  },
...
export const Finished: Story = {
  parameters: {
    ...
    docs: { source: { transform: computerUseFinishedSnippet } },
  },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A legenda aponta para o último passo', async () => {
      ...
      await expect(positionOf(piece)!.textContent).toBe(
        computerUseLabels().position
          .replace('{index}', String(total))
 ...
 *
 * Com o índice no primeiro passo há uma marca só. É o caso que toda sessão
 * atravessa, e o que mais escapa de quem só fotografa o meio.
 */
export const FirstStep: Story = {
  parameters: {
    covers: ['functional.item8', 'visual.item5'],
    docs: { source: { transform: computerUseFirstStepSnippet } },
  },
...
export const WithoutSteps: Story = {
  parameters: {
    ...
    docs: { source: { transform: computerUseWithoutStepsSnippet } },
  },
  render: () => mount('idle', 0, []),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
...
export const ClampedIndex: Story = {
  parameters: {
    ...
    docs: { source: { transform: computerUseClampedSnippet } },
  },

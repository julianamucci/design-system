...
import { activityGraphLabels, mountActivityGraph } from './activity-graph.fixtures';
...
import { createActivityGraphDocs } from '@/components/docs/ActivityGraphDocs';
import { withAutoDocsTab } from '@/lib/withAutoDocsTab';

/**
 * Os três eixos da peça, numa peça só.
 *
 * O estado decide se ela se declara ocupada; a presença de atividade decide se a
 * grade tem tinta — e não se ela existe, que é a diferença desta peça em relação
 * às irmãs da família; e o começo da semana gira as sete linhas.
 *
 * A JANELA NÃO É CONTROLE, e a ausência dela aqui é a decisão: ela é dado, e
 * trocá-la é remontar a peça com outra janela. O que os controles mostram é o
 * que muda SEM trocar o período — e, entre eles, o que a atividade vazia mostra
 * é justamente que a grade continua lá.
 */
type PlaygroundArgs = {
  status: RunStatus;
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(createActivityGraphDocs),
...
/** O invólucro mora nas fixtures: sem janela a fábrica devolve nada, e é ele que
 *  dá à asserção um elemento a que apontar. */
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    // escritos aqui: se a asserção repetisse a conta, as duas errariam juntas.
    ...
    await step('Há uma casa por dia da janela, na ordem do calendário', async () => {
      // A posição na grade é para o olho; a ordem no DOM é a ordem de leitura
      // (WCAG 1.3.2), e ela é a do calendário — não coluna por coluna.
    ...
    await step('O nível chega em atributo e em número, e os dois concordam', async () => {
      // O valor COMPUTADO, e nunca o atributo `style`: propriedade personalizada
    ...
    await step('Cada casa diz a data, a contagem e a palavra do nível', async () => {
      // É a leitura da grade, e é o que chega a quem não vê a tinta. O dia sem
      // atividade tem frase própria, e não a frase de contagem com um zero.
      const cells = [...root.querySelectorAll<HTMLElement>('[data-slot="activity-graph-day"]')];
      for (const [index, cell] of cells.entries()) {
    ...
    await step('A camada que rola tem papel, nome e parada de teclado', async () => {
      ...
      // E é a ÚNICA parada: um ano são centenas de paradas que não levam a lugar
      // nenhum, e o nome de cada casa já chega sem foco algum.
    ...
    await step('As duas fileiras de rótulo ficam fora do que é lido em voz', async () => {
      ...
      // casa: repeti-la em duas fileiras seria dizer a mesma coisa três vezes.

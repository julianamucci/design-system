...
export const AboveField: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Nada no campo sabe que ela existe', async () => {
      ...
      const descritores = ids.map((id) => canvasElement.ownerDocument.getElementById(id));
      await expect(descritores).not.toContain(line);
...
export const Requesting: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E a linha continua como estava — a peça não executa nada', async () => {
      ...
      await expect(label.textContent).toBe(rotulos.status.running);
      await expect(action).toHaveAccessibleName(rotulos.action!.running!);
    });
  },
};

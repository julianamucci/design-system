...
export function createJobProgress(options: JobProgressOptions): HTMLElement {
  const { label, status = 'idle', count, labels, onAction } = options;

  // `<div>`, e não `<p>`: aqui não há uma frase, há um bloco com uma barra
  // dentro. Nenhum papel ARIA na raiz e nenhuma região viva (decisão 1) — o que
  // dá nome ao conjunto é a própria barra, que carrega o rótulo do trabalho.
  const root = document.createElement('div');
  root.dataset.slot = 'job-progress';
  root.className = ['nds-job-progress', options.class].filter(Boolean).join(' ');
  ...
  name.className = 'nds-job-progress-label';
  name.dataset.slot = 'job-progress-label';
  ...
  // vezes a quem ouve — sendo que a cópia que sobra é justamente a que rerroda.
  if (count) {
    const tally = document.createElement('span');
    tally.className = 'nds-job-progress-count';
    tally.dataset.slot = 'job-progress-count';
    tally.setAttribute('aria-hidden', 'true');
    tally.textContent = countText(count, labels);
    root.appendChild(tally);
  }

  // A BARRA É A DO DESIGN SYSTEM, e o que ela mostra sai do vocabulário
  // (decisão 5). `null` é "andando sem estimativa": `createProgress` troca a
  ...
  const bar = createProgress({
    ...
    variant: status === 'complete' ? 'success' : status === 'failed' ? 'destructive' : undefined,
    // Sem nome, `role="progressbar"` é anunciado como "barra de progresso,
    // 24%": o leitor diz quanto, nunca de quê. O nome é o rótulo do trabalho,
    // que é o mesmo texto que se vê ao lado.
    'aria-label': label,
    className: 'nds-job-progress-bar',
  });
  root.appendChild(bar);

  // A PALAVRA É O ESTADO, e é a única parte da peça que não sai do que é lido
  // em voz (decisão 2).
  const word = document.createElement('span');
  word.className = 'nds-job-progress-status';
  word.dataset.slot = 'job-progress-status';
  ...
  if (actionLabel) {
    ...
    action.classList.add('nds-job-progress-action');
    action.dataset.slot = 'job-progress-action';

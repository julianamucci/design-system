...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// O estado é do TRECHO, e não da peça — o que a peça tem é o estado da execução
// que a escreve, e ele decide uma coisa só: se ela se declara ocupada. Por isso
// esta régua fotografa os quatro estados de trecho um sob o outro, e não quatro
// cascatas.
...
export const EveryState: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A palavra do estado chega a quem não vê a forma da marca', async () => {
      // Forma para quem vê — a marca ao lado do nome e o preenchimento da barra
      // —, palavra para quem ouve, e ninguém fica com a cor sozinha (WCAG
      // 1.4.1).
...
export const Failure: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O trecho que quebrou desenha diferente do que terminou', async () => {
      // É o par que resolve a regra da cor: a barra troca de cor e a MARCA troca
      // de forma, então o estado nunca depende só da cor.
      //
 ...
 * parada de teclado, e uma parada que leva a uma caixa vazia é ruído com nome. E
 * são DOIS casos, porque um eixo sem extensão não posiciona nada: o rastro
 * existe e não há régua para dividir.
 */

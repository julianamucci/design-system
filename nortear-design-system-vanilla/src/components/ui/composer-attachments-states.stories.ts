...
import { attachmentLabels, composerLabels, one } from './composer-attachments.fixtures';
import {
...
const itemOf = (root: HTMLElement) =>
  root.querySelector<HTMLElement>('[data-slot="composer-attachment"]')!;

export const Pending: Story = {
  parameters: { covers: ['visual.item2'] },
  render: () => mount(one('pending')),
  play: async ({ canvasElement, step }) => {
    const item = itemOf(canvasElement.querySelector<HTMLElement>('[data-slot="composer"]')!);

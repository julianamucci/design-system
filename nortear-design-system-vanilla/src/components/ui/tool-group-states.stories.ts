...
const mount = (calls: ChatToolCall[], open = false) =>
  createToolGroup({ calls, labels: toolGroupLabels(), open });
...
export const EveryState: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada uma traz a PALAVRA daquele estado, e não só a cor', async () => {
      ...
      for (const [i, state] of TOOL_CALL_STATES.entries()) {
        const badge = items[i]!.querySelector<HTMLElement>('[data-slot="tool-call-state"]')!;
        await expect(badge.textContent).toBe(rotulos.call[state]);
    ...
    await step('E as quatro palavras são DIFERENTES entre si', async () => {
      ...
      const palavras = TOOL_CALL_STATES.map((state) => rotulos.call[state]);
      await expect(new Set(palavras).size).toBe(TOOL_CALL_STATES.length);
    });

    await step('Com uma falha na lista, o resumo fala da falha', async () => {
      // A precedência vem do vocabulário compartilhado, e não de um `if` da
      // tela: a falha vence tudo, porque é ela que não pode ficar escondida.
      await expect(summaryStateOf(group).textContent).toBe(rotulos.summary.failed);
    });
  },
};
...
export const Failed: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E a falha está escrita, não só colorida', async () => {
      ...
      await expect(badge.textContent).not.toBe(rotulos.summary.done);
    });
...
export const Done: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Com tudo terminado, o resumo não pede atenção', async () => {
      await expect(summaryStateOf(group).textContent).toBe(rotulos.summary.done);
    });
...
export const Running: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Algo ainda corre, e o resumo não diz que terminou', async () => {
      ...
      await expect(summaryStateOf(group).textContent).toBe(rotulos.summary.running);
      await expect(summaryStateOf(group).textContent).not.toBe(rotulos.summary.done);
    });
  },
};

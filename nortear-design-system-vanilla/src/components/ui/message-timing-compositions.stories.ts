...
import {
  messageTimingSourceInsideTooltip,
  messageTimingSourceInTightSpace,
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: messageTimingSourceInTightSpace },
      description: {
        component:
...
const settledTiming = () =>
  createMessageTiming({ stats: statsOf('settled'), labels: messageTimingLabels() });
...
export const InTightSpace: Story = {
  ...
  render: () => {
    ...
    tight.appendChild(settledTiming());
    return tight;
  },
...
export const InsideTooltip: Story = {
  parameters: {
    ...
    docs: { source: { transform: messageTimingSourceInsideTooltip } },
  },
  render: () => {
    ...
    const trigger = createButton({
      variant: 'ghost',
      size: 'sm',
      label: messageTimingTriggerLabel(),
      'aria-label': labels.title,
    });
    const composed = createTooltip({ trigger, content: settledTiming(), side: 'top' });
    return wrap(composed, '200px');
  },
  play: async ({ canvasElement, step }) => {
    const labels = messageTimingLabels();

...
import { createComposer, type ComposerOptions } from './composer';
import { composerLabels } from './composer.fixtures';
import { composerSource } from './composer.source';
import { createComposerDocs } from '@/components/docs/ComposerDocs';
...
const meta: Meta<Args> = {
  ...
  parameters: {
    layout: 'padded',
    docs: {
      page: withAutoDocsTab(createComposerDocs),
      // Sem a transform o renderer html imprime o `outerHTML`: o formulário
      // inteiro já desenhado. O painel mostra a chamada de fábrica.
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const submitButton = () => canvas.getByRole('button', { name: rotulos.submit });
    ...
    await step('O campo tem nome próprio, e não depende do marca-lugar', async () => {
      ...
      await expect(input).toHaveAccessibleName(rotulos.input);
    });

...
import { createTraceWaterfallDocs } from '@/components/docs/TraceWaterfallDocs';
 ...
 * barras dividem, e é a única coisa que muda a posição de todas ao mesmo tempo.
 *
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(createTraceWaterfallDocs),
...
/** O invólucro mora nas fixtures: sem trecho a fábrica devolve nada, e é ele que
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    if (args.revealed === 0) {
      // Sem trecho não há cascata, e a fábrica devolve nada — nem moldura, nem
    ...
    await step('Há uma linha por trecho, na ordem em que foram declarados', async () => {
      // A posição no eixo é livre; a ordem no DOM não é, porque ela é a ordem de
      // leitura (WCAG 1.3.2). E a peça não ordena por começo.
    ...
    await step('A peça se declara ocupada só enquanto a execução corre', async () => {
      // Nada aqui é região viva: um rastro ganha trecho mais depressa do que se
      // lê, e narrar cada trecho é a mesma armadilha do relógio ao vivo.

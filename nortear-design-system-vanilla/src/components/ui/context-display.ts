...
function createMeter(form: ContextDisplayForm, percent: number): HTMLElement | null {
  ...
  // portador do mesmo número o faria ser lido duas vezes.
  if (form === 'ring') {
    const ring = document.createElement('span');
    ring.className = 'nds-context-display-ring';
    ring.dataset.slot = 'context-display-meter';
    ring.setAttribute('aria-hidden', 'true');
    // Valor de RUNTIME por custom property, como o progresso do anexo já faz.
    // O que entra é o mesmo inteiro que o texto mostra, e não a fração crua:
    // um anel cheio ao lado de "99%" seriam duas respostas para uma pergunta.
    ring.style.setProperty('--nds-context-used', String(percent));
    return ring;
  }

  const track = document.createElement('span');
  track.className = 'nds-context-display-bar';
  track.dataset.slot = 'context-display-meter';
  track.setAttribute('aria-hidden', 'true');
  // A propriedade fica no TRILHO, e não no preenchimento — custom property
  // herda, e o preenchimento a lê de graça. Com ela nas duas formas presa ao
  // mesmo elemento (`[data-slot="context-display-meter"]`), quem lê o desenho
  // não precisa saber qual forma está montada para achar o número.
  track.style.setProperty('--nds-context-used', String(percent));

  const fill = document.createElement('span');
  fill.className = 'nds-context-display-bar-fill';
  track.appendChild(fill);

  return track;
}

export function createContextDisplay(options: ContextDisplayOptions): HTMLElement {
  const { usage, form = 'ring', labels } = options;
  ...
  root.dataset.slot = 'context-display';
  root.className = ['nds-context-display', options.class].filter(Boolean).join(' ');
  ...
  title.dataset.slot = 'context-display-title';
  title.textContent = labels.title;
  root.appendChild(title);

  // SEM TETO NÃO SE DESENHA MEDIDOR (decisão 5).
  if (percent !== null) {
    const meter = createMeter(form, percent);
    if (meter) root.appendChild(meter);
  }
  ...
  value.className = 'nds-context-display-value';
  value.dataset.slot = 'context-display-value';
  value.textContent =
  ...
  detail.className = 'nds-context-display-detail';
  detail.dataset.slot = 'context-display-detail';
  detail.textContent =
    percent === null
      ? labels.unbounded
  ...
  if (level) {
    ...
    badge.dataset.slot = 'context-display-level';

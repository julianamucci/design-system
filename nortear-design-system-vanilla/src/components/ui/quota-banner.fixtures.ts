...
const { t } = createTranslation(quotaTranslations as Record<string, unknown>);
 ...
 */
export function quotaBannerAction(): HTMLElement {
  return createButton({ variant: 'outline', size: 'sm', label: t('labels.action') });
}

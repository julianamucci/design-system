...
// contrato — quem copiasse só a chamada da fábrica dividiria token por segundo
// na primeira vez que precisasse da velocidade, e um dia mostraria `38.4` numa
...
function build(opts: MessageTimingSnippetOptions): string {
  ...
  return snippet(
    importing('message-timing', 'createMessageTiming'),
    ...
    streaming
      ? [
        ...
        `const messageTiming = ${callLine('createMessageTiming', lines)};`,
      ].join('\n')
      : `const messageTiming = ${callLine('createMessageTiming', lines)};`,
    appendLine('messageTiming'),
  );
}

/** Transform do `meta` — o Playground, que escreve as medidas por extenso. */
export const messageTimingSource: SourceTransform<MessageTimingSnippetOptions> = (_c, ctx) => {
  const args = ctx?.args ?? {};
 ...
 */
export function messageTimingSourceSettled(): string {
 ...
 */
export function messageTimingSourceMeasuring(): string {
  return build({ measures: 3, streaming: true });
}

/** Só parte das medidas — e a linha continua honesta com duas. */
export function messageTimingSourcePartial(): string {
 ...
 */
export function messageTimingSourceEmpty(): string {
 ...
 */
export function messageTimingSourceEveryCase(): string {
  return snippet(
    importing('message-timing', 'createMessageTiming'),
    writerLines(),
    [
 ...
 * responde.
 */
export function messageTimingSourceInTightSpace(): string {
  return snippet(
    importing('message-timing', 'createMessageTiming'),
    writerLines(),
    [
      ...
      `tight.append(${callLine('createMessageTiming', options([
        ['stats', 'measured'],
        ['labels', 'rotulos'],
      ]))});`,
    ].join('\n'),
 ...
 */
export function messageTimingSourceInsideTooltip(): string {
  return snippet(
    [
      importing('message-timing', 'createMessageTiming'),
      importing('button', 'createButton'),
      importing('tooltip', 'createTooltip'),
    ].join('\n'),
    writerLines(),
    [
      'const measured = [',
      ...
      `const trigger = ${callLine('createButton', options([
        ['variant', text('ghost')],
        ['size', text('sm')],
        ['label', text('1,24 s')],
        ['aria-label', text('Tempo desta resposta')],
      ]))};`,
      '',
      `const compact = ${callLine('createTooltip', options([
        ['trigger', 'trigger'],
        ['content', callLine('createMessageTiming', options([
          ['stats', 'measured'],
          ['labels', 'rotulos'],
        ]))],
        ['side', text('top')],

...
import { attachmentLabels, composerLabels, one, queue } from './composer-attachments.fixtures';
import {
...
export const WithField: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Sem tamanho, nenhum número é inventado', async () => {
      ...
      const meta = withoutSize.querySelector('.nds-composer-attachment-meta')!;
      await expect(meta.textContent).toBe(attachmentLabels().state.ready);
    });
  },
};

...
const { t } = createTranslation(approvalTranslations as Record<string, unknown>);
...
export function approvalChoices(
  ...
  return choices.map((choice) => {
    const control = createButton({
      label: t(`labels.choice.${choice}`),
      variant: 'outline',
      size: 'sm',
    });
    control.dataset.approvalChoice = choice;
    return control;
  });
}
 ...
 */
export function approvalAside(): HTMLElement {
  return createButton({

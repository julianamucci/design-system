...
import { createConnectionStateDocs } from '@/components/docs/ConnectionStateDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(createConnectionStateDocs),
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A palavra do estado escolhido está na linha', async () => {
      ...
      await expect(label.textContent).toBe(rotulos.state[args.state]);
    });
    ...
    await step('A região que se anuncia é SÓ a palavra', async () => {
      ...
      const vivas = [...root.querySelectorAll('[role="status"], [role="alert"], [role="log"], [aria-live]')];
      await expect(vivas).toEqual([label]);
    });

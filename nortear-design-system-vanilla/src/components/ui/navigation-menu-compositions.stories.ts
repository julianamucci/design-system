...
export const MegaMenuGrid: Story = {
  ...
  render: () => {
    ...
    const panel = nav.querySelector<HTMLElement>('.nds-navigation-menu-content');
    panel?.classList.add('nds-grid', 'nds-w-lg');
    panel?.setAttribute('data-fixed', '');
    if (panel) {
      panel.dataset.cols = '2';
      panel.dataset.spacing = 'sm';
    }
    return wrap(nav, 340);
  },
  play: async ({ canvasElement, step }) => {
    ...
    // capturar. `open` é idempotente e sobrevive ao replay.
    const panel = await open(trigger, canvasElement);
...
export const WithHighlightedCard: Story = {
  ...
  render: () => {
    ...
    if (panel) {
      ...
      const targets = [...panel.querySelectorAll<HTMLElement>('.nds-navigation-menu-child')];

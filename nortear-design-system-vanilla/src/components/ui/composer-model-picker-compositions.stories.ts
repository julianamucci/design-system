...
export const InRail: Story = {
  ...
  render: () => {
    // O seletor é AUTÔNOMO: ele não é uma prop do campo. Quem consome o monta
...
export const OutsideClick: Story = {
  ...
  render: () => {
    ...
    outside.className = 'nds-text-caption nds-text-muted-foreground';
    outside.dataset.slot = 'outside-target';
    outside.textContent = 'Fora do seletor';

    wrap.append(picker, outside);
    return wrap;
  },
  play: async ({ canvasElement, step }) => {
    const trigger = triggerOf(canvasElement);

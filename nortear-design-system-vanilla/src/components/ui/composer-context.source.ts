...
function build(opts: ContextSnippetOptions, items: string): string {
  const lines = options([
    ['labels', 'rotulos'],
    ['contextLabels', 'rotulosDoContexto'],
    ['context', items],
    ...
    ['onRemoveContext', opts.automatic ? undefined : '(item) => tirar(item.id)'],
  ]);

  return snippet(
    importing('composer', 'createComposer'),
    `const composer = ${callLine('createComposer', lines)};`,
    appendLine('composer'),
  );
}

/** Transform do `meta` — o Playground, que escreve o item por extenso. */
export const composerContextSource: SourceTransform<ContextSnippetOptions> = (_c, ctx) => {
  const args = ctx?.args ?? {};
 ...
 * e mostrar as duas opções aqui ensinaria a declarar o que não se usa.
 */

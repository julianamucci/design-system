...
const mount = (
  ...
  totalMs: number,
  hostClass?: string,
) => mountTraceWaterfall({ spans, totalMs, status: 'running', hostClass });
 ...
 *
 * `depth` é RELATIVO entre os trechos: quem monta não precisa saber de convenção
 * nenhuma sobre onde começa a contagem, e a cascata inteira encosta. O desenho é
 * idêntico ao da story em andamento, e é esse o assunto.
 */
...
export const Shifted: Story = {
  ...
  render: () => mount(SHIFTED_SPANS, TRACE_TOTAL_MS),
 ...
 *
 * Dez trechos com recuo crescente passam de qualquer conversa com as larguras
 * mínimas que a folha declara, e é aí que a camada rola — uma só, com nome e
 * parada de teclado.
 */
export const Wide: Story = {
  ...
  // teto, esta fotografia mostraria uma cascata folgada e o guarda da rolagem
  // ficaria verde sem nada para medir.
  render: () => mount(wideTraceSpans(), WIDE_TOTAL_MS, 'nds-max-w-md'),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
 ...
 *
 * Não existe contador de revelação: quem revela passa menos trechos, e o eixo
 * continua o mesmo. É isso que faz as três barras que sobraram guardarem a
 * posição verdadeira em vez de reescalarem para ocupar a régua inteira.
 */
export const Partial: Story = {
  ...
  render: () => mount(TRACE_SPANS_PARTIAL, TRACE_TOTAL_MS),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
...
export const LongLabels: Story = {
  ...
  render: () => mount(LONG_LABEL_SPANS, LONG_LABEL_TOTAL_MS, 'nds-max-w-md'),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
 ...
 * barras das pontas são recortadas para caber, e cada linha recortada avisa em
 * palavras que o trecho continua fora.
 */
export const Clipped: Story = {
  ...
  render: () => mount(CLIPPED_SPANS, CLIPPED_TOTAL_MS),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
    ...
    await step('A linha recortada avisa que o trecho continua fora', async () => {
      // O id vem da FIXTURE, e não escrito aqui: literal dentro de string não é
      // alcançado por portão nenhum, e um seletor que deixa de casar faz a
 ...
 *
 * São a única superfície de customização da peça, e são elas que decidem quando
 * a cascata passa a rolar. Entram por propriedade personalizada na folha de quem
...
export const TightColumns: Story = {
  ...
  render: () => {
    ...
    const defaultHost = mount(SHORT_LABEL_SPANS, SHORT_LABEL_TOTAL_MS);
    defaultHost.dataset.testid = 'trace-waterfall-default-columns';

    const tightHost = mount(SHORT_LABEL_SPANS, SHORT_LABEL_TOTAL_MS);
    tightHost.dataset.testid = 'trace-waterfall-tight-columns';
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A coluna do nome aperta, e a caixa encolhe com ela', async () => {
      // A largura da caixa é COMPUTADA, e é o que prova que a propriedade chegou

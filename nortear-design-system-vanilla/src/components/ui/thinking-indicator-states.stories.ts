...
function answerPlace(tail: HTMLElement): HTMLElement {
  ...
  stack.append(createMarkdown({ content: questionText() }), tail);
  return stack;
}
...
export const Waiting: Story = {
  ...
  render: () => answerPlace(createThinkingIndicator({ label: generatingLabel() })),
...
export const Arrived: Story = {
  ...
  render: () => answerPlace(createMarkdown({ content: answerText() })),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);

...
const { t, subscribe } = createTranslation(attachmentTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (attachmentTranslations as unknown as Record<
    string,
...
/**
 * Um composer com anexos, rotulado.
 *
...
// ─── createComposerAttachmentsDocs ─────────────────────────────────────────

export function createComposerAttachmentsDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const comAnexos = (attachments: ReturnType<typeof queue>) =>
      createComposer({
    ...
    switch (id) {
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              example('demonstration.labels.queue', () => comAnexos(queue())),
              example('demonstration.labels.uploading', () =>
                comAnexos(one('uploading', { progress: 0.4 }))),
              example('demonstration.labels.failed', () => comAnexos(one('failed'))),
              example('demonstration.labels.withField', () =>
                comAnexos(queue().slice(0, 2))),
            ];
          ...
          pairs: [
            {
              ...
              // O par é a MESMA fila: o que muda é o que o botão de remover
              // diz a quem não vê a tela.
              doPreviewFactory: () => comAnexos(queue().slice(0, 2)),
              dontPreviewFactory: () =>
                createComposer({
                  labels: composerLabels(),
                  // O contraexemplo: o rótulo SEM o nome do arquivo, e a fila
                  // inteira vira o mesmo botão para quem ouve.
                  //
                  // O texto sai do próprio rótulo traduzido, com o lugar do
                  // nome removido — assim o contraexemplo continua nos três
                  // idiomas, e não vira uma palavra portuguesa cravada aqui.
                  attachmentLabels: {
                    ...attachmentLabels(),
                    remove: attachmentLabels().remove.replace(' {name}', ''),
                  },
            ...
            },
            {
              ...
              doPreviewFactory: () => comAnexos(one('uploading', { progress: 0.4 })),
              ...
              dontPreviewFactory: () =>
                createComposer({
                  ...
                  attachments: one('uploading', { progress: 0.4 }),
                }),
            },
          ],
        });
      ...
      case 'estados':
        return createDocsStates({
          ...
          items: ['pending', 'uploading', 'ready', 'failed'].map(k => ({
      ...
      case 'relacionados':
        return createDocsRelated({
          ...
          items: [
            ...
            { name: t('related.items.badge.name'),      description: toPlainText(t('related.items.badge.description')),      path: '?path=/docs/ui-badge--docs' },

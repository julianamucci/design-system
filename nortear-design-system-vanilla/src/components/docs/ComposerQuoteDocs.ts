...
const { t, subscribe } = createTranslation(quoteTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (quoteTranslations as unknown as Record<
    string,
...
/**
 * Um composer com citação, rotulado.
 *
...
// ─── createComposerQuoteDocs ───────────────────────────────────────────────

export function createComposerQuoteDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const comCitacao = (quote: ReturnType<typeof shortQuote> | undefined, anexos = false) =>
      createComposer({
        labels: composerLabels(),
        ...(quote ? { quote, quoteLabels: quoteLabels() } : {}),
        ...(anexos ? { attachments: queue().slice(0, 2), attachmentLabels: attachmentLabels() } : {}),
      });
      ...
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              example('demonstration.labels.short', () => comCitacao(shortQuote())),
              example('demonstration.labels.long', () => comCitacao(longQuote())),
              example('demonstration.labels.withAttachments', () => comCitacao(shortQuote(), true)),
              example('demonstration.labels.absent', () => comCitacao(undefined)),
            ];
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => comCitacao(longQuote()),
              dontPreviewFactory: () => {
                const cortada = longQuote();
                return createComposer({
                  labels: composerLabels(),
                  quoteLabels: quoteLabels(),
                  // O contraexemplo: o trecho cortado em código. Na tela as
                  // duas parecem iguais; a diferença só existe para quem ouve.
                  quote: { ...cortada, excerpt: `${cortada.excerpt.slice(0, 60)}…` },
                });
              },
            },
            {
              ...
              doPreviewFactory: () => comCitacao(shortQuote(), true),
              // O contraexemplo: o botão sem o nome de quem escreveu, ao lado
              // dos botões de anexo — três "×" indistinguíveis por audição.
              dontPreviewFactory: () =>
                createComposer({
                  labels: composerLabels(),
                  quote: shortQuote(),
                  quoteLabels: {
                    ...quoteLabels(),
                    dismiss: quoteLabels().dismiss.replace('{author}', '').trim(),
                  },
      ...
      case 'relacionados':
        return createDocsRelated({
          ...
          items: [
            ...
            { name: t('related.items.attachments.name'), description: toPlainText(t('related.items.attachments.description')), path: '?path=/docs/components-conversational-composerattachments--docs' },

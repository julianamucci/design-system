...
import { createJobProgress, type JobProgressLabels } from '@/components/ui/job-progress';
...
const { t, subscribe } = createTranslation(jobTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (jobTranslations as unknown as Record<
    string,
...
// ─── createJobProgressDocs ────────────────────────────────────────────────────

export function createJobProgressDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    ...
    const piece = (
      ...
      labels: JobProgressLabels = jobProgressLabels(),
    ) => createJobProgress({ label: jobLabel(), status, count, labels });
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => stackOf(piece('running', JOB_COUNT_WITHOUT_TOTAL)),
              // O contraexemplo é montado À MÃO, e tem de ser: a peça trata
              // total zero como ausência, então não há argumento que produza o
              // erro. Aqui a barra determinada em zero é posta no lugar da
              // indeterminada, para que se veja a trilha vazia dizendo "acabou
              // de começar" para algo que já andou muito.
              dontPreviewFactory: () => {
                const wrong = piece('running', JOB_COUNT_WITHOUT_TOTAL);
                const bar = wrong.querySelector('[data-slot="progress"]')!;
                bar.replaceWith(createProgress({
                  value: 0,
                  'aria-label': jobLabel(),
            ...
            },
            {
              ...
              doPreviewFactory: () => stackOf(piece('running'), piece('stopped')),
              dontPreviewFactory: () => {

...
import { createComposerModelPicker } from '@/components/ui/composer-model-picker';
import {
  availableModels,
  everyModel,
  modelLabels,
...
const { t, subscribe } = createTranslation(pickerTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (pickerTranslations as unknown as Record<
    string,
 ...
 *
 * É a única divergência de forma em relação às outras docs pages, e ela sai do
 * desenho: a lista abre PARA CIMA, e uma legenda acima do seletor seria a
...
function example(labelKey: string, build: () => HTMLElement) {
  ...
  wrap.className = 'nds-stack nds-w-full nds-pt-8';
...
// ─── createComposerModelPickerDocs ────────────────────────────────────────────

export function createComposerModelPickerDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    ...
    const inField = (models: ModelOption[], open: boolean) =>
      createComposer({
        ...
        railStart: [picker(models, open, 'fast')],
      });
      ...
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              example('demonstration.labels.closed', () =>
                picker(everyModel(), false, 'balanced')),
              example('demonstration.labels.open', () =>
                picker(availableModels(), true)),
              example('demonstration.labels.unavailable', () =>
                picker(everyModel(), true, 'fast')),
              example('demonstration.labels.withField', () =>
                inField(everyModel(), false)),
            ];
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => picker(everyModel(), true, 'fast'),
              dontPreviewFactory: () => picker(withoutReason(), true, 'fast'),
            },
            {
              doLabel: tNav('common.do'),
              dontLabel: tNav('common.dont'),
              ...
              doPreviewFactory: () => inField(everyModel(), false),
              dontPreviewFactory: () => inField(nameCarryingDescription(), false),
            },
          ],
        });
          ...
          tables: [
            {
              ...
              cols,
              items: ['models', 'labels', 'value', 'onValueChange', 'open', 'onOpenChange']
                .map(k => ({

...
import { createToolGroup, type ToolGroupLabels } from '@/components/ui/tool-group';
...
const { t, subscribe } = createTranslation(groupTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (groupTranslations as unknown as Record<
    string,
...
/** Uma pilha de grupos, para o par do certo e errado. */
function stackOf(...blocks: HTMLElement[]): HTMLElement {
...
export function createToolGroupDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const group = (
      calls: ChatToolCall[],
      open = false,
      labels: ToolGroupLabels = toolGroupLabels(),
    ) => createToolGroup({ calls, labels, open });
    ...
    switch (id) {
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              example('demonstration.labels.failure', () => group(TOOL_CALLS_WITH_FAILURE)),
              example('demonstration.labels.done',    () => group(TOOL_CALLS_ALL_DONE)),
              example('demonstration.labels.running', () => group(TOOL_CALLS_RUNNING)),
              // À vista e aberta: é o que a decisão 5 da folha manda fazer com
              // a chamada que espera por uma pessoa.
              example('demonstration.labels.waiting', () => group([TOOL_CALL_WAITING], true)),
            ];
          ...
          pairs: [
            {
              ...
              // O par é o MESMO grupo: o que muda é se a falha chega a quem não
              // abriu a caixa.
              doPreviewFactory: () => stackOf(group(TOOL_CALLS_WITH_FAILURE)),
              dontPreviewFactory: () => {
                // O contraexemplo: o resumo do conjunto emudecido, e a caixa
                // fechada passa a contar as ferramentas sem dizer o que houve.
                const base = toolGroupLabels();
                const mudo: ToolGroupLabels = {
                  ...base,
                  summary: TOOL_CALL_STATES.reduce((acc, state) => {
                    acc[state] = '';
                    return acc;
                  }, {} as Record<string, string>) as ToolGroupLabels['summary'],
                };
                return stackOf(group(TOOL_CALLS_WITH_FAILURE, false, mudo));
              },
            },
            {
              doLabel: tNav('common.do'),
              ...
              doPreviewFactory: () => {
                ...
                return stackOf(group(waiting, true), group(grouped));
              },
              // O contraexemplo: a que espera por uma pessoa fica DENTRO da
              // caixa fechada, e o pedido nunca chega a quem devia responder.
              dontPreviewFactory: () =>
                stackOf(group([TOOL_CALL_WAITING, ...TOOL_CALLS_WITH_FAILURE])),
            },
          ],
        });
      ...
      case 'propriedades': {
        const interfaceCode = `export interface ToolGroupLabels {

...
import { createAgentPlan, type AgentPlanLabels } from '@/components/ui/agent-plan';
...
const { t, subscribe } = createTranslation(planTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (planTranslations as unknown as Record<
    string,
...
function example(labelKey: string, build: () => HTMLElement | null) {
  ...
  const built = build();
  if (built) wrap.appendChild(built);
  return wrap;
}

// ─── createAgentPlanDocs ──────────────────────────────────────────────────────

export function createAgentPlanDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const plan = (steps: PlanStep[], labels: AgentPlanLabels = agentPlanLabels()) =>
      createAgentPlan({ steps, labels });
    ...
    switch (id) {
        ...
        return createDocsDoDont({
          ...
          pairs: [
            ...
            },
            {
              ...
              doPreviewFactory: () => plan(everyStatePlan())!,
              // O contraexemplo: as palavras apagadas, e a diferença entre os
              // cinco estados passa a existir só na forma e na cor do marcador.
              dontPreviewFactory: () => {
                ...
                return plan(everyStatePlan(), mute)!;
              },
            },
          ],
        });
      ...
      case 'estados':
        return createDocsStates({
          ...
          // A ordem sai de `PLAN_STEP_STATES`: a tabela e a story de estados

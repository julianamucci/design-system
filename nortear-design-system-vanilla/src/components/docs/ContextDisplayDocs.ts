...
/** Uma pilha de medições, para o par do certo e errado. */
...
// ─── createContextDisplayDocs ─────────────────────────────────────────────────

export function createContextDisplayDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const block = (
      ...
      labels: ContextDisplayLabels = contextDisplayLabels(),
    ) => createContextDisplay({ usage: usageOf(name), form, labels });
          ...
          pairs: [
            {
              ...
              // O par é o MESMO par de níveis: o que muda é se a palavra chega
              // a quem não vê a cor do medidor.
              doPreviewFactory: () => stackOf(block('normal'), block('critical')),
              dontPreviewFactory: () => {
                // O contraexemplo: as palavras apagadas, e a diferença entre a
                // janela com folga e a janela no limite passa a existir só na
                // cor do anel.
                ...
                return stackOf(block('normal', 'ring', mute), block('critical', 'ring', mute));
              },
            },
            {
              doLabel: tNav('common.do'),
              ...
              doPreviewFactory: () => stackOf(block('unbounded')),
              ...
              dontPreviewFactory: () =>
                stackOf(
                  createContextDisplay({
                    usage: { input: 0, output: 0, limit: 32_000 },
      ...
      case 'variantes':
        return createDocsVariants({
          ...
          items: CONTEXT_DISPLAY_FORMS.map(form => ({
            name: form,
            description: stripHtml(t(`variants.items.${form}.description`)),
            code: t(`variants.items.${form}.variantCode`),
            previewFactory: () => block('warning', form),
          })),
        });

...
import { createMessageQueue } from '@/components/ui/message-queue';
...
const { t, subscribe } = createTranslation(queueTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (queueTranslations as unknown as Record<
    string,
...
// ─── createMessageQueueDocs ───────────────────────────────────────────────────

export function createMessageQueueDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    /** A fila sozinha, que é como a peça existe. */
    const queue = (messages: QueuedMessage[]) =>
      createMessageQueue({ labels: queueLabels(), messages })!;

    /** A fila empilhada em cima do campo, que é como quem consome a monta. */
    const withField = (messages: QueuedMessage[]) => {
      const stack = document.createElement('div');
      stack.className = 'nds-stack nds-w-full';
      stack.dataset.spacing = 'xs';
      stack.append(queue(messages), createComposer({ labels: composerLabels() }));
      return stack;
    };
      ...
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              example('demonstration.labels.waiting', () => queue(waiting())),
              example('demonstration.labels.sending', () => queue(sending())),
              example('demonstration.labels.long', () => queue(longQueue())),
              example('demonstration.labels.aboveField', () => withField(sending())),
            ];
          ...
          pairs: [
            {
              ...
              // O par é a MESMA fila: o que muda é se cada botão diz qual
              // mensagem retira.
              doPreviewFactory: () => queue(waiting()),
              dontPreviewFactory: () =>
                createMessageQueue({
                  // O contraexemplo: o rótulo do botão sem o texto da mensagem,
                  // e os três passam a se chamar a mesma coisa.
                  labels: { ...queueLabels(), withdraw: t('usage.uxWriting.table.withdraw.bad') },
                  messages: waiting(),
                })!,
            },
            {
              ...
              doPreviewFactory: () => queue(sending()),

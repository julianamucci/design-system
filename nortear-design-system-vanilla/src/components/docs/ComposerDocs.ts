...
const { t, subscribe } = createTranslation(composerTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (composerTranslations as unknown as Record<
    string,
...
export function createComposerDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    switch (id) {
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            // Separador ENTRE os exemplos, e não em volta de cada um: o
            // composer não tem moldura externa, e quatro empilhados sem linha
            // viram um formulário só. A estrutura para quem ouve vem da
            // legenda de cada exemplo, não da linha.
            const examples = [
              example('demonstration.labels.basic', () =>
                createComposer({ labels: composerLabels() })),
              example('demonstration.labels.running', () => {
                const c = createComposer({
                  labels: composerLabels(),
                  value: t('labels.placeholder'),
                });
                c.setRunning(true);
                return c;
              }),
              example('demonstration.labels.limit', () =>
                createComposer({
                  labels: composerLabels(),
                  maxLength: LIMITE_DEMO,
                  value: textOfLength(Math.ceil(LIMITE_DEMO * 0.95)),
                })),
              example('demonstration.labels.rail', () =>
                createComposer({
                  labels: composerLabels(),
                  railStart: [
                    createButton({ label: attachLabel(), variant: 'ghost', size: 'sm' }),
                  ],
                })),
            ];
          ...
          pairs: [
            {
              ...
              // O par é o MESMO composer gerando: o que muda é se o botão
              // troca de nome junto com a forma.
              doPreviewFactory: () => {
                const c = createComposer({ labels: composerLabels(), value: '…' });
                c.setRunning(true);
                return c;
              },
              dontPreviewFactory: () => {
                const c = createComposer({
                  labels: { ...composerLabels(), stop: composerLabels().submit },
                  value: '…',
                });
                c.setRunning(true);
                return c;
            ...
            },
            {
              ...
              // descrição — o número fica só para os olhos, e some para quem
              // não os usa.
              dontPreviewFactory: () =>
                createComposer({
                  labels: { ...composerLabels(), limit: '' },
                  maxLength: LIMITE_DEMO,
                  value: textOfLength(Math.ceil(LIMITE_DEMO * 0.95)),
                }),
            },
      ...
      case 'variantes':
        return createDocsVariants({
          ...
          items: (['enter', 'modifier'] as const).map(modo => ({
            ...
            previewFactory: () =>
              createComposer({ labels: composerLabels(), submitOn: modo }),
          })),
        });
      ...
      case 'propriedades': {
        const interfaceCode = `// createComposer(options)
...
//   getValue()            — o texto agora
//   setValue(texto)       — é por aqui que um rascunho volta
//   setRunning(booleano)  — troca enviar por interromper`;
      ...
      case 'relacionados':
        return createDocsRelated({
          ...
          items: [
            ...
            { name: t('related.items.textarea.name'),   description: toPlainText(t('related.items.textarea.description')),   path: '?path=/docs/ui-textarea--docs' },

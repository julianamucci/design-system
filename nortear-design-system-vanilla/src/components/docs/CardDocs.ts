...
function screenReaderItems(): string[] {
  ...
  return Object.values(
    (cardTranslations as unknown as Record<string, { accessibility?: { screenReader?: Record<string, string> } }>)[locale]
...
export interface CardFooterOptions      { class?: string; }`;
      ...
      case 'tokens': {
        return createDocsTokens({
          ...
          items: [
            { token: '--radius-card', value: '.nds-card', description: t('tokens.table.radiusCard') },
            { token: '--card', value: '.nds-card', description: t('tokens.table.card') },
            { token: '--card-foreground', value: '.nds-card', description: t('tokens.table.cardForeground') },
            { token: '--muted', value: '.nds-card-footer', description: toPlainText(t('tokens.table.muted')) },
            { token: '--muted-foreground', value: '.nds-card-description', description: t('tokens.table.mutedForeground') },
            { token: '--foreground', value: '.nds-card', description: toPlainText(t('tokens.table.foreground')) },
            { token: '--border', value: '.nds-card-footer', description: t('tokens.table.border') },

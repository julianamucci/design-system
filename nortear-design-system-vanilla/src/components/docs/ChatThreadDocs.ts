...
const { t, subscribe } = createTranslation(chatTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (chatTranslations as unknown as Record<
    string,
 ...
 * entre elas.
 */
function example(labelKey: string, messages: ChatExampleMessage[], streaming = false) {
  ...
  wrap.append(caption, createChatThread({ messages: list, labels: chatLabels(), size: 'md' }));
  return wrap;
}

// ─── createChatThreadDocs ─────────────────────────────────────────────────────

export function createChatThreadDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    switch (id) {
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => createChatThread({
                messages: paraMensagens(CHAT_CONVERSA),
                labels: chatLabels(),
                size: 'sm',
              }),
              dontPreviewFactory: () => createChatThread({
                messages: paraMensagens(CHAT_CONVERSA),
                labels: chatLabels(),
                size: 'sm',
              }),
      ...
      case 'variantes':
        return createDocsVariants({
          ...
          items: (['user', 'assistant', 'system'] as const).map(role => ({
            name: role,
            description: stripHtml(t(`variants.items.${role}.description`)),
            code: `createChatThread({\n  messages: [{ role: '${role}', content: texto }],\n  labels,\n});`,
            previewFactory: () => createChatThread({
      ...
      case 'propriedades': {
        const interfaceCode = `// createChatThread(options)
      ...
      case 'relacionados':
        return createDocsRelated({
          ...
          items: [
            ...
            { name: t('related.items.avatar.name'),   description: toPlainText(t('related.items.avatar.description')),   path: '?path=/docs/primitives-display-avatar--docs' },

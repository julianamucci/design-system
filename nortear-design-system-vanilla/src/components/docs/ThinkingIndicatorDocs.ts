...
const { t, subscribe } = createTranslation(indicatorTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (indicatorTranslations as unknown as Record<
    string,
 ...
 * indicador é só um desenho de três pontos; o que a peça é só aparece quando se
 * vê o LUGAR que ele ocupa — e o mesmo lugar depois, com a resposta dentro.
 */
function answerPlace(tail: HTMLElement): HTMLElement {
  const place = document.createElement('div');
  place.className = 'nds-stack nds-w-full';
  place.dataset.spacing = 'sm';
  place.append(createMarkdown({ content: questionText() }), tail);
  return place;
}
 ...
 *
 * Sem ela, três lugares empilhados viram um só — e o assunto da demonstração é
...
// ─── createThinkingIndicatorDocs ──────────────────────────────────────────────

export function createThinkingIndicatorDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    const waiting = () =>
      answerPlace(createThinkingIndicator({ label: generatingLabel() }));
    const arrived = () => answerPlace(createMarkdown({ content: answerText() }));
    ...
    switch (id) {
        ...
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            const examples = [
              ...
              example('demonstration.labels.withComposer', () => {
                const wrap = document.createElement('div');
                wrap.className = 'nds-stack nds-w-full';
                ...
                wrap.append(
                  createThinkingIndicator({ label: searchingLabel() }),
                  composer,
                );
                return wrap;
              }),
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => createThinkingIndicator({ label: generatingLabel() }),
              // O contraexemplo: a frase apagada, e o que sobra para quem ouve
              // a tela é silêncio — os pontos são desenho, e desenho não se lê.
              dontPreviewFactory: () => createThinkingIndicator({ label: '' }),
            },
            {
              doLabel: tNav('common.do'),
              dontLabel: tNav('common.dont'),
      ...
      case 'propriedades': {
        const interfaceCode = `export interface ThinkingIndicatorOptions {
...
// Não há opção de quantidade de pontos: o atraso escalonado que faz três

...
import { createMarkdown } from '@/components/ui/markdown';
...
const { t, subscribe } = createTranslation(markdownTranslations as Record<string, unknown>);

// As chaves de `accessibility.screenReader` variam por componente, então só os
// valores chegam ao container — o `t()` exige nome de chave e não serviria.
function screenReaderItems(): string[] {
  const locale = getLocale();
  const raw = (markdownTranslations as unknown as Record<
    string,
 ...
 *
 * Sem a legenda, quatro documentos empilhados viram um só texto comprido —
 * e o assunto da demonstração é justamente a diferença entre eles.
 */
function example(labelKey: string, content: string, allow?: readonly string[]): HTMLElement {
  ...
  wrap.append(
    ...
    createMarkdown({
      content,
      allow: allow as Parameters<typeof createMarkdown>[0]['allow'],
    }),
  );
  return wrap;
}
...
export function createMarkdownDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    switch (id) {
      ...
      case 'anatomia':
        return createDocsAnatomy({
          ...
          items: [
            ...
            t('anatomy.item4'), t('anatomy.item5'),
          ],
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => createMarkdown({
                content: '[Leia a política de privacidade](javascript:alert(1)) antes de continuar.',
              }),
              dontPreviewFactory: () => createMarkdown({
            ...
            },
            {
              ...
              dontPreviewFactory: () => createMarkdown({
                content: 'Concordo com o **ponto principal**.',
                allow: ALLOW_PRESETS.comment,
              }),
            },
          ],
      ...
      case 'variantes':
        return createDocsVariants({
          ...
          items: [
            {
              name: 'full',
              description: stripHtml(t('variants.items.full.description')),
              code: `createMarkdown({ content: resposta });`,
              previewFactory: () => createMarkdown({ content: MARKDOWN_COMMENT }),
            },
            {
              name: 'chat',
              description: stripHtml(t('variants.items.chat.description')),
              code: `createMarkdown({\n  content: resposta,\n  allow: ['paragraph', 'code', 'blockquote', 'list', 'thematicBreak', 'raw'],\n});`,
              previewFactory: () => createMarkdown({
                content: MARKDOWN_COMMENT,
            ...
            {
              name: 'comment',
              description: stripHtml(t('variants.items.comment.description')),
              code: `createMarkdown({\n  content: resposta,\n  allow: ['paragraph', 'raw'],\n});`,
              previewFactory: () => createMarkdown({
                content: MARKDOWN_COMMENT,
                allow: ALLOW_PRESETS.comment,
      ...
      case 'propriedades': {
        const interfaceCode = `// createMarkdown(options)
export interface MarkdownOptions {
  ...
  class?: string;
        ...
        return createDocsProps({
          ...
          tables: [
            {
              title: 'createMarkdown',
              cols,
              items: ['content', 'streaming', 'allow', 'allowedProtocols', 'onLinkClick', 'class']
                .map(k => ({
      ...
      case 'tokens':
        return createDocsTokens({
          ...
          items: [
            { token: '--foreground',  value: '.nds-markdown',                    description: t('tokens.table.foreground.description') },
            { token: '--primary',     value: '.nds-markdown-link',               description: t('tokens.table.linkColor.description') },
            { token: '--ring',        value: '.nds-markdown-link:focus-visible', description: t('tokens.table.linkRing.description') },
            { token: '--primary',     value: '.nds-markdown-quote',              description: t('tokens.table.quoteBar.description') },
            { token: '--border',      value: '.nds-markdown-rule',               description: t('tokens.table.ruleLine.description') },
            { token: '--radius-md',   value: '.nds-markdown-image',              description: t('tokens.table.imageRadius.description') },
          ],
      ...
      case 'relacionados':
        return createDocsRelated({
          ...
          items: [
            ...
            { name: t('related.items.table.name'),     description: toPlainText(t('related.items.table.description')),     path: '?path=/docs/components-tables-table--docs' },
  ...
  const rerender = () => { renderHeader(); buildSidebar(); renderAllSections(); };

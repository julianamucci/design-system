...
import { createCostMeter } from '@/components/ui/cost-meter';
...
const { t, subscribe } = createTranslation(costTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (costTranslations as unknown as Record<
    string,
...
export function createCostMeterDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    ...
    switch (id) {
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => line('warning'),
              // O contraexemplo: o teto existe, mas não é passado. A peça só
              // pode dizer o que custou, e a notícia de que o gasto está perto
              // do limite se perde — sem que nada pareça errado na tela.
              dontPreviewFactory: () =>
                createCostMeter({ amount: amountOf('warning'), labels: costMeterLabels() }),
            },
            {
              doLabel: tNav('common.do'),
              dontLabel: tNav('common.dont'),
      ...
      case 'propriedades': {
        const interfaceCode = `export interface CostMeterOptions {
        ...
        return createDocsProps({
          ...
          tables: [
            ...
            { title: 'CostBudget', cols, items: rows(['budgetAmount', 'budgetFraction']) },
            {
              ...
              cols,
              items: rows(['labelsTitle', 'labelsLevel', 'labelsOf', 'labelsUnbounded']),
            },
          ],
          interfaceCode,

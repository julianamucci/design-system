...
const { t, subscribe } = createTranslation(connectionTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (connectionTranslations as unknown as Record<
    string,
...
/** Uma pilha de linhas, para o par do certo e errado. */
function stackOf(...lines: HTMLElement[]): HTMLElement {
...
// ─── createConnectionStateDocs ────────────────────────────────────────────────

export function createConnectionStateDocs(): HTMLElement {
  ...
  function buildSection(id: SectionId): HTMLElement {
    // A MESMA contagem vai para os três, de propósito: quem a recusa onde
    // nenhuma tentativa está marcada é a peça, e não esta página.
    const line = (state: ConnectionState, labels: ConnectionStateLabels = connectionStateLabels()) =>
      createConnectionState({ state, countdown: CONNECTION_COUNTDOWN, labels });
    ...
    switch (id) {
          ...
          pairs: [
            {
              ...
              // O par é o MESMO par de estados: o que muda é se a palavra chega
              // a quem não vê a cor do ponto.
              doPreviewFactory: () => stackOf(line('connected'), line('disconnected')),
              dontPreviewFactory: () => {
                // O contraexemplo: as palavras apagadas, e a diferença entre a
                // ligação de pé e a que caiu passa a existir só na cor.
                const mudo: ConnectionStateLabels = {
                  ...connectionStateLabels(),
                ...
                return stackOf(line('connected', mudo), line('disconnected', mudo));
              },
            },
            {
              doLabel: tNav('common.do'),
              ...
              doPreviewFactory: () => stackOf(line('reconnecting'), line('disconnected')),
              // O contraexemplo é montado À MÃO, e tem de ser: a peça recusa
              // desenhar a contagem onde nenhuma tentativa está marcada, então
              // não há argumento que produza o erro. Aqui a marcação é escrita
              // direto, para que se veja o relógio que não corre.
              dontPreviewFactory: () => {
                const rotulos = connectionStateLabels();
                const errado = createConnectionState({
                  state: 'disconnected',
                  labels: rotulos,
                });
                ...
                clock.textContent = CONNECTION_COUNTDOWN;
                errado.insertBefore(clock, errado.lastElementChild);
                return stackOf(line('reconnecting'), errado);
              },
            },
          ],
        });
      ...
      case 'estados':
        return createDocsStates({
          ...
          // A ordem sai de `CONNECTION_STATES`: a tabela e a story de estados

...
import { createApprovalCard } from '@/components/ui/approval-card';
...
const { t, subscribe } = createTranslation(approvalTranslations as Record<string, unknown>);
...
function screenReaderItems(): string[] {
  ...
  const raw = (approvalTranslations as unknown as Record<
    string,
 ...
 * quem consome, e esta página é quem consome.
 */
function card(name: ApprovalExampleName): HTMLElement {
  return createApprovalCard({
    question: approvalQuestion(name),
    scope: approvalScope(name),
    actions: approvalChoices(),
  });
}
...
// ─── createApprovalCardDocs ───────────────────────────────────────────────────

export function createApprovalCardDocs(): HTMLElement {
      ...
      case 'demonstracao':
        return createDocsDemonstration({
          ...
          demoFactory: () => {
            ...
            APPROVAL_EXAMPLE_NAMES.forEach((name, i) => {
              if (i > 0) stack.appendChild(createSeparator());
              stack.appendChild(example(`demonstration.labels.${name}`, () => card(name)));
            });
            return stack;
          },
        });
          ...
          pairs: [
            {
              ...
              doPreviewFactory: () => card('publish'),
              // O contraexemplo se monta com a MESMA API: o alcance entra
              // embutido na frase, e a lista deixa de existir. É o que a peça
              // não pode impedir — quem escreve a pergunta é quem consome.
              dontPreviewFactory: () =>
                createApprovalCard({
                  question: [

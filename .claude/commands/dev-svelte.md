...
- Apenas o meta do arquivo `<slug>.stories.ts` (story principal) carrega `tags: ['autodocs']`. Sub-stories (`<slug>-variantes.stories.ts`, `<slug>-tamanhos.stories.ts`, `<slug>-estados.stories.ts`, `<slug>-composicoes.stories.ts`) **nunca** incluem `tags: ['autodocs']` — isso cria páginas "Docs" duplicadas em cada grupo da sidebar do Storybook.
- A docs page principal é injetada via `parameters: { docs: { page: withAutoDocsTab(<Slug>Docs) } }` **apenas** no arquivo `<slug>.stories.ts`.
- Sub-stories têm apenas `title`, `component`, `parameters.layout`, `parameters.docs.description.component`. Nada mais no meta.
- Categorias de sub-story dependem da categoria do componente. Respeite a guideline de categoria: overlays de confirmação (AlertDialog) não têm `-variantes` nem `-tamanhos`.
- Antes de escrever HTML inline, confirme que não existe componente em `./components/ui/` que resolva.
...
10. `docs/shared/guidelines/12-tokenizacao-dimensoes.md` — **obrigatório**: zero classes hardcoded de altura/size. Use `h-(--height-default)`, `size-(--size-default)`, etc.

---

## REGRA — Tokenização de dimensões

**Proibido usar classes Tailwind hardcoded** para dimensões que variam entre os 7 temas (Nova, Vega, Maia, Lyra, Mira, Luma, Sera):

- ❌ `h-8`, `h-9`, `size-6`, `size-8`
- ✅ `h-(--height-default)`, `h-(--height-sm)`, `size-(--size-default)`

Consulte `docs/shared/guidelines/12-tokenizacao-dimensoes.md` para tabela completa. Exceções: `px-2.5`, `gap-1.5`, SVGs decorativos (`[&_svg]:size-4`).

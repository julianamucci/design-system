...
- **Zero dimensões hardcoded** em containers de seção. Ver `docs/shared/guidelines/12-tokenizacao-dimensoes.md`:
  - ❌ Evitar: `h-8`, `h-10`, `size-6`, `size-8` em previews, demos e wrappers. Essas classes **não respeitam o tema ativo** (Nova, Vega, Maia, Lyra, Mira, Luma, Sera).
  - ✅ Preferir: deixar que o **conteúdo dite a altura** (a maioria dos containers de seção não precisa de altura fixa). Se precisar, use `h-(--height-default)`, `h-(--height-sm)`, etc.
  - `rounded-lg`, `rounded-xl`, `shadow-sm`, `shadow-md` podem ser usados normalmente — já derivam de tokens.

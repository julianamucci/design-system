...
## Regras Anti-Boilerplate (leia antes de criar qualquer story)

- Apenas o meta do arquivo `<slug>.stories.tsx` (story principal) carrega `tags: ["autodocs"]`. Sub-stories (`<slug>-variantes.stories.tsx`, `<slug>-tamanhos.stories.tsx`, `<slug>-estados.stories.tsx`, `<slug>-composicoes.stories.tsx`) **nunca** incluem `tags: ["autodocs"]` — isso cria páginas "Docs" duplicadas em cada grupo da sidebar do Storybook.
- A docs page principal é injetada via `parameters: { docs: { page: withAutoDocsTab(<Slug>Docs) } }` **apenas** no arquivo `<slug>.stories.tsx`.
- Sub-stories têm apenas `title`, `component`, `parameters.layout`, `parameters.docs.description.component`. Nada mais no meta.
- Categorias de sub-story dependem da categoria do componente. Respeite a guideline de categoria: overlays de confirmação (AlertDialog) não têm `-variantes` nem `-tamanhos`; componentes sem `cva()` não têm `-variantes` etc.
- Antes de escrever HTML inline em uma docs page ou story, confirme que não existe componente em `./components/ui/` que resolva.
...
8. `docs/shared/guidelines/12-tokenizacao-dimensoes.md` — **obrigatório**: zero classes hardcoded de altura/size/radius/shadow. Use `h-(--height-default)`, `size-(--size-default)`, `rounded-lg`, `shadow-sm` (derivam de tokens CSS).
9. `design-system-react/.storybook/preview.ts` — configuração global

---

## REGRA — Tokenização de dimensões

**Proibido usar classes Tailwind hardcoded** para dimensões que devem variar entre os 7 temas (Nova, Vega, Maia, Lyra, Mira, Luma, Sera). Em componentes UI, stories e docs pages:

- ❌ `h-8`, `h-9`, `h-10`, `size-6`, `size-8`
- ✅ `h-(--height-default)`, `h-(--height-sm)`, `h-(--height-lg)`, `size-(--size-default)`

Consulte `docs/shared/guidelines/12-tokenizacao-dimensoes.md` para a tabela completa de tokens disponíveis e exemplos. Exceções: paddings internos (`px-2.5`, `gap-1.5`), SVGs decorativos inline (`[&_svg]:size-4`), `min-h-16` de Textarea.

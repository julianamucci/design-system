...
- Sub-stories têm apenas `title`, `parameters.layout`, `parameters.docs.description.component`. Nada mais no meta.
...
- Antes de chamar `document.createElement()` diretamente, confirme que não existe factory em `./components/ui/` (`createButton`, `createCard`, etc.) que resolva.
...
8. `docs/shared/guidelines/12-tokenizacao-dimensoes.md` — **leia mas NÃO aplica a Basecoat** (usa classes do pacote basecoat-css, não Tailwind). Docs pages em Basecoat, porém, seguem as mesmas regras de tokenização quando usam Tailwind para layout.

...
| `dead_lib_in_infra` | mesmo vocabulário nas skills, guidelines, skill-refs, CSS compartilhado e no `.storybook/*.html` de cada stack — este último acrescentado em 2026-09-05, e o furo dele não era de varredura, era de CATEGORIA: um `<meta>` estático no manager não é código que um slug possui nem instrução que gera código, então nenhuma das três regras de vocabulário o reivindicava. Pior, o arquivo ERA aberto: o `measurement_id_committed` lê o `manager-head.html` inteiro e casa uma única expressão. Arquivo lido por regra que mede uma linha é arquivo não auditado. Sai sob a chave `_infra` (slug-independente): é a infra que **gera** componente novo, e o vocabulário sumia do código para sobreviver nas instruções que o recriam. Menção que registra a remoção ("resíduo do shadcn", "nenhuma lib atual expõe") não conta; dívida já mapeada usa `<!-- audit-ignore: dead-lib — motivo -->` no próprio arquivo |
...
| `icone_de_botao_embrulhado` | SVG de ícone dentro de um `<span>` antes de virar filho do botão. A folha seleciona o SVG como filho DIRETO em três regras, e o embrulho tira o ícone 2,2px do centro num botão de 36×36 — medido. Passe o ícone direto em `children` |
...
**3j. Toda seção com exemplo traz COMPONENTE VIVO** — guideline
`08-docs-pages-foundations.md` §15, e é a regra que este passo mais deixou
passar. A docs page é a vitrine do componente: markup estático imitando a
aparência dele documenta um desenho, não um comportamento, e diverge no dia em
que o componente muda sem que nada reprove.

- **Demonstração traz o exemplo do PLAYGROUND da story**, não uma vitrine
  paralela. Divergiu ao longo do tempo em 38 componentes, e o `audit.mjs` só
  mede os que já foram alinhados (`DEMO_PARIDADE_EXIGIDA`) — acrescente o slug
  ao conjunto ao fechar o componente. Exemplo que sobra vira **composição**
  nomeada, que é auditável, em vez de sumir
- **As demais seções trazem a versão que representa o caso de uso delas** —
  Variantes, Estados, Composições e **Do & Don't**. O `dodont_preview_sem_componente`
  cobra a última, comparando as cinco entre si
- **Overlay na docs page nasce FECHADO** — dialog, alert-dialog, sheet, drawer,
  popover. Vivo é o gatilho; abrir é do leitor. Overlay aberto por padrão cobre a
  página, empilha com o da seção seguinte e o axe passa a medir o overlay, não a
  docs page
- Dois casos que o tooltip pagou e valem para qualquer componente: anti-padrão
  que seria violação REAL de axe (botão icon-only sem nome acessível) mantém o
  `aria-label` e passa a lição para o texto do card, porque o `postVisit` varre a
  docs page inteira e um "don't" honesto reprovaria a própria página; e lição que
  depende de MEDIDA (texto que estoura o balão, rótulo que quebra em duas linhas)
  só existe renderizada — em imitação ela é uma afirmação sobre um desenho

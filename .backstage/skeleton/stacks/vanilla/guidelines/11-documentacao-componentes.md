# Documentação de Componentes Vanilla (Vanilla TS) — Regras Obrigatórias

## Princípio Fundamental: Use os Section Containers

Todas as docs pages Vanilla **DEVEM usar as factory functions** em `src/components/docs/shared/sections/`. Cada seção é uma função `createDocsXxx(props): HTMLElement` que encapsula o layout, o wrapper card, os headings, os grids e a semântica. A docs page é apenas o **orquestrador** — constrói o DOM chamando essas factories e passando dados + **factory functions de preview**.
...
## Regras Vanilla-Específicas

### Factory functions de preview

Em Vanilla não há slots/snippets. Previews são passados como **factory functions** `() => HTMLElement`:
...
Vanilla não tem reatividade automática. A docs page:

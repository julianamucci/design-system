...
1. `nortear-design-system-vanilla/src/components/ui/alert.stories.ts` — Playground
2. `nortear-design-system-vanilla/src/components/ui/alert-variantes.stories.ts` — variantes
3. `nortear-design-system-vanilla/src/components/ui/alert-estados.stories.ts` — estados
4. `nortear-design-system-vanilla/src/components/ui/alert-composicoes.stories.ts` — composições
5. `nortear-design-system-vanilla/src/components/docs/AlertDocs.ts` — docs page completa (REFERÊNCIA)
6. `nortear-design-system-vanilla/.storybook/preview.ts` — configuração global
...
**Override obrigatório para `th`**: vanilla usa `text-foreground` para `th`, mas o design system usa `text-muted-foreground` para consistência com as outras stacks.
...
**REGRA OBRIGATÓRIA: Importe as factory functions de `@/components/ui/<slug>`.** Todos os componentes Vanilla já estão implementados com factory functions tipadas. Stories e docs pages NUNCA devem recriar elementos com classes inline — use sempre as factories existentes.
...
O `accordion.ts` é o modelo mais completo de componente interativo Vanilla. Leia-o como referência antes de implementar dialogs, tabs, selects e outros com estado.
...
Se um container ainda não existe em `shared/sections/`, rode `/docs-sections --stack vanilla` primeiro. Nunca monte `<section id="..."><h2>...</h2><table>...</table></section>` via `innerHTML` replicando o que `createDocsProps`, `createDocsStates`, `createDocsTokens` já fazem.
...
**REGRA CRÍTICA: Cada stack deve ser independente.** A docs page Vanilla DEVE renderizar TODO o conteúdo das 15 seções usando `translations.json`. NUNCA exiba mensagens como "Documentação completa disponível na stack React" ou placeholders genéricos. Cada projeto será usado de forma independente.

**Referência obrigatória**: Leia `nortear-design-system-vanilla/src/components/docs/AlertDocs.ts` inteiro antes de criar qualquer docs page. Ele é o modelo completo para Vanilla.

**Referência de conteúdo**: Leia a docs page React (`nortear-design-system-react/src/components/docs/<Slug>Docs.tsx`) para entender quais seções, tabelas, cards e demos existem. A versão Vanilla deve ter conteúdo IDÊNTICO usando as factory functions + `innerHTML` com `sanitizeHtml` para texto.

Localização: `src/components/docs/<ComponentName>Docs.ts`

O padrão Vanilla exporta uma função `create<ComponentName>Docs(): HTMLElement` que:
...
Toda docs page Vanilla deve renderizar TODAS estas seções com conteúdo real:
...
## Convenções Vanilla-Específicas
...
```bash
ls nortear-design-system-vanilla/src/components/docs/shared/sections/ 2>/dev/null
```

Se existirem (`createDocsDoDont.ts`, `createDocsVariants.ts`, etc.), **use-os**. Se não existirem, rode `/docs-sections --stack vanilla` primeiro.
...
```bash
git add -A
git commit -m "skill(dev-vanilla): $ARGUMENTS"
```

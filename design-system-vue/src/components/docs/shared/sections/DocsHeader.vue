...
<template>
  <header class="ds-docs mb-8 sm:mb-12 border-b pb-6 sm:pb-8 border-border/50">
    ...
    <div class="space-y-3 sm:space-y-4">
      ...
      <p class="text-muted-foreground text-base sm:text-lg max-w-3xl leading-relaxed">
        {{ description }}
      </p>
    </div>
    <div
      v-if="installNote"
      class="mt-6 flex items-center gap-3 text-sm text-muted-foreground/80"
    >
      <span class="flex items-center gap-1.5">
        <code class="bg-muted px-1.5 py-0.5 rounded text-xs font-mono border border-border/50">{{ installNote }}</code>
      </span>
    </div>
  </header>
</template>

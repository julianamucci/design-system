...
## Integração com Storybook (principal)

Toda Showcase page deve ter um arquivo MDX unattached no Storybook. Este é o padrão principal — a integração com `App.tsx` é opcional (sandbox apenas).

### 1. Criar o arquivo MDX

```mdx
{/* src/components/docs/[Categoria]ShowcaseDocs.mdx */}
import { Meta } from '@storybook/blocks';
import { [Categoria]ShowcaseDocs } from './[Categoria]ShowcaseDocs';

<Meta title="UI/[Categoria]/Showcase" />
<[Categoria]ShowcaseDocs />
```

### 2. Criar o stub de stories (evitar conflito de título)

```tsx
// src/components/ui/[categoria]-showcase.stories.tsx
export default { title: 'UI/[Categoria]/Showcase' };
```

**Resultado na sidebar:**
```
UI / [Categoria]
  └── Showcase   ← página de showcase completa
```

**Regras:**
- Título: `"UI/[Categoria]/Showcase"` (ou `"Foundations/[Categoria] Showcase"` para fundamentos)
- Arquivo MDX: `[Categoria]ShowcaseDocs.mdx`
- Stub stories: `[categoria]-showcase.stories.tsx` com `export default { title }` apenas

## Exemplos de Títulos Storybook por Categoria

| Categoria | Title no Storybook | Arquivo MDX |
|-----------|-------------------|-------------|
| Layout | `"UI/Layout/Showcase"` | `LayoutShowcaseDocs.mdx` |
| Navigation | `"UI/Navigation/Showcase"` | `NavigationShowcaseDocs.mdx` |
| Form | `"UI/Form/Showcase"` | `FormShowcaseDocs.mdx` |
| Feedback | `"UI/Feedback/Showcase"` | `FeedbackShowcaseDocs.mdx` |
| Display | `"UI/Display/Showcase"` | `DisplayShowcaseDocs.mdx` |
| Disclosure | `"UI/Disclosure/Showcase"` | `DisclosureShowcaseDocs.mdx` |
| Overlay | `"UI/Overlay/Showcase"` | `OverlayShowcaseDocs.mdx` |

## Integração com App.tsx (opcional — sandbox apenas)

A integração com `App.tsx` é **opcional**. Use apenas se quiser visualizar a Showcase no sandbox de desenvolvimento fora do Storybook.

```typescript
// Opcional — apenas para desenvolvimento local no sandbox
...
**Regra**: Showcase no `App.tsx` deve ser sempre o **primeiro item** da categoria, mas registrá-la lá não é obrigatório.
...
### Integração Storybook (obrigatória)
- [ ] Arquivo MDX criado: `[Categoria]ShowcaseDocs.mdx` com `<Meta title="UI/[Categoria]/Showcase" />`
- [ ] Stub de stories criado: `[categoria]-showcase.stories.tsx` com `export default { title }`
- [ ] Página aparece corretamente na sidebar do Storybook

### Integração App.tsx (opcional — sandbox)
- [ ] Se registrado no App.tsx: adicionado como **primeiro item** da categoria
- [ ] Path segue padrão kebab-case: `[categoria]-showcase`
...
7. ✅ **Integração Storybook obrigatória**: arquivo MDX + stub de stories
8. ✅ **Usar apenas variáveis CSS** do design system
9. ✅ **Integração App.tsx opcional** (sandbox apenas)

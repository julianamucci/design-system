<script lang="ts">
  ...
  const { tStore } = useTranslation(contextTranslations);

  const labels = $derived(contextDisplayLabelsFor($locale));

  /**
   * O contraexemplo do primeiro par: a palavra de cada nível apagada.
   *
   * A diferença entre a janela com folga e a janela no limite passa a existir só
   * na cor do anel — e cor não chega a quem ouve a tela (WCAG 1.4.1). O texto
   * sai dos próprios rótulos traduzidos, esvaziados, e não de uma palavra
   * cravada aqui.
   */
  const WORDLESS_LEVEL: ContextDisplayLabels = $derived({
    ...labels,
  ...
  const interfaceCode = `export interface ContextDisplayLabels {
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
  ...
  <!-- ── Demonstração ───────────────────────────────────────────── -->
  <!--
    A legenda diz QUAL caso está desenhado — sem ela, quatro medições empilhadas
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.ring')}
        </p>
        <ContextDisplay usage={usageOf('warning')} form="ring" {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        ...
        <ContextDisplay usage={usageOf('warning')} form="bar" {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.text')}
        </p>
        <ContextDisplay usage={usageOf('warning')} form="text" {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        ...
        <ContextDisplay usage={usageOf('unbounded')} {labels} />
      </div>
    </div>
  </DocsDemonstration>
  ...
  <!-- ── Do & Don't ─────────────────────────────────────────────── -->
  <!--
    O primeiro par desenha o MESMO par de níveis: o que muda é se a palavra
    chega a quem não vê a cor do medidor. O segundo mostra a ausência de teto
    tratada como informação, e depois tratada como zero.
  -->
  {#snippet doPair1()}
    <div class="nds-stack nds-w-full" data-spacing="sm">
      <ContextDisplay usage={usageOf('normal')} {labels} />
      <ContextDisplay usage={usageOf('critical')} {labels} />
    </div>
  {/snippet}
  {#snippet dontPair1()}
    <div class="nds-stack nds-w-full" data-spacing="sm">
      <ContextDisplay usage={usageOf('normal')} labels={WORDLESS_LEVEL} />
      <ContextDisplay usage={usageOf('critical')} labels={WORDLESS_LEVEL} />
    </div>
  {/snippet}
  {#snippet doPair2()}
    <div class="nds-stack nds-w-full" data-spacing="sm">
  ...
  {/snippet}
  <!--
    O contraexemplo: a MESMA medição sem teto, desenhada como se o teto fosse
    conhecido e nada tivesse sido gasto. É o que sai de tratar a ausência de teto
    como um zero — um anel vazio e "0%", que é o oposto de "não se sabe quanto
    cabe".
  -->
  {#snippet dontPair2()}
    <div class="nds-stack nds-w-full" data-spacing="sm">
      <ContextDisplay usage={{ input: 0, output: 0, limit: 32000 }} {labels} />
    </div>
  {/snippet}
  ...
  <!-- ── Formas ─────────────────────────────────────────────────── -->
  <!--
    A ordem sai de `CONTEXT_DISPLAY_FORMS`: a seção e a story de formas leem a
    mesma lista, e nenhuma das duas fica para trás quando o tipo cresce. O
    desenho de cada uma vem de um snippet nomeado, e a tabela abaixo os liga à
    lista — cadeia de ternários faria o mesmo com um ramo a mais para ler.
  -->
  {#snippet variantRing()}
    <ContextDisplay usage={usageOf('warning')} form="ring" {labels} />
  {/snippet}
  ...
  <DocsVariants
    ...
    items={CONTEXT_DISPLAY_FORMS.map(form => ({
      name: form,
      description: stripHtml($tStore(`variants.items.${form}.description`)),
      code: $tStore(`variants.items.${form}.variantCode`),
      preview: { ring: variantRing, bar: variantBar, text: variantText }[form],
    }))}
  />

  <!-- ── Estados ────────────────────────────────────────────────── -->
  <!--
    Os três níveis saem do primitivo compartilhado; os dois últimos são casos que

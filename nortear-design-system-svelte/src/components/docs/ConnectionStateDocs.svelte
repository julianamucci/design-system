<script lang="ts">
  ...
  // O tipo vem apelidado porque a PORTA da peça já ocupa o nome nesta stack: um
  // é componente, o outro é vocabulário, e os dois cabem no mesmo arquivo.
  import {
    CONNECTION_STATES,
  ...
  // Não há linha sobrescrita aqui: as quatro props têm o mesmo nome nesta stack
  // e no conteúdo compartilhado — o aviso de que alguém pediu para tentar de
  // novo continua chegando por `onRetry` —, e a tabela dos rótulos descreve o
  // vocabulário de tela, que não muda conforme quem renderiza.
  const { tStore } = useTranslation(connectionTranslations);

  const labels = $derived(connectionStateLabelsFor($locale));

  /**
   * O contraexemplo do primeiro par: a palavra de cada estado apagada.
   *
   * A diferença entre a ligação de pé e a que caiu passa a existir só na cor do
   * ponto — e cor não chega a quem ouve a tela (WCAG 1.4.1). O texto sai dos
   * próprios rótulos traduzidos, esvaziados, e não de uma palavra cravada aqui.
   */
  const WORDLESS_STATE: ConnectionStateLabels = $derived({
    ...labels,
  ...
  // ─── Active section ──────────────────────────────────────────────────────────
  //
  // Não há seção de variantes: esta peça não tem eixo de forma. A linha é sempre
  // a mesma, e o que muda é se ainda há por onde pedir — que é a seção de
  // estados.
  ...
  const interfaceCode = `export interface ConnectionStateLabels {
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.connected')}
        </p>
        <ConnectionState state="connected" countdown={CONNECTION_COUNTDOWN} {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.reconnecting')}
        </p>
        <ConnectionState state="reconnecting" countdown={CONNECTION_COUNTDOWN} {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.disconnected')}
        </p>
        <ConnectionState state="disconnected" countdown={CONNECTION_COUNTDOWN} {labels} />
      </div>
    </div>
  </DocsDemonstration>
  ...
  <!-- ── Do & Don't ─────────────────────────────────────────────── -->
  <!--
    Os dois pares desenham o MESMO par de estados. O que muda é se a palavra
    chega a quem não vê a cor do ponto, e se a contagem só aparece onde há
    tentativa marcada.
  -->
  {#snippet doPair1()}
  ...
  {#snippet dontPair2()}
    <!--
      O contraexemplo é escrito À MÃO, e tem de ser: a peça recusa desenhar a
      contagem onde nenhuma tentativa está marcada, então não há argumento que
      produza o erro. A marcação vai literal, para que se veja o relógio que não
      corre ao lado de "Sem ligação".
    -->
    <div class="nds-stack nds-w-full" data-spacing="sm">
      <ConnectionState state="reconnecting" countdown={CONNECTION_COUNTDOWN} {labels} />
      <p

<script lang="ts">
  ...
  // conteúdo compartilhado descreve o tipo na API do primitivo de referência,
  // onde os controles chegam como lista de nós do documento. Aqui eles chegam
  // como bloco de marcação. O NOME da prop é o mesmo nas duas, e o aviso da
  // escolha continua sendo `onChoose` — então só o tipo diverge, mesma
  // divergência que `chat-thread` já registrou para as ações do turno.
  //
  // A DESCRIÇÃO nunca entra: ela já é neutra de API por regra, e vontade de
  // sobrescrevê-la é sinal de que o defeito está no texto compartilhado.
  const { tStore } = useTranslation(approvalTranslations, {
  ...
  const publishScope = $derived(approvalScopeOf(labels, 'publish'));
  const writeFileScope = $derived(approvalScopeOf(labels, 'writeFile'));
   ...
   * pareamento passa a viver na pontuação, e pontuação não sobrevive à
   * navegação por lista de um leitor de tela.
   */
  const inlineScopeQuestion = $derived(
    [
      labels.question.publish,
      publishScope.map((item) => `${item.term}: ${item.detail}`).join(' · '),
    ].join(' '),
  );
  ...
  // variante. O que existe são as FORMAS que ela toma do que recebe, e elas
  // moram na seção de estados.
  ...
  const interfaceCode = `export interface ApprovalScopeItem {
...
<!--
  Os controles vêm do mesmo andaime das stories, e é de propósito: eles são de
  quem consome, e esta página é quem consome. Nenhum deles recebe ênfase — dar
  destaque a "permitir" num cartão de autorização empurra para aprovar, e um
  design system que ensinasse isso o espalharia por todo produto que copiasse o
  exemplo.
-->
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
  ...
  >
    <div class="nds-stack nds-w-full" data-spacing="lg">
      {#each APPROVAL_EXAMPLE_NAMES as name, index (name)}
        {#if index > 0}
          <Separator />
        {/if}
        <div class="nds-stack nds-w-full" data-spacing="xs">
          <p class="nds-text-caption nds-text-muted-foreground">
            {stripHtml($tStore(`demonstration.labels.${name}`))}
          </p>
          <ApprovalCard
            question={labels.question[name]}
            scope={approvalScopeOf(labels, name)}
  ...
  <!-- ── Do & Don't ─────────────────────────────────────────────── -->
  <!--
    Os dois contraexemplos se montam com a MESMA API: é o que a peça não pode
    impedir — quem escreve a pergunta é quem consome.
  -->
  {#snippet doPair1()}
    <ApprovalCard
  ...
  {/snippet}
  {#snippet dontPair1()}
    <!-- O alcance entra embutido na frase, e a lista deixa de existir. -->
    <ApprovalCard question={inlineScopeQuestion} actions={choiceControls} />
  {/snippet}
  {#snippet doPair2()}
    <ApprovalCard
  ...
  {#snippet dontPair2()}
    <!--
      A pergunta genérica e sem alcance nenhum: quem responde está autorizando o
      que não viu.
    -->
    <ApprovalCard question={labels.question.vague} actions={choiceControls} />
  {/snippet}
  ...
  <DocsImport
    ...
    secondaryCode={$tStore('import.withActionsCode')}
  />

  <!-- ── Estados ────────────────────────────────────────────────── -->
  <!--
    Não há máquina de estados aqui, e isso é decisão da folha. A tabela descreve
    as FORMAS que o cartão toma do que recebe: com alcance, sem alcance, valor

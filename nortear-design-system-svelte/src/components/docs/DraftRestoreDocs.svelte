<script lang="ts">
  ...
  // Não há linha sobrescrita aqui: as props têm o mesmo nome nesta stack e no
  // conteúdo compartilhado. O retorno é um callback nas duas, e o carimbo chega
  // pelo mesmo nome.
  const { tStore } = useTranslation(draftTranslations);

  const labels = $derived(composerLabelsFor($locale));
  const draftLabels = $derived(draftLabelsFor($locale));

  /**
   * O rascunho JÁ CORTADO — o contraexemplo do primeiro par.
   *
   * O que sumiu aqui sumiu do documento também: some da busca do navegador e
   * nunca chega a quem ouve. É o oposto do corte da folha, que é só visual.
   */
  const TRUNCATED_DRAFT = `${SAMPLE_DRAFT_LONG.slice(0, 96)}…`;

  /**
   * O VERBO SOZINHO, tirado do próprio rótulo.
   *
   * O contraexemplo da decisão 3 é o rótulo sem o objeto, e derivá-lo do rótulo
   * traduzido é o que o mantém correto nos três idiomas — escrever o verbo à
  ...
  const VERB_ONLY_LABELS: DraftRestoreLabels = $derived({
    title: draftLabels.title,
    restore: firstWord(draftLabels.restore),
    discard: firstWord(draftLabels.discard),
  });
  ...
  // ─── Active section ──────────────────────────────────────────────────────────
  //
  // Não há seção de variantes: a peça tem um desenho só, e tudo o que muda nela
  // é o que ela encontrou. A navegação segue a do conteúdo compartilhado.
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.found')}
        </p>
        <DraftRestore labels={draftLabels} draft={SAMPLE_DRAFT} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.dated')}
        </p>
        <DraftRestore
          labels={draftLabels}
          draft={SAMPLE_DRAFT}
      ...
      <!--
        ...
        utilitário de pilha — a distância até o campo já é do desenho da faixa,
        e uma pilha por cima somaria o gap dela ao respiro da folha.
      -->
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.aboveComposer')}
        </p>
  ...
  {/snippet}
  {#snippet dontPair2()}
    <DraftRestore labels={VERB_ONLY_LABELS} draft={SAMPLE_DRAFT} />
  {/snippet}
    ...
    pairs={[
      {
        ...
        // O par é o MESMO rascunho longo: o que muda é quem corta. À esquerda o
        // texto vai inteiro e a folha corta; à direita ele chega já cortado, e
        // o que sumiu sumiu do documento também.
        doPreview: doPair1,
  ...
  <DocsImport
    ...
    secondaryCode={$tStore('import.withTimestampCode')}
  />
  ...
  <DocsRelated
    ...
    items={[
      ...
      { name: $tStore('related.items.sonner.name'),      description: toPlainText($tStore('related.items.sonner.description')),      path: '?path=/docs/components-feedback-sonner--docs' },
    ]}
  />

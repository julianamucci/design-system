<script lang="ts">
  ...
  // no conteúdo compartilhado — o aviso de dispensar é uma prop de função dos
  // dois lados —, e a tabela da citação descreve DADO, que não muda de
  // vocabulário conforme quem renderiza.
  const { tStore } = useTranslation(quoteTranslations);

  const labels = $derived(composerLabelsFor($locale));
  const quoteLabels = $derived(quoteLabelsFor($locale));
   ...
   */
  const TRUNCATED_QUOTE: ComposerQuote = {
    ...long,
   ...
   */
  const NAMELESS_DISMISS: ComposerQuoteLabels = $derived({
    ...quoteLabels,
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.short')}
        </p>
        <Composer {labels} {quoteLabels} quote={short} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        ...
        <Composer {labels} {quoteLabels} quote={long} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.withAttachments')}
        </p>
        <Composer
          {labels}
          {quoteLabels}
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.absent')}
        </p>
        <Composer {labels} />
      </div>
    </div>
  ...
  {/snippet}
  {#snippet dontPair1()}
    <Composer {labels} {quoteLabels} quote={TRUNCATED_QUOTE} />
  {/snippet}
  {#snippet doPair2()}
    <Composer
      {labels}
  ...
  <DocsImport
    ...
    secondaryCode={$tStore('import.fromThreadCode')}
  />

<script lang="ts">
  ...
  import {
    ...
    SAMPLE_LEVEL,
    voiceLabelsFor,
  } from '@/components/ui/composer/composer-voice.fixtures';
  ...
  // conteúdo compartilhado. O retorno é um callback nas duas, e o `state` chega
  // pelo mesmo nome.
  const { tStore } = useTranslation(voiceTranslations);

  const labels = $derived(composerLabelsFor($locale));
  const voiceLabels = $derived(voiceLabelsFor($locale));

  /**
   * O contraexemplo dos dois pares: a palavra do estado apagada.
   *
   * A diferença entre uma captura de pé e uma parada — e o motivo de o
   * alternador não responder — passa a existir só no desenho, e desenho não
   * chega a quem ouve a tela. O texto sai dos próprios rótulos traduzidos,
   * esvaziados, e não de uma palavra cravada aqui.
   */
  const WORDLESS_STATUS: ComposerVoiceLabels = $derived({
    ...voiceLabels,
  ...
  // é situação. A navegação segue a do conteúdo compartilhado.
  ...
  const interfaceCode = `export interface ComposerVoiceLabels {
...
<!--
  O trilho é um espaço, e nesta stack esse espaço é um `{#snippet}`. Ele vem
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.idle')}
        </p>
        <ComposerVoice labels={voiceLabels} state="idle" />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.recording')}
        </p>
        <ComposerVoice
          labels={voiceLabels}
          state="recording"
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.transcribing')}
        </p>
        <ComposerVoice
          labels={voiceLabels}
          state="transcribing"
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.inRail')}
        </p>
        <Composer {labels} railStart={railVoice} />
      </div>
    </div>
  </DocsDemonstration>
  ...
  {/snippet}
  {#snippet doPair2()}
    <ComposerVoice labels={voiceLabels} state="transcribing" elapsed={SAMPLE_ELAPSED_DONE} />
  {/snippet}
  {#snippet dontPair2()}
    <ComposerVoice labels={WORDLESS_STATUS} state="transcribing" elapsed={SAMPLE_ELAPSED_DONE} />
  {/snippet}
    ...
    pairs={[
      {
        ...
        // palavra a quem não vê o medidor.
        doPreview: doPair1,
  ...
  <DocsImport
    ...
    secondaryCode={$tStore('import.withLevelCode')}
  />

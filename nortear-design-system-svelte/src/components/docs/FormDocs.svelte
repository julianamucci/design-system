<script lang="ts">
  ...
  // nesta stack o controle vai no snippet `children`. Overrides trocam nome e
  ...
  const { tStore } = useTranslation(formTranslations, {
    ...
    'pt-BR': {
      'props.extensibility':
        '<code>FormField</code> aceita qualquer controle como filho — campo de texto, área de texto, seleção, marcação e composições. Use <code>class</code> para customizações pontuais com tokens do projeto.',
    },
    en: {
      'props.extensibility':
        '<code>FormField</code> accepts any control as its child — text field, text area, select, checkbox and compositions. Use <code>class</code> for custom styling with project tokens.',
    },
    es: {
      'props.extensibility':
        '<code>FormField</code> acepta cualquier control como hijo — campo de texto, área de texto, selección, marcación y composiciones. Usa <code>class</code> para personalización puntual con tokens del proyecto.',
    },
  });

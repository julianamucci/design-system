<script lang="ts">
  ...
  // Não há linha sobrescrita aqui: as duas props da peça têm o mesmo nome nesta
  // stack e no conteúdo compartilhado, e as tabelas dos rótulos e do passo
  // descrevem vocabulário de dado e de tela, que não muda conforme quem
  // renderiza.
  const { tStore } = useTranslation(planTranslations);
   ...
   * marcador — e nenhuma das duas chega a quem ouve a tela (WCAG 1.4.1). O texto
   * sai dos próprios rótulos traduzidos, esvaziados, e não de uma palavra
   * cravada aqui.
   */
  const WORDLESS_STATE: AgentPlanLabels = $derived({
    ...labels,
  ...
  // ─── Active section ──────────────────────────────────────────────────────────
  //
  // Não há seção de variantes: esta peça não tem eixo de forma. A lista é sempre
  // a mesma, e o que muda é em que pé está cada passo — que é a seção de estados.
...
<DocsPageLayout navGroups={NAV_GROUPS} activeSection={section.value}>
    ...
    <div class="nds-stack nds-w-full" data-spacing="lg">
      <div class="nds-stack nds-w-full" data-spacing="xs">
        <p class="nds-text-caption nds-text-muted-foreground">
          {$tStore('demonstration.labels.proposed')}
        </p>
        <AgentPlan steps={proposedPlan()} {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        ...
        <AgentPlan steps={runningPlan()} {labels} />
      </div>
      ...
      <div class="nds-stack nds-w-full" data-spacing="xs">
        ...
        <AgentPlan steps={finishedPlan()} {labels} />
      </div>
    </div>
  </DocsDemonstration>
  ...
  <!-- ── Do & Don't ─────────────────────────────────────────────── -->
  <!--
    O primeiro par é o MESMO plano: o que muda é se o passo que ficou pelo
    caminho continua na lista, com o motivo. O segundo é a MESMA grade de
    estados: o que muda é se a palavra chega a quem não vê o marcador.
  -->
  {#snippet doPair1()}
    <AgentPlan steps={finishedPlan()} {labels} />
  {/snippet}
  {#snippet dontPair1()}
    <!--
      O contraexemplo: o passo pulado some, e o plano passa a contar uma história
      diferente da que aconteceu.
    -->
    <AgentPlan steps={finishedPlan().filter(step => step.state !== 'skipped')} {labels} />
  {/snippet}
  {#snippet doPair2()}
    <AgentPlan steps={everyStatePlan()} {labels} />
  {/snippet}
  {#snippet dontPair2()}
    <!--
    ...
    -->
    <AgentPlan steps={everyStatePlan()} labels={WORDLESS_STATE} />
  {/snippet}
  ...
  <!-- ── Estados ────────────────────────────────────────────────── -->
  <!--
    A ordem sai de `PLAN_STEP_STATES`: a tabela e a story de estados leem a mesma
  ...
  <DocsProps
    ...
    tables={[
      ...
      },
      {
        ...
        items: ['labelsPlan', 'labelsState'].map(k => ({
          name: $tStore(`props.table.${k}.name`),
          type: $tStore(`props.table.${k}.type`),
          defaultValue: $tStore(`props.table.${k}.default`),
          required: $tStore(`props.table.${k}.required`),
      ...
      },
      {
        ...
        items: ['stepId', 'stepLabel', 'stepState', 'stepDetail'].map(k => ({
          name: $tStore(`props.table.${k}.name`),
          type: $tStore(`props.table.${k}.type`),
          defaultValue: $tStore(`props.table.${k}.default`),
          required: $tStore(`props.table.${k}.required`),

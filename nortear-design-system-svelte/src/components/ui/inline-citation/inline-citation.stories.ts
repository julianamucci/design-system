...
import InlineCitationDocs from '@/components/docs/InlineCitationDocs.svelte';
import { withAutoDocsTab } from '@/lib/withAutoDocsTab';

/**
 * Os dois eixos desta peça: o que a citação traz, e se a prévia nasce aberta.
 *
 * O control da citação é uma ESCOLHA entre os três desenhos que a peça faz
...
export const Playground: Story = {
  ...
  render: (args) => ({ Component: InlineCitationStory, props: { ...args } }),
  play: async ({ canvasElement, step, args }) => {
    ...
    // Nascer aberta acontece DEPOIS da montagem. Esperar por relógio antes de
    // ler é o que mantém a asserção honesta nos dois valores do control, e o
    // que a mantém fora da armadilha da espera por observador de mutação.

...
/**
 * Monta a lista com a largura das fotos.
 *
 * A largura entra pela prop `class`, e não por um invólucro: a peça já aceita
 * classe, e a `<ol>` é o elemento que precisa de limite — é ela que decide onde
 * o rótulo longo quebra.
 */
const mount = (steps: PlanStep[]) => ({
  Component: AgentPlan,
  props: { steps, labels: agentPlanLabels(), class: 'nds-max-w-lg' },
});

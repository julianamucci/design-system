...
export function parseEmbedMessage(provider: EmbedProvider, data: unknown): EmbedEvent | null {
  ...
  if (provider === 'youtube') {
    if (payload.event === 'onStateChange') {
      ...
      return state ? ({ type: state } as EmbedEvent) : null;
    }
  ...
  const eventData = payload.data as { seconds?: number; duration?: number } | undefined;
  if (eventName === 'play') return { type: 'playing' };
  if (eventName === 'pause') return { type: 'paused' };
  if (eventName === 'ended') return { type: 'ended' };
  if (eventName === 'timeupdate' && eventData && typeof eventData.seconds === 'number') {
    return { type: 'time', currentTime: eventData.seconds, duration: eventData.duration ?? 0 };
  }
  return null;
}

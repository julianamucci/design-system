...
import MediaPlayerTwoPlayersStory from './MediaPlayerTwoPlayersStory.svelte';
import {
  canvasStream,
  captionTrack,
  mediaPlayerLabels,
...
import { mediaPlayerVideoSource, mediaPlayerYouTubeSource } from './media-player.source';

/** Espiões de escopo de MÓDULO — dentro do render seriam inalcançáveis pela play. */
const firstPlay = fn();
const firstPause = fn();
...
/**
 * A composição dos dois players monta OUTRO componente — a marcação de dois
 * irmãos não cabe no `render` de uma story desta stack. O tipo acompanha o
 * componente que é montado; herdar o do `meta` cobraria as props do MediaPlayer
 * de um invólucro que não as tem.
 */
type TwoPlayersStory = StoryObj<typeof MediaPlayerTwoPlayersStory>;
 ...
 */
export const TwoPlayers: TwoPlayersStory = {
  parameters: {
    docs: {
  ...
  render: () => ({
    Component: MediaPlayerTwoPlayersStory,
    props: {
      onFirstPlay: firstPlay,
      onFirstPause: firstPause,
      onSecondPause: secondPause,
    },
  }),

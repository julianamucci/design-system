...
const meta: Meta = {
  title: 'Components/Form/Calendar/Layouts',
  component: Calendar,
  tags: ['form'],
  parameters: {
...
export const CaptionDropdown: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Trocar o mês no seletor leva o grid junto', async () => {
      ...
      await userEvent.selectOptions(monthSelector(), '6');
      await expect(
      ...
      await userEvent.selectOptions(monthSelector(), '4');
      await expect(
    ...
    await step('Trocar o ano no seletor leva o grid junto', async () => {
      ...
      const yearSelector = () => canvas.getAllByRole('combobox')[1];

...
import { chatThreadLabels, toMessages } from './chat-thread.fixtures';
import ChatThreadActionsStory from './ChatThreadActionsStory.svelte';
...
/**
 * As ações do turno entram por `{#snippet}`, que só existe dentro de marcação:
 * a story monta OUTRO componente, e o tipo acompanha o que é montado.
 */
type ActionsStory = StoryObj<typeof ChatThreadActionsStory>;
...
export const WithReasoningAndTools: Story = {
  ...
  render: () => ({
    Component: ChatThread,
    props: { messages: withTools(), labels: chatThreadLabels(), size: 'lg' },
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('O raciocínio nasce fechado — o destino vem antes do caminho', async () => {
      ...
      await expect(reasoning).toHaveTextContent('não da memória');
    });
    ...
    await step('A ferramenta diz o estado por escrito', async () => {
      ...
      await expect(call).toHaveTextContent(labels.toolState.done);
    });
    ...
    await step('A fonte de endereço recusado é legível, e NÃO é clicável', async () => {
      ...
      await expect(refused).toHaveTextContent('recusado');
      await expect(sources.querySelector('a[href^="javascript:"]')).toBeNull();
    });
  },
};

export const WithActions: ActionsStory = {
  parameters: { covers: ['accessibility.item4'] },
  render: () => ({ Component: ChatThreadActionsStory, props: { onCopy } }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('Em repouso as ações estão invisíveis — mas NO percurso do teclado', async () => {
      ...
      //
      // A precondição é própria: a play reexecuta no mesmo DOM, e a rodada
      // anterior terminou com o foco no botão.
      copy.blur();

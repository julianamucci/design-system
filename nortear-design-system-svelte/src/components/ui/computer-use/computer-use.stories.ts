...
import ComputerUseDocs from '@/components/docs/ComputerUseDocs.svelte';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(ComputerUseDocs),
...
export const Playground: Story = {
  ...
  render: (args) => ({ Component: ComputerUseStory, props: { ...args } }),
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('O endereço aparece, com a palavra que só quem ouve recebe', async () => {
      ...
      // A monoespaçada vem de uma utilitária que mora na MARCAÇÃO, não na folha
      // desta peça — e por isso some em silêncio quando alguém copia a árvore
      // pela metade. Esta asserção é o que impede o silêncio.
    ...
    await step('A tela entra no quadro, e a peça não toca no que ela traz', async () => {
      // A tela é ESPAÇO de quem consome: a peça renderiza o trecho inteiro e não
      // mexe em nada do que veio com ele — nem no texto alternativo, nem no
      // `inert` que faz dele uma foto em vez de um formulário para preencher.

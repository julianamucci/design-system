...
import JobProgressStory from './JobProgressStory.svelte';
import { jobProgressLabels } from './job-progress.fixtures';
...
import JobProgressDocs from '@/components/docs/JobProgressDocs.svelte';
 ...
 * há fração alguma. A grade dos cinco estados mora em `States`; aqui o assunto é
...
/**
 * A conta que os controls descrevem — a mesma leitura que o invólucro faz.
 *
 * Campo numérico vazio é ausência de conta, e não uma conta em branco. Total
 * zero é ausência pelo mesmo motivo, e quem responde por ele é o vocabulário
 * compartilhado.
 */
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(JobProgressDocs),
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A conta aparece escrita, e fica FORA do que é lido em voz', async () => {
      // Ela se reescreve a cada unidade processada, e a barra já carrega o mesmo
      // número — a cópia que sai é a que rerroda.
    ...
    await step('Nada na peça é região viva', async () => {
      ...
      // coisa: nada está bloqueado e ninguém deve resposta (decisão 1 da folha).
      // O que existe no lugar é a peça se declarar ocupada.

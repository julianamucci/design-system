<script lang="ts" module>
  ...
  // composição de `alert` com um medidor da família. As irmãs medem o que JÁ
  // FOI; esta mede o que AINDA HÁ, e o número da manchete é `teto − uso`. Virar
  // a medição do avesso muda a decisão de quem lê: "84% usados" se confere,
  ...
  // janela conhecida é uma contagem, custo sem orçamento é o comum. Aqui o teto
  // É O ASSUNTO — "quanto ainda resta" não tem resposta sem ele —, e por isso
  // ele não é opcional no tipo. A regra da família continua valendo, um passo
  // antes: em vez de desenhar um trilho vazio que leria como zero, a peça não
  // existe. Quem não tem teto não monta a faixa.
  //
   ...
   * opcional, existiria a faixa sem cota — que não é uma medição pela metade, é
   * a ausência da pergunta.
   */
  export interface QuotaAllowance {
     ...
     * É por isso que ele não é opcional, ao contrário do teto das irmãs.
     */
    limit: number;
  }
  ...
  export interface QuotaBannerLabels {
     ...
     * Ele não aparece na tela porque quem vê sabe pelo lugar em que a faixa
     * está; quem ouve não sabe.
     */
    title: string;
     ...
     * compartilhado reprova a compilação aqui, em vez de desenhar uma etiqueta
...
<script lang="ts">
  ...
  // função.
  const remaining = $derived(remainingUnits(quota.used, quota.limit));
  ...
  const fraction = $derived(spentFraction(quota.used, quota.limit) ?? 1);
  const percent = $derived(fractionPercent(fraction));
  const level = $derived(fractionLevel(fraction));
   ...
   * muda é o TEXTO da manchete, e texto não precisa de gancho de folha — zero
   * contado lê como medição, e não como fim.
   */
   ...
   * truncamento do inteiro. É este texto que permite à barra ser só desenho —
   * sem ele, ela viraria a única portadora da fração e passaria a dever 3:1
   * entre a parte cheia e a vazia.
   */
...
<!--
  `<div>`, e não `<p>` como as três irmãs: esta peça tem duas linhas de texto e
  uma caixa de controles, e botão dentro de parágrafo é marcação inválida.
  Nenhum papel ARIA, nenhuma região viva (decisão 2 da folha).

  `data-level` está SEMPRE presente, ao contrário das irmãs — lá o atributo some
  quando não há teto, e aqui não há caso sem teto.
-->
<div
  data-slot="quota-banner"
  class={cn('nds-quota-banner', className)}
  data-level={level}
>
  <!--
    O NÚMERO TEM NOME, e o nome é o ESCOPO (decisão 4). Ele não aparece na tela:
    quem vê já sabe de qual cota se trata pelo lugar em que a faixa está, e quem
    ouve não sabe.
  -->
  <span class="nds-sr-only" data-slot="quota-banner-title">{labels.title}</span>

  <p class="nds-quota-banner-headline" data-slot="quota-banner-headline">
    <span class="nds-quota-banner-remaining" data-slot="quota-banner-remaining"
      >{remainingText}</span
    >
    ...
    {#if renewsIn}
      ...
      -->
      <span class="nds-quota-banner-renews" data-slot="quota-banner-renews"
        >{labels.renews} {renewsIn}</span
      >
    {/if}
  ...
  <!--
    O medidor fica ENTRE as duas linhas de texto, e é por isso que ele vem aqui
    na marcação: pô-lo no fim exigiria `order` para desfazer a ordem de leitura,
    e `order` que discorda do foco é a armadilha da decisão 5.

    O MEDIDOR É DECORATIVO (decisão 1 da folha), e sai inteiro do que é lido em
    voz: a razão do rodapé já diz o mesmo, e repeti-la em desenho não acrescenta
    nada a quem ouve. Nenhum papel, nenhum `aria-valuenow` — um segundo portador
    da mesma fração a faria ser lida duas vezes, uma delas como controle.

    Valor de RUNTIME por custom property, como o medidor das três peças irmãs. A
    propriedade fica no TRILHO, e não no preenchimento — ela herda, então o
    ...
    viraria nó de texto dentro de um elemento cujo conteúdo tem de ser vazio — o
    medidor não fala, ele desenha.
  -->
  <span
    ...
    ><span class="nds-quota-banner-bar-fill"></span></span
  >

  <div class="nds-quota-banner-footer" data-slot="quota-banner-footer">
    <span class="nds-quota-banner-detail" data-slot="quota-banner-detail"
      >{detailText}</span
    >

...
import type { ApprovalScopeItem } from './index';
 ...
 * que a play procura é sempre o que o cartão desenha.
 */
export function approvalCardLabels(): ApprovalCardLabels {
  return approvalCardLabelsFor(get(locale));
}
 ...
 * consome passa os próprios controles pelo espaço que a peça abre e escolhe a
 * ênfase que quiser — a fronteira já é dele. O que não se faz é ENSINAR a
 * escolha aqui. Este parágrafo existe para que "deixar bonito" não desfaça a
 * decisão depois.
 *

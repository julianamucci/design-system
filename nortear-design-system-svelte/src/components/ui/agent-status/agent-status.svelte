<script lang="ts" module>
  ...
  //
  // A DECISÃO QUE GOVERNA A PEÇA: ela NÃO É REGIÃO VIVA, apesar de o estado
  // mudar sozinho. A tentação é grande, e é errada: quem lê está ouvindo a
  // RESPOSTA ser gerada logo abaixo, e um anúncio a cada troca de estado corta
  // a leitura no meio. Quem quiser anunciar põe a região viva por fora, sabendo
  // o que está fazendo — e é por isso que não há `aria-live` nem `role` nenhum
  // aqui.
  //
  // `stopped-run` DO CATÁLOGO É UM ESTADO DAQUI, e não outra peça: `RunStatus`
  // já separa `stopped` de `failed` porque um oferece continuar e o outro
  // tentar de novo. O desenho muda, o componente não.
  //
  // O QUE O COMPONENTE NÃO FAZ: parar, retomar, repetir, contar o tempo ou
  // formatá-lo. Ele desenha o estado que recebe e avisa que alguém pediu a
  // ação — mesma divisão de `approval` no `chat-thread` e da fila de envio.
  //
   ...
   * ditado por voz.
   *
   * `start` cobre retomar e refazer de propósito. A diferença entre continuar
   * de onde parou e começar do zero é política de produto — o design system
   * desenha a mesma linha nos dois casos, e é quem consome que sabe o que
   * sobrou.
   */
  ...
  export interface AgentStatusLabels {
     ...
     * vocabulário compartilhado reprova a compilação aqui, em vez de desenhar
     ...
     * enquanto corre, "Tentar de novo" depois da falha. Botão que troca de
     * função sem trocar de nome é o mesmo botão fazendo coisas diferentes, e
     * quem chega nele por tabulação não tem como saber qual das duas.
     *
     * Em espera e concluída ficam de fora nas cinco stacks, e é decisão:
     * começar uma execução é do campo de mensagem, não desta linha, e sobre uma
     * resposta pronta não há o que fazer aqui.
     */
...
<script lang="ts">
   ...
   */
  const intent = $derived<AgentStatusIntent>(isRunFinished(status) ? 'start' : 'stop');
</script>
...
<p
  ...
  class={cn('nds-agent-status', className)}
  data-status={status}
>
  <!--
    O PONTO É DECORATIVO (decisão 2). Ele é a leitura rápida para quem vê, e sai
    inteiro do que é lido em voz: a palavra ao lado já diz tudo, e repeti-la em
    desenho não acrescenta nada a quem ouve.
  -->
  <span
  ...
  <span
    ...
    data-slot="agent-status-label">{labels.status[status]}</span
  >

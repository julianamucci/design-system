...
import { traceWaterfallLabels } from './trace-waterfall.fixtures';
...
import TraceWaterfallDocs from '@/components/docs/TraceWaterfallDocs.svelte';
 ...
 * quanto do rastro aparece; e o EIXO decide onde cada barra cai — é ele que as
 * barras dividem, e é a única coisa que muda a posição de todas ao mesmo
 * tempo.
 *
 * O EIXO É CONTROLE DE PROPÓSITO, e é o que esta story existe para mostrar:
 * encolhê-lo não reescala nada, ele RECORTA — as barras guardam o instante em
 * que começam, e o que passa do fim some. Uma peça que derivasse o eixo dos
 * trechos não teria este controle, porque não teria a decisão.
 */
type PlaygroundArgs = {
  status: RunStatus;
  revealed: number;
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      ...
      // Sem docgen, o gerador de source monta a tag a partir do nome interno
      // da função compilada. A transform devolve o uso real.
...
export const Playground: Story = {
  ...
  // Os eixos chegam ao invólucro como eles são, e é ele que os transforma em
  // dado — a fatia da lista. Traduzir aqui deixaria o painel Code e a foto
  // lendo duas contas diferentes do mesmo controle.
  render: (args) => ({
    Component: TraceWaterfallStory,
    props: { ...args, testid: 'trace-waterfall-host' },
  }),
  play: async ({ canvasElement, step, args }) => {
    ...
    if (args.revealed === 0) {
      // Sem trecho não há cascata, e a peça não desenha nada — nem moldura,
      // nem camada que rola. Uma parada de teclado que leva a uma caixa vazia
      // é ruído com nome, e por isso a peça prefere não existir.
    ...
    await step('Há uma linha por trecho, na ordem em que foram declarados', async () => {
      // A posição no eixo é livre; a ordem no DOM não é, porque ela é a ordem
      // de leitura (WCAG 1.3.2). E a peça não ordena por começo.
      const rows = [...root.querySelectorAll<HTMLElement>('[data-slot="trace-waterfall-row"]')];
    ...
    await step('Cada barra cai na fração do eixo que o dado manda', async () => {
      ...
      const bars = [...root.querySelectorAll<HTMLElement>('[data-slot="trace-waterfall-bar"]')];
      ...
      for (const [index, bar] of bars.entries()) {
        ...
        // E a barra nunca passa do fim do eixo: encolher a régua recorta, não
        // reescala.
    ...
    await step('A camada que rola tem papel, nome e parada de teclado', async () => {
      ...
      // E é a ÚNICA parada: uma linha não faz nada, e uma dúzia de paradas que
      // não levam a lugar nenhum é ruído para quem usa teclado.

...
import TraceWaterfallEveryStateStory from './TraceWaterfallEveryStateStory.svelte';
import TraceWaterfallEmptyStory from './TraceWaterfallEmptyStory.svelte';
...
type EveryStateStory = StoryObj<typeof TraceWaterfallEveryStateStory>;
type EmptyStory = StoryObj<typeof TraceWaterfallEmptyStory>;
...
export const EveryState: EveryStateStory = {
  ...
  render: () => ({ Component: TraceWaterfallEveryStateStory }),
...
export const Failure: Story = {
  ...
  render: () => ({
    ...
    props: { spans: TRACE_SPANS_FAILURE, totalMs: TRACE_TOTAL_MS, status: 'failed' },
  }),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)!;
    ...
    await step('O trecho que quebrou desenha diferente do que terminou', async () => {
      ...
      // não é alcançado por portão nenhum, e um seletor que deixa de casar faz
      // a story LANÇAR em vez de reprovar.
...
export const Running: Story = {
  ...
  render: () => ({
    Component: TraceWaterfallStory,
    props: { spans: TRACE_SPANS_ORDER, totalMs: TRACE_TOTAL_MS, status: 'running' },
  }),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)!;
 ...
 *
 * Devolver uma moldura vazia seria pior que devolver nada — a camada que rola
 * é parada de teclado, e uma parada que leva a uma caixa vazia é ruído com
 * nome. E são DOIS casos, porque um eixo sem extensão não posiciona nada: o
 * rastro existe e não há régua para dividir.
 */
export const Empty: EmptyStory = {
  ...
  render: () => ({ Component: TraceWaterfallEmptyStory }),
  play: async ({ canvasElement, step }) => {

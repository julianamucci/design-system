...
import CostMeterAllLevelsStory from './CostMeterAllLevelsStory.svelte';
import {
  COST_METER_SPEND,
  amountOf,
  budgetOf,
...
// passa do teto, e a execução para a qual ninguém declarou orçamento. Nas três o
// desenho sozinho falharia — a barra fica igual em duas delas, e some na
...
export default meta;
type Story = StoryObj<typeof CostMeter>;

/**
 * A grade dos níveis monta o andaime, e não a peça.
 *
 * Ela decide o gasto de cada linha a partir da lista do primitivo, então não
 * recebe `amount` nem `budget` — e o tipo tem de sair do invólucro para dizer
 * isso. Tipá-la pela peça obrigaria a passar uma quantia que a grade ignoraria.
 */
type AllLevelsStory = StoryObj<typeof CostMeterAllLevelsStory>;
...
export const AllLevels: AllLevelsStory = {
  ...
  render: () => ({
    Component: CostMeterAllLevelsStory,
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Há uma linha por nível, na ordem do primitivo', async () => {
      await expect(lines).toHaveLength(BUDGET_LEVELS.length);
    });
 ...
 *
 * A barra para no cheio e o por cento trava em cem — o desenho não tem para onde
 * ir. A quantia escrita continua dizendo o valor de verdade, e é ela que mostra
 * o quanto passou.
 */
 ...
 *
 * O caso comum, e o que não pode parecer zero: sem orçamento não há fração, e um
 * trilho vazio leria como "não gastou nada".
 */

<script lang="ts" module>
  ...
  // guarda as seis decisões de acessibilidade. A CONTA — o por cento que se lê,
  // o limiar e o nível — vem de `@shared/primitives/token-budget`, e é a MESMA
  // que a peça da janela de contexto lê: se as duas aparecem lado a lado usando
  // a palavra "perto do teto", ela precisa querer dizer a mesma coisa nas duas.
  //
  ...
  // precedente que o tempo decorrido do ditado, o relógio do estado da execução
  // e o carimbo da faixa de rascunho já fixaram, e aqui ele vale com mais
  // força: duração tem separador e ordem; moeda tem símbolo, POSIÇÃO do
  // símbolo, separador de milhar, separador decimal e número de casas — e os
  // cinco variam por idioma E por moeda. A mesma quantia se escreve `US$ 0,42`,
  // `$0.42` ou `0,42 US$` conforme quem lê, e o símbolo troca de ponta entre um
  // idioma e outro. Quem sabe disso é quem escolheu a moeda, e não um
  // componente que decidiria idioma em cinco stacks.
  //
  ...
  // há `aria-live` em lugar nenhum, e nada aqui se reanuncia — um contador que
  // se reanuncia torna a tela impossível de ouvir enquanto a resposta é gerada
  // logo ao lado.
  //
  // O QUE O COMPONENTE NÃO FAZ: buscar preço, calcular tarifa, converter moeda,
  // formatar dinheiro, decidir o que fazer quando o orçamento acaba. Ele recebe
  // o que custou e desenha — §2 da guideline 17.
  //
  ...
  export interface CostBudget {
     ...
     * que entra na conta. Sai de `spentFraction`, do primitivo compartilhado,
     * que é quem guarda o recorte em uma volta e a resposta de que sem teto não
     * há fração nenhuma.
     */
    fraction: number;
  }
  ...
  export interface CostMeterLabels {
     ...
     *
     * "US$ 0,42" sozinho não diz de quê — nem de qual execução, nem se é do
     * turno ou da conversa inteira. O título não aparece na tela, porque quem vê
     * já sabe pelo lugar em que a peça está; quem ouve não sabe.
     */
    title: string;
    /**
     ...
     * ela, a ausência de fração vira informação: o que se sabe é quanto custou,
     * e não quanto ainda pode custar.
     */
    unbounded: string;
  }
...
<script lang="ts">
  ...
  // do número que se lê em voz — um gasto de verdade não sai como 0%, e um gasto
  // que não é tudo não sai como 100% — e o limiar de cada nível são a mesma
  // resposta nas cinco stacks. Escritas aqui, uma delas arredondaria para cima e
  // diria "100%" com espaço sobrando.
  const percent = $derived(budget ? fractionPercent(budget.fraction) : null);
  const level = $derived(budget ? fractionLevel(budget.fraction) : null);

  /**
   * O DETALHE MANTÉM A FRAÇÃO EM TEXTO, e não é adorno: é ele que permite à
   * barra ser só desenho. Se o por cento saísse da tela, a barra viraria a única
  ...
  const detailText = $derived(
    budget ? `${percent}% ${labels.of} ${budget.amount}` : labels.unbounded,
  );
</script>

<!--
  `<p>`, e não `<div>`: é uma frase sobre uma quantia, e a etiqueta de nível é
  conteúdo de frase. Nenhum papel ARIA, nenhuma região viva (decisão 1).

  Sem teto não há nível, e o atributo fica FORA — em vez de sair como uma palavra
  vazia que a folha ainda tentaria colorir.
-->
<p
  data-slot="cost-meter"
  class={cn('nds-cost-meter', className)}
...
>
  ...
  -->
  <span class="nds-sr-only" data-slot="cost-meter-title">{labels.title}</span>

  <!--
    A QUANTIA É SEMPRE A QUANTIA (decisão 6). Ao contrário da peça da janela, o
    valor não troca de significado entre os dois casos: dinheiro é dinheiro com
    teto e sem teto, e o que aparece e some é o que o QUALIFICA. Ela sai daqui
    exatamente como chegou — a peça não formata nada.
  -->
  <span class="nds-cost-meter-amount" data-slot="cost-meter-amount">{amount}</span>

  <span class="nds-cost-meter-detail" data-slot="cost-meter-detail">{detailText}</span>

  {#if budget && level !== null}
    <!--
    ...
    <!--
      ...
      A custom property é valor de RUNTIME, como o medidor das duas peças irmãs,
      e fica no TRILHO — ela herda, então o preenchimento a lê de graça, e o
      número mora sempre no mesmo elemento. O que entra é o MESMO inteiro que o
      detalhe mostra, e não a fração crua: uma barra cheia ao lado de "99%"
      seriam duas respostas para uma pergunta.
      ...
      E ele vem POR ÚLTIMO no DOM: a folha o joga para a segunda linha com uma
      declaração só, e como é `aria-hidden`, a ordem de leitura não muda.
    -->
    <span
      ...
      ><span class="nds-cost-meter-bar-fill"></span></span
    >
  {/if}
</p>

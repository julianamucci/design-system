...
import ActivityGraphNoWindowStory from './ActivityGraphNoWindowStory.svelte';
import {
  SCALE_DAYS,
  SCALE_EMPTY_DATE,
  SCALE_END,
...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// O estado aqui é da CASA — a força com que ela foi pintada —, e não da
// peça: o que a peça tem é o estado da execução que a escreve, e ele decide
// uma coisa só, se ela se declara ocupada. Por isso esta janela de uma semana
...
type NoWindowStory = StoryObj<typeof ActivityGraphNoWindowStory>;
...
export const Scale: Story = {
  ...
  render: () => ({
    ...
    props: { days: SCALE_DAYS, start: SCALE_START, end: SCALE_END },
  }),
 ...
 * da família: sem nó não há grafo e sem eixo não há cascata, mas um
 * trimestre em que nada aconteceu É a resposta.
 */
export const EmptyWindow: Story = {
  ...
  render: () => ({
    Component: ActivityGraphStory,
    props: { days: ACTIVITY_DAYS_EMPTY },
  }),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)!;
 ...
 *
 * É o único caso em que esta peça não desenha nada — e não é o caso da
 * atividade vazia, que continua sendo uma grade. Devolver moldura vazia
 * seria pior que devolver nada: a camada que rola é parada de teclado, e uma
 * parada que leva a uma caixa vazia é ruído com nome. E são DOIS casos,
 * porque sem degrau todo dia pintaria igual: a grade existe e não há escala
 * para dividi-la.
 */
export const NoWindow: NoWindowStory = {
  parameters: {
    covers: ['functional.item8'],
    docs: { source: { transform: activityGraphNoWindowSnippet } },
  },
  render: () => ({ Component: ActivityGraphNoWindowStory }),
  play: async ({ canvasElement, step }) => {

...
import ActivityGraphStory from './ActivityGraphStory.svelte';
import ActivityGraphTightCellsStory from './ActivityGraphTightCellsStory.svelte';
import {
  OUTSIDE_DAY,
  WIDE_END,
  WIDE_START,
 ...
 */
type Story = StoryObj<typeof ActivityGraphStory>;
type TightCellsStory = StoryObj<typeof ActivityGraphTightCellsStory>;
 ...
 * que terminam hoje, e não há como pedir março.
 */
export const SingleMonth: Story = {
  ...
  render: () => ({
    Component: ActivityGraphStory,
    props: { start: ACTIVITY_MONTH_START, end: ACTIVITY_MONTH_END },
  }),
 ...
 *
 * Cinquenta e três colunas passam de qualquer conversa com o tamanho de casa
 * que a folha declara, e é aí que a camada rola — uma só, com nome e parada
 * de teclado.
 */
export const WholeYear: Story = {
  ...
  render: () => ({
    Component: ActivityGraphStory,
    props: { start: WIDE_START, end: WIDE_END, hostClass: 'nds-max-w-md' },
  }),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
...
export const OutsideDay: Story = {
  ...
  render: () => ({
    Component: ActivityGraphStory,
    props: { days: [...ACTIVITY_DAYS, OUTSIDE_DAY] },
  }),
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement);
 ...
 */
export const TightCells: TightCellsStory = {
  parameters: {
    covers: ['visual.item8'],
    docs: { source: { transform: activityGraphTightCellsSnippet } },
  },
  render: () => ({ Component: ActivityGraphTightCellsStory }),
  play: async ({ canvasElement, step }) => {

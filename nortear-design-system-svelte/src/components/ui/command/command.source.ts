...
/**
 * Composição AsCombobox: a paleta não flutua sozinha — quem flutua é o Popover.
 *
 * O gatilho anuncia a escolha por `role="combobox"`, e esse papel não aceita
...
const rotulo = $derived(
  itens.find((item) => item.value === selecionado)?.label ?? 'Selecione um item...',
);

function escolher(value: string) {
  selecionado = value;
  // Fechar faz parte do gesto: aberto, o painel cobre o valor recém-escolhido.
  aberto = false;
}`,
    `<Popover bind:open={aberto}>
  <span id="combobox-rotulo" class="nds-sr-only">Componente</span>
  <PopoverTrigger>
    {#snippet child({ props })}
      <Button
        variant="outline"
        ...
        {...props}
      >
        <span id="combobox-valor">{rotulo}</span>

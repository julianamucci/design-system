<script lang="ts">
	 ...
	 *
	 * `.nds-drawer-body` traz `flex: 1 1 auto`, `min-height: 0` e
	 * `overflow: auto`. A base `auto` é o que faz o corpo contribuir com a
	 * altura do conteúdo para o painel — com a base zero do atalho `flex: 1` o
	 * teto de altura nunca aperta ninguém, e o conteúdo transborda em vez de
	 * rolar. O `min-height: 0` é o que o deixa ceder altura dentro do flex em
	 * coluna, em vez de esticar o painel e empurrar o rodapé (com as ações)
	 * para fora da tela.
	 */
	let {
		ref = $bindable(null),

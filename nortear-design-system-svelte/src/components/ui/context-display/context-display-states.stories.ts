...
import ContextDisplayAllLevelsStory from './ContextDisplayAllLevelsStory.svelte';
import {
...
const meta: Meta<typeof ContextDisplay> = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: contextDisplayAllLevelsSource },
      description: {
        component:
 ...
 *
 * Ela decide a medição de cada bloco a partir da lista do primitivo, então não
 * recebe `usage` — e o tipo tem de sair do invólucro para dizer isso. Tipá-la
 * pela peça obrigaria a passar uma medição que a grade ignoraria.
 */
type AllLevelsStory = StoryObj<typeof ContextDisplayAllLevelsStory>;
...
export const AllLevels: AllLevelsStory = {
  ...
  render: () => ({
    Component: ContextDisplayAllLevelsStory,
    props: { labels: contextDisplayLabels() },
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada um traz a PALAVRA daquele nível', async () => {
      ...
      for (const [i, level] of BUDGET_LEVELS.entries()) {
        const badge = blocks[i]!.querySelector<HTMLElement>(

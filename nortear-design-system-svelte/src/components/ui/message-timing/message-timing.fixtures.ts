...
import type { MessageTimingLabels, MessageTimingStat } from './index';

/**
 * A anotação de tipo é o PORTÃO: a seção `labels` é lida como
 * `MessageTimingLabels` em CADA idioma, então rótulo que sumir do JSON — ou
 * idioma que ficar para trás — reprova no type-check, e não na tela. Sem a
 * palavra do estado, a peça em movimento ficaria distinguível só pela ausência
 * de uma etiqueta que ninguém veria faltar.
 */
const CONTENT: Record<
  Locale,
  {
    labels: MessageTimingLabels & {
      speedUnit: string;
      stats: Record<'firstToken' | 'total' | 'speed' | 'chunks', string>;
    };
  }
> = messageTimingTranslations;
 ...
 */
function durationTextFor(target: Locale, value: number, unit: DurationUnit): string {
 ...
 * liga ao teto.
 */
function speedTextFor(target: Locale, value: number): string {
...
function countTextFor(target: Locale, value: number): string {
...
export function messageTimingLabels(): MessageTimingLabels {
  return messageTimingLabelsFor(get(locale));
}
...
/**
 * As medições de cada caso, num idioma, na ordem em que quem mediu as produziu.
 *
 ...
 */
export function statsOfFor(target: Locale, name: MessageTimingCase): MessageTimingStat[] {
...
/** As medições daquele caso, já escritas no idioma da página. */
export function statsOf(name: MessageTimingCase): MessageTimingStat[] {
  return statsOfFor(get(locale), name);
}
...
/**
 * O gatilho da forma compacta, montado por QUEM CONSOME — num idioma.
 *
...
/** O rótulo do gatilho da forma compacta, no idioma da página. */
export function messageTimingTriggerLabel(): string {
  return messageTimingTriggerLabelFor(get(locale));
}

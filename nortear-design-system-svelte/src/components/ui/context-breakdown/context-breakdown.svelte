<script lang="ts" module>
  ...
  // as opções são props — a mesma divergência já registrada na peça irmã desta
  // stack. Não há evento nem retorno: a peça é só leitura, e é justamente o
  // evento o ponto em que as cinco stacks costumam deixar de se parecer. As duas
  // props que importam têm o mesmo nome nas cinco. Markup, classes `.nds-*`,
  // `data-slot`, ARIA e comportamento são os mesmos.
  //
  // O vocabulário mora neste bloco de módulo, e não no índice da pasta, porque a
  // peça é autônoma: ela não entra na API de nenhuma outra, e quem a usa a
  // importa por inteiro — componente e tipos — de um lugar só.

  export interface ContextBreakdownLabels {
    /**
     * O que está sendo repartido.
     *
     * Aparece na tela, ao contrário do título da peça irmã: lá o número mora
     * numa linha cujo lugar já diz do que se trata, e aqui o que se vê é uma
     * lista de nomes e números que sem título não diz o que foi dividido.
     */
    title: string;
...
<script lang="ts">
  ...
  const slices = $derived(contextSlices(parts));
  const total = $derived(contextTotal(parts));

  /** `25.000 tokens` — o mesmo formato no total e em cada parcela. */
  function countText(tokens: number): string {
    return `${tokens.toLocaleString()} ${labels.unit}`;
  }
  ...
  function nameOf(id: string): string {
    return labels.parts[id] ?? id;
  }
</script>

<!--
  `<div>`, e não `<p>`: o corpo desta peça é uma lista, e lista não cabe dentro
  de parágrafo. Nenhum papel ARIA, nenhuma região viva (decisão 1 da folha).

  Não há atributo de estado, e a ausência é decisão: o que varia aqui é quantas
  parcelas chegaram e quanto cada uma pesa, e as duas coisas são desenho por
  posição, não por estado.
-->
<div data-slot="context-breakdown" class={cn('nds-context-breakdown', className)}>
  <!--
  ...
  -->
  <p class="nds-context-breakdown-headline" data-slot="context-breakdown-headline">
    <span
      class="nds-context-breakdown-title"
      data-slot="context-breakdown-title">{labels.title}</span
    >
    <span
      class="nds-context-breakdown-total"
      data-slot="context-breakdown-total">{countText(total)}</span
    >
  </p>
  ...
  <!--
    ...
    nenhum `aria-valuenow` — um segundo portador dos mesmos números os faria ser
    lidos duas vezes, uma delas como controle.

    As fatias vêm coladas ao trilho de propósito, como o preenchimento da peça
    irmã: espaço entre as tags viraria nó de texto dentro de um elemento cujo
    conteúdo tem de ser vazio — a barra não fala, ela desenha.
    ...
    pareamento de cor por `:nth-child` que a folha declara. Uma fatia a menos não
    desapareceria da tela: ela deslocaria a cor de todas as seguintes.

    A chave é o ÍNDICE de propósito: aqui a posição É a identidade, e é dela que
    a cor sai. Chavear pelo endereço quebraria no dia em que a medição trouxesse
    duas parcelas com o mesmo endereço, e a cor escorregaria de fatia sem que
    nada reprovasse.

    Valor de RUNTIME por custom property, como o medidor da peça irmã já faz. O
    que entra é o MESMO inteiro que a legenda mostra, e não a fração crua: uma
    fatia que discordasse do número ao lado seriam duas respostas para uma
    pergunta só.
  -->
  <span
    ...
    aria-hidden="true"
    >{#each slices as slice, index (index)}<span
        class="nds-context-breakdown-slice"
        data-slot="context-breakdown-slice"
        style="--nds-context-share: {slice.percent}"
      ></span>{/each}</span
  >

  <!--
    A LEGENDA É UMA LISTA DE VERDADE (decisão 3): a contagem e a posição chegam a
    quem ouve, e é por posição que esta repartição pede para ser comparada de um
    turno para o seguinte.
  -->
  <ul class="nds-context-breakdown-legend" data-slot="context-breakdown-legend">
    {#each slices as slice, index (index)}
      <li class="nds-context-breakdown-part" data-slot="context-breakdown-part">
        <!--
        ...
        <!--
          ...
          palavra para o endereço, o endereço é o que aparece.
        -->
        <span
          class="nds-context-breakdown-name"
          data-slot="context-breakdown-name">{nameOf(slice.id)}</span
        >
        <span
          class="nds-context-breakdown-tokens"
          data-slot="context-breakdown-tokens">{countText(slice.tokens)}</span
        >
        <span
          class="nds-context-breakdown-percent"
          data-slot="context-breakdown-percent">{slice.percent}%</span
        >
      </li>
    {/each}
  </ul>

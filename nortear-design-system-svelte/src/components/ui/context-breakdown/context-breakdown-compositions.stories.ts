...
import ContextBreakdownBesideBudgetStory from './ContextBreakdownBesideBudgetStory.svelte';
import ContextBreakdownInsideDisclosureStory from './ContextBreakdownInsideDisclosureStory.svelte';
import { contextBreakdownLabels, partsOf } from './context-breakdown.fixtures';
import { usageOf } from '@/components/ui/context-display/context-display.fixtures';
import {
...
// isso.
//
// A divergência de API do bloco de expansão está registrada no invólucro que o
// monta: aqui ele é raiz, gatilho e conteúdo, e a abertura inicial se declara
// por estado ligado à raiz.
...
export default meta;
type Story = StoryObj<typeof ContextBreakdown>;
...
export const BesideBudget: Story = {
  ...
  render: () => ({
    Component: ContextBreakdownBesideBudgetStory,
    props: { parts: partsOf('typical'), labels: contextBreakdownLabels() },
  }),
...
export const InsideDisclosure: Story = {
  ...
  render: () => ({
    Component: ContextBreakdownInsideDisclosureStory,
    props: { parts: partsOf('typical'), labels: contextBreakdownLabels() },
  }),
  play: async ({ canvasElement, step }) => {
    const breakdown = blockOf(canvasElement);

...
const meta: Meta<typeof ComposerVoice> = {
...
export default meta;
type Story = StoryObj<typeof ComposerVoice>;

/** Espião de escopo de módulo: dentro do render, a play não o alcança. */
const onToggle = fn();

const mount = (state: VoiceState, elapsed?: string) => ({
  Component: ComposerVoice,
  props: {
    labels: voiceLabels(),
    state,
...
export const Recording: Story = {
  ...
  render: () => mount('recording', SAMPLE_ELAPSED),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    const labels = voiceLabels();
...
export const Transcribing: Story = {
  ...
  render: () => mount('transcribing', SAMPLE_ELAPSED_DONE),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    const labels = voiceLabels();
...
export const Disabled: Story = {
  ...
  render: () => ({
    Component: ComposerVoice,
    props: { labels: voiceLabels(), state: 'idle', disabled: true, onToggle },
  }),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);

...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// O que ACONTECE com a lista: ela está fechada, ou está aberta com um modelo que
// não pode responder agora. A forma de cada opção — etiqueta, descrição — mora
// em `Variants`.
...
export const Closed: Story = {
  ...
  render: () => ({
    Component: ComposerModelPickerStory,
    props: { models: everyModel(), value: 'balanced', onValueChange },
  }),
...
export const Unavailable: Story = {
  ...
  render: () => ({
    Component: ComposerModelPickerStory,
    props: { models: everyModel(), value: 'fast', open: true, onValueChange },
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('E o motivo está em TEXTO, dentro dela', async () => {
      // Quem não percebe o cinza não recebe nem a pista, então o cinza não pode
      // ser o único portador.
    ...
    await step('A lista tem nome próprio, para quem chega por audição', async () => {
    ...
    await step('Confirmá-la não troca nada, e a lista continua aberta', async () => {
      ...
      await expect(
        panel.querySelector('[aria-selected="true"]')!.getAttribute('data-model-id'),
      ).toBe('fast');
    });
  },
};

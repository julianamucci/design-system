...
import DraftRestore from './draft-restore.svelte';
import {
  SAMPLE_DRAFT,
  SAMPLE_DRAFT_LONG,
  SAMPLE_TIMESTAMP,
...
const meta: Meta<typeof DraftRestore> = {
...
export default meta;
type Story = StoryObj<typeof DraftRestore>;

/** Espião de escopo de módulo: dentro do render, a play não o alcança. */
const onAction = fn();

const mount = (draft: string, timestamp?: string) => ({
  Component: DraftRestore,
  props: { labels: draftLabels(), draft, timestamp, onAction },
});
...
export const Dated: Story = {
  ...
  render: () => mount(SAMPLE_DRAFT, SAMPLE_TIMESTAMP),
  play: async ({ canvasElement, step }) => {
...
export const LongDraft: Story = {
  ...
  render: () => mount(SAMPLE_DRAFT_LONG, SAMPLE_TIMESTAMP),
  play: async ({ canvasElement, step }) => {

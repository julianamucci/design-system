...
const mount = (models: ModelOption[]) => ({
  Component: ComposerModelPickerStory,
  props: { models, open: true, onValueChange },
});
...
export const Descriptions: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A lista tem NOME PRÓPRIO e é uma lista de opções', async () => {
      // Sem nome, ela é "lista" ao lado de outras listas da tela — e o campo do
      // composer tem três.
    ...
    await step('E o cursor da lista aponta uma opção, sem depender de foco visível', async () => {
      ...
      const pointedEl = panel.ownerDocument.getElementById(pointed!);
      await expect(pointedEl).not.toBeNull();
      await expect(panel.contains(pointedEl)).toBe(true);
    });
  },
};
...
export const WithBadge: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A etiqueta desenha com o badge do sistema, no lugar da folha', async () => {
      ...
      // Duas classes, dois papéis: o desenho vem do badge, o lugar na grade vem
      // da folha do composer.

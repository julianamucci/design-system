...
import ComposerAttachmentsDocs from '@/components/docs/ComposerAttachmentsDocs.svelte';
...
// docs page não tem seção de variantes na navegação. Sem control nenhum, o
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O tamanho aparece convertido, com a unidade em palavra', async () => {
      // A conta vem do primitivo compartilhado; a palavra, dos rótulos. O que
      // se afirma é a conversão: o arquivo de 2.516.582 bytes se lê em
      // megabytes, e o número de bytes não aparece em lugar nenhum.
      const first = list.children[0]!;
      await expect(first).toHaveTextContent(sizeLabel(SIZE_MB, rotulos));

 ...
 *
 * Os rótulos do CAMPO não moram aqui: nesta stack eles já são os mesmos para
 * todas as peças da moldura, e `composer.fixtures.ts` é quem os tem — quem
 * precisa do campo importa `composerLabelsFor` de lá. Uma segunda cópia
 * divergiria sem sinal, que é exatamente o que este arquivo existe para evitar.
 *
...
import type { DraftRestoreLabels } from './draft-restore.svelte';
 ...
 * controle sem nome vira um destino sem assunto para quem chega nele por
 * tabulação, que é o defeito que a decisão 3 da folha existe para não ter.
 */
...
/**
 * Os rótulos da faixa fora de um componente — `props` de story e `play` não
 * são render.
 *
 * Lê a MESMA store de locale que o `useTranslation` da página, então o rótulo
 * que a play procura é sempre o que a faixa desenha.
 */
export function draftLabels(): DraftRestoreLabels {
  return draftLabelsFor(get(locale));
}

...
/**
 * Os rótulos do painel fora de um componente — `props` de story e `play` não são
 * render.
 *
 * Lê a MESMA store de locale que o `useTranslation` da página, então o rótulo
 * que a play procura é sempre o que o painel desenha.
 */
export function triggerLabels(): TriggerPopoverLabels {
  return triggerLabelsFor(get(locale));
}
...
/** A mesma menção fora de um componente. */
export function mentionSource(): TriggerSource {
  return mentionSourceFor(get(locale));
}

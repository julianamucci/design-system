<script lang="ts" module>
  ...
  // contexto é REFERÊNCIA: aponta para o que já está lá, não sobe nada e não
  // tem o que esperar. É por isso que aqui não há `state` nem `progress`: não
  // existe espera para comunicar, e uma barra parada seria uma barra mentindo.
  //
  ...
  // que alguém pediu para tirar um item. Quem consome monta a pergunta e
  // decide — mesma divisão de `approval` no `chat-thread`.
  import type { ContextKind } from '@shared/primitives/chat-protocol';

  /**
   * O vocabulário da lista.
   *
   * Tudo aqui é TEXTO de interface, e por isso tem três idiomas: a espécie que
   * chega a quem ouve, a marca do que entrou sozinho e o nome do botão que
   * tira um item.
   */
...
<script lang="ts">
   ...
   *
   * Ele é DECORATIVO, e a espécie viaja em texto dentro do item (decisão 2 da
   * folha): o ícone é a única pista visual de que aquilo é um trecho e não o
   * arquivo inteiro, e pista que só existe em desenho não chega a quem ouve
   * (WCAG 1.1.1).
   *
   ...
   */
  const KIND_ICONS: Record<ContextKind, typeof File> = {
    selection: SquareDashedText,
    file: File,
    directory: Folder,
    page: AppWindow,
  ...
  /**
   * O nome acessível leva o NOME DO ITEM.
   *
   * Uma lista de botões chamados "Remover" é um botão só para quem navega por
   * audição (decisão 4 da folha).
   */
  function removeLabel(item: ContextItem): string {
    return labels.remove.replace('{label}', item.label);
  }
</script>

<!--
  `<ul>`: é o que faz o leitor de tela dizer QUANTOS itens a pergunta leva antes
  de percorrê-los, e aqui a contagem é a informação — saber que são sete
  arquivos muda o que se pergunta. Uma pilha de `div` não anuncia nada.
-->
<ul data-slot="composer-context" class="nds-composer-context" aria-label={labels.list}>
  {#each items as item, index (item.id ?? index)}
    ...
    >
      ...
      <Icon class="nds-composer-context-icon" aria-hidden="true" />

      <!--
        A espécie em TEXTO, dentro do item (decisão 2 da folha). Ela não vai num
        `aria-label` do `<li>`: rótulo em item de lista SUBSTITUI o conteúdo no
        anúncio, e o recorte — que é o que separa um pedaço do todo — sairia
      ...
      -->
      {#if item.detail}
        <span class="nds-composer-context-detail">{item.detail}</span>
      {/if}
      ...
      {:else}
        <span
          ...
          data-slot="composer-context-automatic">{labels.automatic}</span
        >
      {/if}
    </li>
  {/each}

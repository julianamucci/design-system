...
import type { ComposerQuote } from "@/components/ui/composer-quote";
...
// ─── Nav ─────────────────────────────────────────────────────────────────────
//
// Sem seção de variantes, e não por esquecimento: a citação tem uma forma só. O
// que muda entre um caso e outro é o TRECHO — cabe, não cabe, não existe —, e
// isso é estado.
...
export function ComposerQuoteDocs() {
  ...
  /** Um composer com citação, sem repetir os rótulos em cada chamada. */
  const withQuote = (quote?: ComposerQuote, comAnexos = false) => (
    <Composer
      labels={labels}
      {...(quote ? { quote, quoteLabels } : {})}
      {...(comAnexos
        ? { attachments: queue().slice(0, 2), attachmentLabels }
        : {})}
    />
  );
  ...
  const examples = [
    example("demonstration.labels.short", withQuote(shortQuote())),
    example("demonstration.labels.long", withQuote(longQuote())),
    example("demonstration.labels.withAttachments", withQuote(shortQuote(), true)),
    example("demonstration.labels.absent", withQuote(undefined)),
  ];

  /** A mesma citação longa, mas com o trecho cortado em código. */
  ...
  return (
    ...
    >
        ...
        pairs={[
          {
            ...
            doPreview: withQuote(longQuote()),
            // O contraexemplo: o trecho cortado em código. Na tela as duas
            // parecem iguais; a diferença só existe para quem ouve.
            dontPreview: (
              <Composer
                labels={labels}
          ...
          },
          {
            ...
            doPreview: withQuote(shortQuote(), true),
            // O contraexemplo: o botão sem o nome de quem escreveu, ao lado dos
            // botões de anexo — três "×" indistinguíveis por audição.
            //
            // O texto sai do próprio rótulo traduzido, com o lugar do nome
            // removido — assim o contraexemplo continua nos três idiomas, e não
            // vira uma palavra portuguesa cravada aqui.
            dontPreview: (
              <Composer
                labels={labels}
                quote={shortQuote()}
      ...
      <DocsImport
        ...
        secondaryCode={tContent("import.fromThreadCode")}
      />

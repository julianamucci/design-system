...
// ─── Nav ─────────────────────────────────────────────────────────────────────
//
// Sem seção de variantes, e não por esquecimento: o eixo desta peça é ESTADO —
// na fila, enviando, pronto, falhou —, e não há variante de forma para listar.
...
export function ComposerAttachmentsDocs() {
  ...
  /** Um composer com anexos, sem repetir os rótulos em cada chamada. */
  const withAttachments = (attachments: Attachment[]) => (
    <Composer labels={labels} attachmentLabels={attachmentLabels} attachments={attachments} />
  );
  ...
  const examples = [
    example("demonstration.labels.queue", withAttachments(queue())),
    example(
      "demonstration.labels.uploading",
      withAttachments(one("uploading", { progress: 0.4 })),
    ),
    example("demonstration.labels.failed", withAttachments(one("failed"))),
    example("demonstration.labels.withField", withAttachments(queue().slice(0, 2))),
  ];
  ...
  return (
    ...
    >
        ...
        pairs={[
          {
            ...
            // quem não vê a tela.
            doPreview: withAttachments(queue().slice(0, 2)),
            // O contraexemplo: o rótulo SEM o nome do arquivo, e a fila inteira
            // vira o mesmo botão para quem ouve.
            //
          ...
          },
          {
            ...
            doPreview: withAttachments(one("uploading", { progress: 0.4 })),
      ...
      <DocsImport
        ...
        secondaryCode={tContent("import.withProgressCode")}
      />

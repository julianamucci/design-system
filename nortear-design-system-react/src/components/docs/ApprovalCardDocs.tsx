...
const INTERFACE_CODE = `export interface ApprovalScopeItem {
...
//
//   <Button data-approval-choice="allow-once">…</Button>
//
...
export function ApprovalCardDocs() {
  ...
  // prop é o mesmo nas duas, então só o tipo diverge — mesma divergência que
  // `chat-thread` já registrou para as ações do turno.
  ...
  // O hook, e não a função pura: é ele que subscreve a loja e faz os cartões
  // desta página se redesenharem quando o idioma muda. A função pura serve à
  // `play`, onde não há componente para pendurar um hook.
  const labels: ApprovalCardLabels = useApprovalCardLabels();
  ...
  /**
   * Um cartão inteiro daquele exemplo.
   *
   * Os controles vêm do mesmo andaime das stories, e é de propósito: eles são
   * de quem consome, e esta página é quem consome.
   */
  const card = (name: ApprovalExampleName) => (
    <ApprovalCard
      question={approvalQuestion(labels, name)}
      scope={approvalScope(labels, name)}
      actions={approvalChoices(labels)}
    />
  );
  ...
  const examples = APPROVAL_EXAMPLE_NAMES.map((name) =>
    example(`demonstration.labels.${name}`, card(name)),
  );

  // O contraexemplo do primeiro par se monta com a MESMA API: o alcance entra
  // embutido na frase, e a lista deixa de existir. É o que a peça não pode
  // impedir — quem escreve a pergunta é quem consome.
  const scopeInsideTheQuestion = (
    <ApprovalCard
      question={[
        approvalQuestion(labels, "publish"),
        approvalScope(labels, "publish")
  ...
  const vagueQuestion = (
    <ApprovalCard
      question={approvalVagueQuestion(labels)}
      actions={approvalChoices(labels)}
    />
  );
  ...
  return (
    ...
    >
        ...
        pairs={[
          {
            ...
            doPreview: card("publish"),
            dontPreview: scopeInsideTheQuestion,
          },
          {
            doLabel: tNav("common.do"),
            dontLabel: tNav("common.dont"),
            ...
            doPreview: card("writeFile"),
            dontPreview: vagueQuestion,
          },
        ]}
      />
      ...
      <DocsImport
        ...
        secondaryCode={tContent("import.withActionsCode")}
      />

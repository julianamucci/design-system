...
const INTERFACE_CODE = `export interface FlowGraphLabels {
...
interface WrongGraphProps {
  nodes: readonly FlowNode[];
  edges: readonly FlowEdge[];
  /** A frase que cada nó diz a quem ouve. Ausente, sobra a curva. */
  withReading: boolean;
...
/**
 * O grafo montado À MÃO, e tem de ser: a peça sempre escreve a leitura de cada
 * nó e sempre põe papel e nome na camada que rola, então não há propriedade que
 * produza o erro. A marcação abaixo é a mesma da peça, com uma parte removida
 * de cada vez.
 */
function WrongGraph({ nodes, edges, withReading, named }: WrongGraphProps) {
  const labels = useFlowGraphLabels();
  const drawing = resolveFlowGraph(nodes, edges);
  if (!drawing) return null;

  return (
    <div className="nds-flow-graph" data-slot="flow-graph" aria-busy>
      <div
              ...
              >
                ...
                {withReading ? (
                  <span className="nds-sr-only" data-slot="flow-graph-node-reading">
                    ...
                    {drawn.dependsOn.length > 0
                      ? `${labels.state[drawn.node.state]} ${labels.dependsOn.replace(
                          "{sources}",
                          drawn.dependsOn.join(", "),
                        )}`
                      : labels.state[drawn.node.state]}
                  </span>
                ) : null}
              </li>
            ))}
...
export function FlowGraphDocs() {
  ...
  // O hook, e não a função pura: é ele que subscreve a loja e faz as peças desta
  // página se redesenharem quando o idioma muda.
  const labels = useFlowGraphLabels();
  ...
  /** Um grafo, sem repetir as ligações e os rótulos em cada chamada. */
  const piece = (
    nodes: readonly FlowNode[],
  ...
  /** O grafo largo dentro de um teto de largura, que é o que o faz rolar. */
  ...
  const examples = [
    ...
    example("demonstration.labels.wide",    inNarrowColumn(piece(wideFlowNodes(), wideFlowEdges()))),
  ];
        ...
        pairs={[
          {
            ...
            doPreview: stackOf(piece(FLOW_NODES_ORDER)),
            // Sem a frase de cada nó sobra a curva — que é exatamente o que não
            // chega a quem lê de ouvido.
            dontPreview: stackOf(
              <WrongGraph
          ...
          },
          {
            ...
            doPreview: stackOf(
              inNarrowColumn(piece(wideFlowNodes(), wideFlowEdges())),
            ),

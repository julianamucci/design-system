...
import { elapsedOf, useAgentStatusLabels } from "@/components/ui/agent-status.fixtures";
...
const INTERFACE_CODE = `export interface AgentStatusLabels {
...
export function AgentStatusDocs() {
  ...
  // O hook, e não a função pura: é ele que subscreve a loja e faz as nove
  // linhas desta página se redesenharem quando o idioma muda. A função pura
  // serve à `play`, onde não há componente para pendurar um hook.
  ...
  /** Uma linha de estado, sem repetir os rótulos em cada chamada. */
  const line = (status: RunStatus, custom: AgentStatusLabels = labels) => (
    <AgentStatus status={status} elapsed={elapsedOf(status)} labels={custom} />
  );
  ...
  // a execução em curso e a que falhou passa a existir só na cor.
  const mute: AgentStatusLabels = {
    ...labels,
  ...
  // O contraexemplo do segundo par: um rótulo só para os dois estados, e o
  // botão passa a fazer coisas diferentes com o mesmo nome.
  const fixedAction: AgentStatusLabels = {
    ...labels,
    action: {
        ...
        pairs={[
          {
            ...
            doPreview: stackOf(line("running"), line("failed")),
            dontPreview: stackOf(line("running", mute), line("failed", mute)),
          },
          {
            doLabel: tNav("common.do"),
            dontLabel: tNav("common.dont"),
            ...
            doPreview: stackOf(line("running"), line("failed")),
            dontPreview: stackOf(line("running", fixedAction), line("failed", fixedAction)),
          },
        ]}
      />

...
import { MessageTiming, type MessageTimingStat } from "@/components/ui/message-timing";
...
const INTERFACE_CODE = `export interface MessageTimingProps {
...
export function MessageTimingDocs() {
  ...
  const timing = (name: MessageTimingCase) => (
    <MessageTiming stats={statsOf(name)} streaming={isMeasuring(name)} labels={labels} />
  );

  /** Uma linha montada à mão — só os contraexemplos precisam disto. */
  const custom = (stats: MessageTimingStat[]) => (
    <MessageTiming stats={stats} labels={labels} />
  );
  ...
  const examples = [
    ...
    example(
      "demonstration.labels.tight",
      <div className="nds-max-w-3xs">{timing("settled")}</div>,
    ),
  ];
  ...
  return (
    ...
    >
      ...
      <DocsStates
        ...
        // Só um destes é um estado que a peça guarda — a medição em andamento.
        // Os outros três são o que a mesma linha faz conforme quantas medidas
        // chegaram.
        ...
        tables={[
          {
            title: "MessageTiming",
            cols: propsCols,
            items: propRows(["stats", "streaming", "labels"]),
          },
          {
            title: "MessageTimingStat",
            cols: propsCols,
            items: propRows(["statLabel", "statValue"]),
          },
          {
            title: "MessageTimingLabels",
            cols: propsCols,
            items: propRows(["labelsTitle", "labelsMeasuring"]),
          },
        ]}

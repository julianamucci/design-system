 ...
 * nenhuma. O arquivo que existia aqui era um stub `!test` herdado da migração
 * para MDX: exportava um `<></>` e não verificava nada. Busca, estado vazio e
 * rótulo do botão de copiar ficaram sem asserção nas cinco stacks.
 *
...
export const Gallery: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O tile desenha a geometria do catálogo, no tamanho certo', async () => {
      const problemas = [...auditarTile(canvasElement, 'Search'), ...auditarTile(canvasElement, 'Package')];
      await expect(problemas, problemas.join('\n')).toEqual([]);
    });

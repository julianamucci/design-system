...
function CaseExample({ name }: { name: QuotaBannerCase }) {
  return (
    <QuotaBanner
      quota={quotaOf(name)}
...
function AllLevelsExample() {
  ...
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="md">
      {(["normal", "warning", "critical"] as const).map((name) => (
        <QuotaBanner
          key={name}
          quota={quotaOf(name)}
          renewsIn={renewalOf(name)}
...
export const Exhausted: Story = {
  ...
  render: () => <CaseExample name="exhausted" />,
  play: async ({ canvasElement, step }) => {
    const banner = bannerOf(canvasElement)
    const labels = quotaBannerLabels()
...
export const NoRenewal: Story = {
  ...
  render: () => <CaseExample name="noRenewal" />,
  play: async ({ canvasElement, step }) => {
    const banner = bannerOf(canvasElement)
    const labels = quotaBannerLabels()

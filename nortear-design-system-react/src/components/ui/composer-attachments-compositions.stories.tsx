...
function FieldExample({ attachments }: { attachments: Attachment[] }) {
  return (
    <Composer
...
/** O mesmo campo sem fila nenhuma — nem os rótulos dela. */
...
export const WithField: Story = {
  ...
  render: () => (
    <FieldExample
      ...
      attachments={[
        ...queue().slice(0, 2),
        { id: "a5", name: "anotacoes.txt", state: "ready" },
      ]}
    />
  ),
...
export const Removing: Story = {
  ...
  // O progresso vem fora da faixa de propósito: é o caso que a barra precisa
  // aguentar sem estourar.
  render: () => <FieldExample attachments={one("uploading", { progress: 2.5 })} />,
  play: async ({ canvasElement, step }) => {

...
import { ContextDisplayDocs } from "@/components/docs/ContextDisplayDocs"
...
function PlaygroundExample({ input, output, limit, form }: PlaygroundArgs) {
  return (
    <ContextDisplay
      ...
      usage={{ input, output, limit: limit || undefined }}
      form={form}
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step("O número tem NOME, e ele não aparece na tela", async () => {
      ...
      const heading = root.querySelector<HTMLElement>('[data-slot="context-display-title"]')!
      await expect(heading.textContent).toBe(labels.title)
      await expect(heading).toHaveClass("nds-sr-only")
    })

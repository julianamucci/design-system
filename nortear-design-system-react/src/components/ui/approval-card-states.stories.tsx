...
import { ApprovalCard, type ApprovalScopeItem } from "./approval-card"
import {
  approvalCardLabels,
  approvalChoices,
  approvalQuestion,
...
function CardExample({
  ...
  const scope: ApprovalScopeItem[] | undefined = withScope
    ? approvalScope(labels, name)
    : undefined
  ...
  return (
    <ApprovalCard
      ...
      scope={scope}
      actions={withActions ? approvalChoices(labels) : undefined}
      onChoose={onChoose}
...
export const WithScope: Story = {
  ...
  render: () => <CardExample name="publish" />,
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement)
    const expected = approvalScope(approvalCardLabels(), "publish")
...
export const WithoutScope: Story = {
  ...
  render: () => <CardExample name="publish" withScope={false} />,
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement)
    ...
    await step("E a pergunta continua à vista, dentro da região que se anuncia", async () => {
      ...
      await expect(question.textContent).toBe(approvalQuestion(labels, "publish"))
...
export const LongDetail: Story = {
  ...
  render: () => <CardExample name="writeFile" />,
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement)
    const expected = approvalScope(approvalCardLabels(), "writeFile")
    const details = [
...
export const WithoutActions: Story = {
  ...
  render: () => <CardExample name="publish" withActions={false} />,
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement)
    const ask = card.querySelector<HTMLElement>('[data-slot="approval-card-ask"]')!
    ...
    await step("E a pergunta continua à vista, com o alcance inteiro", async () => {
      ...
      await expect(card.querySelectorAll('[data-slot="approval-card-scope-term"]')).toHaveLength(
        approvalScope(approvalCardLabels(), "publish").length,
      )
    })
  },
}

 ...
 * há estado imperativo para traduzir. A citação é dado que entra e desenho que
 * sai. O `id` continua vindo de fora — quem o gera é o campo que a cita, e um
 * id criado aqui dentro seria um endereço que o `aria-describedby` de lá não
 * conhece.
 */
...
function ComposerQuote({ quote, labels, id, onDismiss }: ComposerQuoteProps) {
  ...
  const prefix = `${labels.describes.replace("{author}", "").trim()} `

  return (
    <div
      data-slot="composer-quote"
      className="nds-composer-quote"
      id={id}
      data-quote-id={quote.id}
      data-role={quote.role}
    >
      {/* O prefixo existe SÓ para quem ouve, e é o que transforma dois pedaços
          numa frase: sem ele a descrição do campo começaria com um nome solto,
          e um nome solto não diz que se está respondendo a alguém. Na tela ele
          é redundante — a barra e a posição já dizem o que é —, então não ocupa
          espaço. */}
      <span className="nds-composer-quote-author">
        <span className="nds-sr-only">{prefix}</span>
        {quote.author}
      </span>

      {/* O texto vai INTEIRO. Quem corta é a folha, por linha. */}
      <p className="nds-composer-quote-excerpt">{quote.excerpt}</p>

      {/* O nome de quem escreveu entra no botão: numa tela com citação e
          anexos, dois "×" seguidos são o mesmo botão para quem ouve. */}
      <Button

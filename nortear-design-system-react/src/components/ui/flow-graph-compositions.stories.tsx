...
import {
  flowGraphLongLabelsSource,
  flowGraphPartialSource,
  flowGraphRejoinSource,
  flowGraphTightColumnsSource,
  flowGraphWideSource,
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: flowGraphWideSource },
      description: {
        component:
...
export const Shifted: Story = {
  parameters: {
    ...
    docs: { source: { transform: flowGraphWideSource } },
  },
  render: () => <FlowGraphHost nodes={SHIFTED_NODES} edges={FLOW_EDGES_ORDER} />,
...
export const Partial: Story = {
  parameters: {
    covers: ["visual.item5"],
    docs: { source: { transform: flowGraphPartialSource } },
  },
  render: () => <FlowGraphHost nodes={FLOW_NODES_PARTIAL} edges={FLOW_EDGES_ORDER} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
...
export const LongLabels: Story = {
  parameters: {
    covers: ["visual.item7"],
    docs: { source: { transform: flowGraphLongLabelsSource } },
  },
  render: () => <FlowGraphHost nodes={LONG_LABEL_NODES} edges={LONG_LABEL_EDGES} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    ...
    await step("O rótulo inteiro está no DOM, sem reticências", async () => {
      ...
      await expect(firstLabel.getBoundingClientRect().height).toBeGreaterThan(
        fontSize * 1.9,
      )
    })
  },
...
export const Rejoin: Story = {
  parameters: {
    covers: ["visual.item8"],
    docs: { source: { transform: flowGraphRejoinSource } },
  },
  render: () => <FlowGraphHost nodes={REJOIN_NODES} edges={REJOIN_EDGES} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
...
function TightColumnsExample() {
  ...
  // `.nds-flow-graph`, e propriedade personalizada declarada dentro do próprio
  // seletor vence a herança — o invólucro nunca a alcançaria. Como a peça não
  // publica `style` na assinatura, quem a escreve é uma ref de retorno de
  // chamada, que corre depois de os filhos já estarem no documento.
  const tighten = useCallback((element: HTMLDivElement | null) => {
    element
      ?.querySelector<HTMLElement>('[data-slot="flow-graph"]')
      ?.style.setProperty("--flow-graph-column-min", "var(--spacing-16)")
  }, [])
  ...
  return (
    ...
    <div className="nds-stack nds-max-w-md" data-spacing="lg">
      <FlowGraphHost
        nodes={wideFlowNodes()}
        edges={wideFlowEdges()}
        testid="flow-graph-default-columns"
      />
      <div ref={tighten}>
        <FlowGraphHost
          nodes={wideFlowNodes()}
          edges={wideFlowEdges()}
          testid="flow-graph-tight-columns"
        />
      </div>
    </div>
  )
...
export const TightColumns: Story = {
  parameters: {
    docs: { source: { transform: flowGraphTightColumnsSource } },
  },
    ...
    const widthOf = (testid: string) =>
      canvasElement
        .querySelector<HTMLElement>(
          `[data-testid="${testid}"] [data-slot="flow-graph-node"]`,
        )!

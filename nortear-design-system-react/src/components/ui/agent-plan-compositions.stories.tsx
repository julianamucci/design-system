 ...
 */
function StackedExample({ status, steps }: { status: RunStatus; steps: PlanStep[] }) {
  return (
...
export const ProposedPlan: Story = {
  ...
  render: () => <StackedExample status="idle" steps={proposedPlan()} />,
 ...
 * e o estado da linha acima. É a decisão de não criar um segundo componente,
 * escrita em código.
 */
export const TaskList: Story = {
  ...
  render: () => <StackedExample status="running" steps={runningPlan()} />,
  play: async ({ canvasElement, step }) => {
    const plan = planOf(canvasElement)
    const steps = stepsOf(canvasElement)

...
import { flowGraphLabels, FlowGraphHost } from "./flow-graph.fixtures"
...
import { FlowGraphDocs } from "@/components/docs/FlowGraphDocs"
...
export const Playground: Story = {
  ...
  // O invólucro mora nas peças de andaime deste componente: sem nó a peça
  ...
  play: async ({ canvasElement, step, args }) => {
    const host = canvasElement.querySelector<HTMLElement>(
      '[data-testid="flow-graph-host"]',
    )!
    const labels = flowGraphLabels()

    if (args.revealed === 0) {
      // Sem nó não há grafo, e a peça devolve `null` — nem moldura, nem camada
      // que rola. Uma parada de teclado que leva a uma caixa vazia é ruído com
      // nome, e por isso a peça prefere não existir.
    ...
    await step("Há um nó por nó, na ordem em que foram declarados", async () => {
      ...
      const nodes = [
        ...root.querySelectorAll<HTMLElement>('[data-slot="flow-graph-node"]'),
      ]

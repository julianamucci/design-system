...
import {
  traceWaterfallClippedSource,
  traceWaterfallLongLabelsSource,
  traceWaterfallPartialSource,
  traceWaterfallTightColumnsSource,
  traceWaterfallWideSource,
...
// conversa, quando o rótulo é longo demais para a coluna, quando o eixo é
// uma janela mais curta que o rastro e quando ele chega pela metade.
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: traceWaterfallWideSource },
      description: {
        component:
...
export const Shifted: Story = {
  parameters: {
    ...
    docs: { source: { transform: traceWaterfallWideSource } },
  },
  render: () => <TraceWaterfallHost spans={SHIFTED_SPANS} totalMs={TRACE_TOTAL_MS} />,
...
export const Partial: Story = {
  parameters: {
    covers: ["visual.item5"],
    docs: { source: { transform: traceWaterfallPartialSource } },
  },
  render: () => <TraceWaterfallHost spans={TRACE_SPANS_PARTIAL} totalMs={TRACE_TOTAL_MS} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    ...
    await step("As barras que sobraram guardam a posição do rastro inteiro", async () => {
      // É a razão de o eixo não ser derivado: derivado, ele encolheria aqui
      // e as três barras reescalariam para ocupar a régua inteira.
...
export const LongLabels: Story = {
  parameters: {
    covers: ["visual.item7"],
    docs: { source: { transform: traceWaterfallLongLabelsSource } },
  },
  render: () => (
    <TraceWaterfallHost
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Ele não quebra: a linha continua de uma linha só", async () => {
      ...
      // vizinha, que é o contrário do que uma cascata precisa. Medido
      // contra `font-size`, que computa em pixel em qualquer motor —
      // `line-height` declarada sem unidade nem sempre computa.
...
export const Clipped: Story = {
  parameters: {
    ...
    docs: { source: { transform: traceWaterfallClippedSource } },
  },
  render: () => <TraceWaterfallHost spans={CLIPPED_SPANS} totalMs={CLIPPED_TOTAL_MS} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    ...
    await step("A linha recortada avisa que o trecho continua fora", async () => {
      // O id vem da FIXTURE, e não escrito aqui: literal dentro de string
 ...
 */
function TightColumnsExample() {
  // Declarada NO PRÓPRIO elemento, e não no invólucro: a folha a declara em
  // `.nds-trace-waterfall`, e propriedade personalizada declarada dentro do
  // próprio seletor vence a herança — o invólucro nunca a alcançaria. Como a
  // peça não publica `style` na assinatura, quem a escreve é uma ref de
  // retorno de chamada, que corre depois de os filhos já estarem no
  // documento.
  const tighten = useCallback((element: HTMLDivElement | null) => {
    element
      ?.querySelector<HTMLElement>('[data-slot="trace-waterfall"]')
      ?.style.setProperty("--trace-waterfall-name-min", "var(--spacing-8)")
  }, [])
  ...
  return (
    ...
    <div className="nds-stack nds-max-w-md" data-spacing="lg">
      <TraceWaterfallHost
        spans={SHORT_LABEL_SPANS}
        totalMs={SHORT_LABEL_TOTAL_MS}
        testid="trace-waterfall-default-columns"
      />
      <div ref={tighten}>
        <TraceWaterfallHost
          spans={SHORT_LABEL_SPANS}
          totalMs={SHORT_LABEL_TOTAL_MS}
          testid="trace-waterfall-tight-columns"
        />
      </div>
    </div>
  )
...
export const TightColumns: Story = {
  parameters: {
    docs: { source: { transform: traceWaterfallTightColumnsSource } },
  },
  render: () => <TightColumnsExample />,
  play: async ({ canvasElement, step }) => {

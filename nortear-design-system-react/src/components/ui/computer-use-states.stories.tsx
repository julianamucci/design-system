...
import { computerUseLabels, DemoScreen, useComputerUseLabels } from "./computer-use.fixtures"
import {
  ...
  return (
    <div className="nds-stack nds-max-w-md" data-spacing="lg">
      {RUN_STATUSES.map((status) => (
        <Piece key={status} status={status} activeIndex={3} />
      ))}
    </div>
  )
}
...
export const Running: Story = {
  ...
  render: () => <Piece status="running" activeIndex={3} />,
  play: async ({ canvasElement, step }) => {
...
export const Finished: Story = {
  ...
  render: () => <Piece status="complete" activeIndex={COMPUTER_STEPS_LOGIN.length - 1} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
...
export const FirstStep: Story = {
  ...
  render: () => <Piece status="running" activeIndex={0} steps={COMPUTER_STEPS_SHORT} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
...
export const WithoutSteps: Story = {
  ...
  render: () => <Piece status="idle" activeIndex={0} steps={[]} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
...
export const ClampedIndex: Story = {
  ...
  render: () => <Piece status="complete" activeIndex={99} />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)

...
//
// Módulo de TS puro, sem `.tsx` e sem React: é o que deixa `media-embed.test.ts`
// rodar no projeto `unit` do vitest, em node, sem subir navegador.
...
export function parseEmbedMessage(provider: EmbedProvider, data: unknown): EmbedEvent | null {
  ...
  if (provider === 'youtube') {
    if (payload.event === 'onStateChange') {
      const changed = YOUTUBE_STATE[Number(payload.info)];
      return changed ? ({ type: changed } as EmbedEvent) : null;
    }
  ...
  const named = payload.event;
  const details = payload.data as { seconds?: number; duration?: number } | undefined;
  if (named === 'play') return { type: 'playing' };
  if (named === 'pause') return { type: 'paused' };
  if (named === 'ended') return { type: 'ended' };
  if (named === 'timeupdate' && details && typeof details.seconds === 'number') {

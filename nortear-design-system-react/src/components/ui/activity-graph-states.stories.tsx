...
import {
  activityGraphBusySource,
  activityGraphEmptySource,
  activityGraphNoWindowSource,
  activityGraphScaleSource,
...
// uma coisa só, se ela se declara ocupada. Por isso esta janela de uma
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: activityGraphScaleSource },
      description: {
        component:
...
export const Scale: Story = {
  parameters: {
    ...
    docs: { source: { transform: activityGraphScaleSource } },
  },
  render: () => (
    <ActivityGraphHost
      days={SCALE_DAYS}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("O dia do teto diz a palavra do teto, e o vazio tem frase própria", async () => {
      ...
      // E a contraprova: a frase do vazio NÃO traz a palavra de nível
      // nenhum.
...
export const EmptyWindow: Story = {
  parameters: {
    ...
    docs: { source: { transform: activityGraphEmptySource } },
  },
  render: () => (
    <ActivityGraphHost
...
export const Busy: Story = {
  parameters: {
    ...
    docs: { source: { transform: activityGraphBusySource } },
  },
  render: () => (
    <ActivityGraphHost
      days={ACTIVITY_DAYS}
 ...
 * parada que leva a uma caixa vazia é ruído com nome.
 */
export const NoWindow: Story = {
  parameters: {
    covers: ["functional.item8"],
    docs: { source: { transform: activityGraphNoWindowSource } },
  },
  render: () => (
    <div className="nds-stack nds-w-full" data-spacing="lg">
      <ActivityGraphHost
        ...
        thresholds={ACTIVITY_THRESHOLDS}
        status="complete"
        testid="activity-graph-no-window-host"
      />
      <ActivityGraphHost
        days={ACTIVITY_DAYS}
        start={ACTIVITY_START}
        end={ACTIVITY_END}
        // Sem degrau, todo dia pintaria igual e a peça deixaria de dizer
        // algo.
        thresholds={[]}
        status="complete"
        testid="activity-graph-no-scale-host"
      />
    </div>
  ),
  play: async ({ canvasElement, step }) => {

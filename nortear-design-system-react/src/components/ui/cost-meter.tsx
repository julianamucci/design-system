 ...
 */
function CostMeterBar({ percent }: { percent: number }) {
  ...
  const style = { "--nds-cost-spent": String(percent) } as CSSProperties

  return (
    <span
      className="nds-cost-meter-bar"
      data-slot="cost-meter-meter"
      aria-hidden="true"
      style={style}
    >
      <span className="nds-cost-meter-bar-fill" />
    </span>
  )
}

function CostMeter({ amount, budget, labels, className }: CostMeterProps) {
  ...
  const percent = budget ? fractionPercent(budget.fraction) : null
  const level = budget ? fractionLevel(budget.fraction) : null
  ...
  return (
    ...
    <p
      ...
      className={cn("nds-cost-meter", className)}
    ...
    >
      ...
      <span className="nds-sr-only" data-slot="cost-meter-title">
        {labels.title}
      </span>
      ...
      {/* A QUANTIA É SEMPRE A QUANTIA (decisão 6). Ao contrário da peça da
          ...
          QUALIFICA. Ela sai exatamente como chegou — a peça não formata. */}
      <span className="nds-cost-meter-amount" data-slot="cost-meter-amount">
        {amount}
      </span>
      ...
      {/* O DETALHE MANTÉM A FRAÇÃO EM TEXTO, e não é adorno: é ele que permite à
          ...
          de progresso. Sem teto, a ausência vira informação em vez de parecer
          medição pela metade. */}
      <span className="nds-cost-meter-detail" data-slot="cost-meter-detail">
        {budget && percent !== null
          ? `${percent}% ${labels.of} ${budget.amount}`
          : labels.unbounded}
      </span>
      ...
      {/* O NÍVEL É PALAVRA (decisão 3), e a cor acompanha em vez de substituir.
          ...
          O `data-slot` da etiqueta é sobrescrito para o desta peça, como a
          medição da janela e o plano do agente já fazem. */}
      {level ? (
        <Badge
      ...
      {/* O medidor vem POR ÚLTIMO no DOM, e a folha o joga para a segunda linha
          ...
          não muda. */}
      {percent === null ? null : <CostMeterBar percent={percent} />}
    </p>
  )
}

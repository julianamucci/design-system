...
import {
  activityGraphMonthSource,
  activityGraphTightCellsSource,
  activityGraphWeekStartSource,
  activityGraphYearSource,
...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// O que muda quando a janela muda: um mês, um ano, a semana começando em
// outro dia, um dia declarado fora do período — e o que muda quando quem
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: activityGraphYearSource },
      description: {
        component:
 ...
 *
 * É o que a fonte absorvida não sabia fazer: lá a janela é fixa nos 365
 * dias que terminam hoje, e não há como pedir março.
 */
export const SingleMonth: Story = {
  parameters: {
    covers: ["visual.item5"],
    docs: { source: { transform: activityGraphMonthSource } },
  },
  render: () => (
    <ActivityGraphHost
      days={ACTIVITY_DAYS}
 ...
 *
 * Cinquenta e três colunas passam de qualquer conversa com o tamanho de
 * casa que a folha declara, e é aí que a camada rola — uma só, com nome e
 * parada de teclado.
 */
export const WholeYear: Story = {
  parameters: {
    covers: ["functional.item10", "visual.item6"],
    docs: { source: { transform: activityGraphYearSource } },
  },
  // A LARGURA É PARTE DO ASSUNTO: a story precisa ser mais estreita que a
  // grade para que a barra exista, e o canvas do Storybook é largo. Sem o
  // teto, esta fotografia mostraria uma grade folgada e o guarda da rolagem
  // ficaria verde sem nada para medir.
  render: () => (
    <ActivityGraphHost
      days={ACTIVITY_DAYS}
      start={WIDE_START}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Uma só camada rola, e é a que tem nome e foco", async () => {
      ...
      // barra existir, e é o elemento que RECORTA que precisa ser medido —
      // a raiz do documento não transborda.
 ...
 *
 * As sete linhas giram junto, e os rótulos de dia acompanham — é a mesma
 * janela lida por quem não começa a semana no domingo.
 */
export const WeekStartsMonday: Story = {
  parameters: {
    covers: ["visual.item7"],
    docs: { source: { transform: activityGraphWeekStartSource } },
  },
  render: () => (
    <ActivityGraphHost
      days={ACTIVITY_DAYS}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("As linhas giram: o mesmo dia cai em outra linha", async () => {
      // A conta de referência sai do primitivo, com o mesmo começo de
      // semana — e a contraprova é a grade que começa no domingo, que a põe
      // noutra linha.
...
export const OutsideDay: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    // A referência é a MESMA janela SEM o dia de fora: se ele entrasse, o
    // total e a contagem de casas mudariam, e é isso que a asserção mede.
    const reference = resolveActivityCalendar(ACTIVITY_DAYS, {
      start: ACTIVITY_START,
      end: ACTIVITY_END,
      thresholds: ACTIVITY_THRESHOLDS,
    ...
    await step("O dia de fora não é desenhado", async () => {
      // A data vem da FIXTURE como constante nomeada: literal dentro de
      // string não é alcançado por portão nenhum.
      await expect(
...
export const TightCells: Story = {
  parameters: {
    covers: ["visual.item8"],
    docs: { source: { transform: activityGraphTightCellsSource } },
  },
  // OS DOIS LADO A LADO, e é o único jeito de a asserção medir alguma
  // coisa: o valor da propriedade lido por `getPropertyValue` é o
  // ESPECIFICADO, e num token declarado por `calc()` ele volta como a
  // expressão, não como o pixel. O que compara de verdade é a largura da
  // caixa, e para comparar é preciso haver duas.
  render: () => (
    <div className="nds-stack nds-w-full" data-spacing="lg">
      ...
      />
      <div
        // Declarada NO PRÓPRIO elemento, e não no invólucro: a folha a
        // declara em `.nds-activity-graph`, e propriedade personalizada
        // declarada dentro do próprio seletor vence a herança — o
        // invólucro nunca a alcançaria. Como a peça não publica `style` na
        // assinatura, quem a escreve é uma ref de retorno de chamada, que
        // corre depois de os filhos já estarem no documento.
        ref={(element) => {
          element
            ?.querySelector<HTMLElement>('[data-slot="activity-graph"]')
            ?.style.setProperty("--activity-graph-cell", "var(--spacing-2)")
        }}
      >
        <ActivityGraphHost
          days={ACTIVITY_DAYS}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("A casa aperta, e a grade encolhe com ela", async () => {
      // A largura da casa é COMPUTADA, e é o que prova que a propriedade

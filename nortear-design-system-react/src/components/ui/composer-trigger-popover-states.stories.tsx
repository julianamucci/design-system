...
import {
  TriggerComposerExample,
  triggerLabels,
} from "./composer-trigger-popover.fixtures"
import {
  composerTriggerEmptySource,
  composerTriggerFilteredSource,
...
export const Filtered: Story = {
  parameters: {
    ...
    docs: { source: { transform: composerTriggerFilteredSource } },
  },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Com termo, a lista filtra", async () => {
      ...
      await waitFor(() => expect(within(panel()).getAllByRole("option")).toHaveLength(3))
    })

    await step("A seta desce, e o campo aponta a nova opção", async () => {
      await expect(active()).toHaveTextContent("Ana Souza")
...
export const Empty: Story = {
  parameters: {
    ...
    docs: { source: { transform: composerTriggerEmptySource } },
  },
  render: () => <TriggerComposerExample onSubmit={onSubmit} />,
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement)
...
export const Closed: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Voltar o CURSOR para dentro do gatilho reabre a lista", async () => {
      // O passo que faltava, e a falta apareceu ao plantar o defeito: trocar a
      // posição do cursor pelo fim do texto não reprovava nada, porque toda
      // play digitava e deixava o cursor no fim. É aqui que o gatilho prova
      // depender de ONDE o cursor está, e não do que o texto contém.
      await userEvent.clear(input)
      input.focus()
      await userEvent.type(input, "avisa a @an fim")
      await waitFor(() => expect(panel().hidden).toBe(true))

      // Quatro passos à esquerda param logo depois do segundo caractere do
      // termo.

 ...
 * molduras no axe (`label` e `label-title-only`). Nenhum dos três candidatos a
 * nome que estavam ali serve — o prefixo `https://` completa o formato, o
 * texto de erro é `aria-describedby` (descrição, não nome) e o nome do GRUPO
 * pertence ao conjunto, não ao campo.
 */
...
export function inputGroupRoot(canvasElement: HTMLElement): HTMLElement {
  return canvasElement.querySelector<HTMLElement>('[data-slot="input-group"]')!
}
...
export function inputGroupRoots(canvasElement: HTMLElement): HTMLElement[] {
  return Array.from(
    canvasElement.querySelectorAll<HTMLElement>('[data-slot="input-group"]'),
  )
}
...
export function inputGroupControl<T extends HTMLElement = HTMLInputElement>(
  ...
  return canvasElement.querySelector<T>(CONTROL_SELECTOR)!
}

/** O addon de uma posição, ou `null` — a busca por papel não o alcança. */
export function addonOfAlign(
  root: HTMLElement,

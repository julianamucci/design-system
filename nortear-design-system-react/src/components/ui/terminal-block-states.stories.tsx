...
import { exitCodeFor, linesFor, terminalBlockLabels, useTerminalBlockLabels } from "./terminal-block.fixtures"
import {
...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// Os cinco momentos de um comando, e o caso em que ele não escreveu nada. Não
// há eixo de forma nesta peça: a estrutura é sempre a mesma — o que rodou, o
 ...
 * interrompido para no meio porque alguém o cortou, o que terminou traz a
 * tabela alinhada e o que falhou traz a linha larga. Uma saída só para os cinco
 * faria as fotos mostrarem o mesmo texto com palavras diferentes embaixo, que é
 * exatamente o que esta peça não faz.
 *
 * Os rótulos vêm de hook, então o render passa por um componente.
 */
function StatusExample({ status }: { status: RunStatus }) {
  return (
    <TerminalBlock
...
/**
 * O comando que terminou sem escrever nada, e o que ainda não rodou.
 *
 * Os dois casos dizem a mesma coisa sobre a caixa: não há o que mostrar.
 */
function WithoutOutputExample() {
  const labels = useTerminalBlockLabels()
  return (
    <div className="nds-stack" data-spacing="lg">
      <TerminalBlock command={TERMINAL_COMMAND} status="complete" exitCode={0} labels={labels} />
      <TerminalBlock
        command={TERMINAL_COMMAND}
        lines={linesFor("idle")}
        status="idle"
        exitCode={exitCodeFor("idle")}
        labels={labels}
      />
    </div>
  )
...
export const Running: Story = {
  ...
  render: () => <StatusExample status="running" />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)

    await step("Enquanto a saída chega, a peça se declara ocupada", async () => {
      // `aria-busy` diz que aquele pedaço da tela ainda se escreve, sem
      // anunciar nada — é o contrário da região viva (decisão 1 da folha).
...
export const Complete: Story = {
  ...
  render: () => <StatusExample status="complete" />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    const labels = terminalBlockLabels()
...
export const Failed: Story = {
  ...
  render: () => <StatusExample status="failed" />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    const output = outputOf(piece)!
...
export const Stopped: Story = {
  ...
  render: () => <StatusExample status="stopped" />,
  play: async ({ canvasElement, step }) => {
    const piece = pieceOf(canvasElement)
    const labels = terminalBlockLabels()
 ...
 * caixa é o assunto: caixa vazia com parada de tabulação dentro seria dar foco
 * a lugar nenhum.
 */

...
// story: `export function esperarPainel()` dentro de um `*.stories.tsx` viraria
 ...
 *
 *   · o popup entra no DOM antes de o floating-ui devolver a medida — `data-side`
 *     é o sinal público de que a medição terminou;
 *   · o popup entra com transição de opacidade (`data-starting-style`), e ler cor
 *     ou contraste no primeiro frame mede um elemento translúcido — é assim que
 *     nasce a violação de contraste ~1.0 do axe;

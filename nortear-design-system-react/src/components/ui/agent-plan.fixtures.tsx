 ...
 *
 * DOIS acessos ao mesmo dicionário, como no andaime da linha de estado, e a
...
const CONTENT = planTranslations as unknown as Record<string, PlanContent>
 ...
 * andaime.
 */
function read(locale: Locale): AgentPlanLabels {
  const raw = (CONTENT[locale] ?? CONTENT["pt-BR"]).labels

  const state = {} as Record<PlanStepState, string>
  for (const item of PLAN_STEP_STATES) state[item] = raw.state[item] ?? ""

  return { plan: raw.plan, state }
}

/** Os rótulos da lista, no idioma corrente. Para dentro de um componente. */
export function useAgentPlanLabels(): AgentPlanLabels {

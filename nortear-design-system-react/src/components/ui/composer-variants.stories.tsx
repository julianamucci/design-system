...
import { Composer, type ComposerSubmitOn } from "./composer"
import { useComposerLabels } from "./composer.fixtures"
import { composerEnterSource, composerModifierSource, composerSource } from "./composer.source"
...
function SubmitModeExample({ submitOn }: { submitOn: ComposerSubmitOn }) {
  return (
    <Composer
...
export const SubmitOnEnter: Story = {
  ...
  render: () => <SubmitModeExample submitOn="enter" />,
...
export const SubmitOnModifier: Story = {
  ...
  render: () => <SubmitModeExample submitOn="modifier" />,
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement)
    const input = canvas.getByRole("textbox")

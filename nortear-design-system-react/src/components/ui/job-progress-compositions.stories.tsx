...
function JobQueueExample() {
  ...
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="lg">
      <JobProgress
        label={label}
        status="running"
        count={JOB_COUNT}
      ...
      <JobProgress
        ...
        onAction={onAction}
      />
      <JobProgress label={label} status="idle" labels={labels} onAction={onAction} />
    </div>
  )
}
...
/** A peça sozinha, para o pedido ter um alvo sem ambiguidade. */
function RequestingExample() {
  return (
    <JobProgress
      label={useJobLabel()}
      status="running"

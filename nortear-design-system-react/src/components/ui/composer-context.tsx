 ...
 * as stacks, porque aqui não há estado imperativo para traduzir. A lista é dado
 * que entra e desenho que sai. O que muda é só o portador do ícone — aqui um
 * componente, e não a lista `[tag, atributos]` do pacote agnóstico.
 */
 ...
 */
const KIND_ICONS: Record<ContextKind, LucideIcon> = {
...
function ComposerContext({ items, labels, onRemove }: ComposerContextProps) {
  return (
    ...
    <ul
      ...
      className="nds-composer-context"
      ...
      {items.map((item, index) => {
        ...
        return (
          <li
            ...
            className="nds-composer-context-item"
          ...
          >
            ...
            <Icon className="nds-composer-context-icon" aria-hidden="true" />

            {/* A espécie em TEXTO, dentro do item (decisão 2 da folha). Ela
                não vai num `aria-label` do `<li>`: rótulo em item de lista
                SUBSTITUI o conteúdo no anúncio, e o recorte — que é o que
                separa um pedaço do todo — sairia junto. Escondida do olho,
                presente para o ouvido, e na frente do nome porque é assim que a
                frase se lê: "Trecho relatorio.ts, linhas 12–48". */}
            <span className="nds-sr-only">{labels.kind[item.kind]}</span>

            <span className="nds-composer-context-label">{item.label}</span>

            {/* O recorte, quando o item é um PEDAÇO. Sem ele, um trecho seria o
                nome de um arquivo repetido. */}
            {item.detail ? (
              <span className="nds-composer-context-detail">{item.detail}</span>
            ) : null}

            {/* A decisão 3 da folha, e a máquina dela mora no protocolo:
                contexto automático não ganha botão de remover — ele voltaria na
                próxima pergunta, e botão que desfaz o que se refaz sozinho é
                armadilha. A marca ocupa o lugar do botão, e é TEXTO: a moldura
                tracejada sozinha não descreve estado (WCAG 1.4.1). */}
            {isContextRemovable(item) ? (
              // O nome acessível leva o NOME DO ITEM: uma lista de botões
              ...
              <span
                className="nds-composer-context-detail"

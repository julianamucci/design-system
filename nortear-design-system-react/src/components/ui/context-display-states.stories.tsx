...
function CaseExample({ name }: { name: ContextDisplayCase }) {
  return <ContextDisplay usage={usageOf(name)} labels={useContextDisplayLabels()} />
}
 ...
 */
function AllLevelsExample() {
  const labels = useContextDisplayLabels()
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="md">
      {BUDGET_LEVELS.map((entry) => (
        <ContextDisplay key={entry} usage={usageOf(entry)} labels={labels} />
      ))}
    </div>
  )
}
...
export const AllLevels: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("Cada um traz a PALAVRA daquele nível", async () => {
      ...
      for (const [i, entry] of BUDGET_LEVELS.entries()) {
        const badge = blocks[i]!.querySelector<HTMLElement>(
          '[data-slot="context-display-level"]',
        )!
        await expect(badge.textContent).toBe(labels.level[entry])
      }
    })
...
export const AtThreshold: Story = {
  ...
  render: () => <CaseExample name="threshold" />,
  play: async ({ canvasElement, step }) => {
...
export const Unbounded: Story = {
  ...
  render: () => <CaseExample name="unbounded" />,
  play: async ({ canvasElement, step }) => {
    const block = blockOf(canvasElement)
    const labels = contextDisplayLabels()

 ...
 *
 * DIVERGÊNCIA DE API, e só de forma: aqui o aviso sai por `onAction`, uma
...
function DraftRestore({
  ...
  return (
    <div
      ...
      // `role="status"`, e NÃO `role="alert"` (decisão 1 da folha). A faixa
      // nasce junto com a tela e quem lê está começando a ler: `alert`
      // interromperia a leitura em curso por algo que não é urgente. `status`
      // anuncia na primeira pausa — e é por isso também que nada aqui chama
      // `focus()`: a faixa não rouba o foco (decisão 4), ela só está antes do
      // campo na ordem de leitura.
      role="status"
      className={cn("nds-composer-draft", className)}
    >
      <p className="nds-composer-draft-title" data-slot="composer-draft-title">
        {labels.title}
        ...
        */}
        {timestamp ? (
          <span data-slot="composer-draft-timestamp">{` · ${timestamp}`}</span>
        ) : null}
      </p>
      ...
      */}
      <p className="nds-composer-draft-preview" data-slot="composer-draft-preview">
        {draft}
      </p>
      ...
      {/*
        ...
        Cada um leva o NOME do rascunho, e não só o verbo (decisão 3). O rótulo
        é visível e é também o nome acessível: não há `aria-label` separado,
        porque o texto que se vê já diz sobre o que o controle age — e nome
        acessível que diverge do texto visível quebra WCAG 2.5.3 pelo caminho.
        ...
        nenhum.
      */}
      <div className="nds-composer-draft-actions" data-slot="composer-draft-actions">
        <Button

 ...
 * importa.
 */
function BeforeAnswerExample() {
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="sm">
      <ToolGroup calls={TOOL_CALLS_WITH_FAILURE} labels={useToolGroupLabels()} />
      <p>Não publiquei o relatório: o destino recusou por falta de permissão de escrita.</p>
    </div>
  )
}

/** A caixa sozinha, para o abrir e fechar ter um alvo sem ambiguidade. */
 ...
 */
function WaitingOutsideExample() {
  ...
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="sm">
      {/* À vista e aberta: pedir autorização dentro de uma caixa fechada é
          pedir sem mostrar. */}
      <ToolGroup calls={waiting} labels={labels} open />
      <ToolGroup calls={grouped} labels={labels} />
    </div>
  )
}

export const BeforeAnswer: Story = {
  render: () => <BeforeAnswerExample />,

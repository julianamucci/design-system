...
function CommandSequenceExample() {
  ...
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="lg">
      {(["complete", "failed", "idle"] as const).map((status) => (
        <TerminalBlock
          key={status}
 ...
 */
function LongOutputExample() {
  return (
    <div
      className="nds-stack nds-max-w-lg"
      data-spacing="lg"
      style={{ "--terminal-block-max-block-size": "6rem" } as CSSProperties}
    >
      <TerminalBlock
        command={TERMINAL_COMMAND}

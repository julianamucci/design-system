...
describe('embedHandshake', () => {
  it('YouTube abre a conversa com uma escuta', () => {
    ...
    expect(JSON.parse(message)).toMatchObject({ event: 'listening' });
  });
...
describe('embedCommand', () => {
  ...
  it('o segundo argumento do seekTo do YouTube pede a busca imediata', () => {
    const command = JSON.parse(embedCommand('youtube', { kind: 'seek', value: 12.5 }));
    expect(command.func).toBe('seekTo');
    expect(command.args).toEqual([12.5, true]);
  });
});

describe('parseEmbedMessage', () => {
  it('lê o estado numérico do YouTube', () => {
    const read = (info: number) => parseEmbedMessage('youtube', { event: 'onStateChange', info });
    expect(read(1)).toEqual({ type: 'playing' });
    expect(read(2)).toEqual({ type: 'paused' });
    expect(read(0)).toEqual({ type: 'ended' });
  ...
  it('aceita a carga como TEXTO e como objeto', () => {
    ...
    // "às vezes funciona".
    const asText = parseEmbedMessage(
      'youtube',
      JSON.stringify({ event: 'onStateChange', info: 1 }),
    );
    expect(asText).toEqual({ type: 'playing' });
  ...
  it('ignora ruído sem estourar', () => {
    ...
    for (const junk of ['nao é json', '', null, undefined, 42, { event: 'outro' }, {}]) {
      expect(parseEmbedMessage('youtube', junk)).toBeNull();
      expect(parseEmbedMessage('vimeo', junk)).toBeNull();
    }
  });
});

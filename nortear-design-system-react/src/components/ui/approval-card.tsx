import { Children, Fragment, type MouseEvent, type ReactNode } from "react"
 ...
 *
 * A API DIVERGE do vanilla em UM ponto, e é divergência de framework: lá os
 * controles chegam como lista de elementos do documento, aqui chegam como nó —
 * qualquer coisa que o renderizador saiba desenhar. O nome da prop é o mesmo, e
 * a tabela da página de documentação sobrescreve só o tipo. Mesma divergência
 * que `chat-thread` já registrou para as ações do turno.
 */
...
function ApprovalCard({
  ...
  // vão com afastamento e nada dentro que a folha existe para não ter.
  ...
  // sem ela um atributo posto num ancestral por engano viraria resposta.
  function handleChoice(event: MouseEvent<HTMLDivElement>) {
    if (!onChoose) return
    ...
    // Atributo vazio não é escolha: relatar `''` faria quem consome procurar
    // uma política com nome nenhum.
  ...
  return (
    ...
    <div data-slot="approval-card" className={cn("nds-approval-card", className)}>
      {/* A CAIXA QUE SE ANUNCIA (decisão 1), e ela existe como elemento
          ...
          prazo. */}
      <div
        className="nds-approval-card-ask"
        data-slot="approval-card-ask"
        role="status"
      >
        <p
          className="nds-approval-card-question"
        ...
        {/* O ALCANCE É UMA LISTA DE DEFINIÇÃO (decisão 2). Com `<dl>`, o termo e
            o valor continuam pareados quando alguém pula de item em item sem
            ler a linha inteira — o que uma frase com dois-pontos não garante. */}
        {scope?.length ? (
          <dl className="nds-approval-card-scope" data-slot="approval-card-scope">
            {scope.map((item, index) => (
              // O par é filho DIRETO da lista: um `<div>` em volta quebraria a
              // grade de duas colunas, que é da lista e não de cada par.
              <Fragment key={`${index}-${item.term}`}>
                <dt
                  className="nds-approval-card-scope-term"
                  data-slot="approval-card-scope-term"
                >
                  {item.term}
                </dt>
                <dd
                  className="nds-approval-card-scope-detail"
      ...
      {hasActions ? (
        <div
          className="nds-approval-card-actions"

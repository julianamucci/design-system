...
import { useToolGroupLabels } from "./tool-group.fixtures"
import {
 ...
 */
function OutsideTheGroupExample() {
  ...
  return (
    <div className="nds-stack nds-max-w-lg" data-spacing="sm">
      {waiting.map((call) => (
        <ApprovalCard
          key={call.name}
          question={approvalWaitingQuestion(labels)}
 ...
 */
function AnsweringExample() {
  const labels = useApprovalCardLabels()
  return (
    <ApprovalCard
      question={approvalQuestion(labels, "publish")}
      scope={approvalScope(labels, "publish")}
      actions={[
        ...approvalChoices(labels, [APPROVAL_CHOICE_ALLOW_ONCE]),
        approvalAside(labels),
      ]}
      onChoose={onChoose}
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("A que espera por uma pessoa virou um cartão, e ele está À VISTA", async () => {
      ...
      await expect(card.querySelector('[data-slot="approval-card-question"]')?.textContent).toBe(
        approvalWaitingQuestion(labels),
      )
    })

    await step("O alcance dele carrega o que a execução dizia", async () => {
      // Quem faz essa tradução é quem consome: a peça receberia o vocabulário
      // de outra e passaria a conhecer as duas.
      const details = [

 ...
 * execução.
 *
 * A API NÃO DIVERGE do vanilla nos NOMES: `label`, `status`, `count`, `labels` e
 * `onAction` são os mesmos, porque aqui não há estado imperativo para traduzir.
 * O que diverge é onde a classe da barra cai — ver o comentário da barra.
 */
 ...
 */
function countText(count: JobCount, labels: JobProgressLabels): string {
...
function JobProgress({
  ...
  const intent: JobProgressIntent = isRunFinished(status) ? "start" : "stop"
  ...
  return (
    ...
    <div
      ...
      className={cn("nds-job-progress", className)}
      data-status={status}
    ...
    >
      ...
      {/* A CONTA É DECORATIVA (decisão 4 do bloco, decisão 3 da folha). Ela se
          reescreve a cada unidade processada, e o que a barra já carrega em
          `aria-valuenow` chegaria duas vezes a quem ouve — sendo que a cópia
          que sobra é justamente a que rerroda. */}
      {count ? (
        <span
          ...
          aria-hidden="true"
        >
          {countText(count, labels)}
        </span>
      ) : null}

      {/* A BARRA É A DO DESIGN SYSTEM, e o que ela mostra sai do vocabulário
          (decisão 5). `null` é "andando sem estimativa": a barra troca a fração
          por um traço que percorre o trilho e deixa de escrever
          `aria-valuenow`, porque zero mentiria sobre o que ninguém sabe.

          Sem nome, `role="progressbar"` é anunciado como "barra de progresso,
          24%": o leitor diz quanto, nunca de quê. O nome é o rótulo do
          trabalho, que é o mesmo texto que se vê ao lado.

          A COR SEMÂNTICA SÓ EXISTE ONDE FOI MEDIDA — concluído e falhou. Quem
          a troca é `data-variant`, a API pública do progresso, e não uma regra
          que leia `data-status`.

          DIVERGÊNCIA DE FORMA, registrada e não corrigida: aqui a peça é
          composta e a classe cai na RAIZ do progresso (`.nds-progress-root`),
          não no trilho. É a raiz que é filha da grade, então é ela que precisa
          de `grid-column: 1 / -1` — e é também ela que a folha alcança por
          `.nds-progress-root[data-variant="…"]`. No vanilla os dois elementos
          são um só. */}
      <Progress
        className="nds-job-progress-bar"
        value={jobProgressValue(status, count)}
        data-variant={

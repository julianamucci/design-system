...
function ShapeExample({ models }: { models: ModelOption[] }) {
  return (
    <ComposerModelPicker
...
export const Descriptions: Story = {
  ...
  render: () => <ShapeExample models={availableModels()} />,
  play: async ({ canvasElement, step }) => {
    ...
    await step("E o cursor da lista aponta uma opção, sem depender de foco visível", async () => {
      ...
      await expect([...panel.children].map((option) => option.id)).toContain(pointed)
    })
  },
}
...
export const WithBadge: Story = {
  ...
  render: () => <ShapeExample models={badgedModels()} />,
  play: async ({ canvasElement, step }) => {
    const panel = panelOf(canvasElement)

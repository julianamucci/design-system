 ...
 *
 * DIVERGÊNCIA DE API, e só de forma: a lista chega por propriedade, o desenho
 * extra entra por `className`, e a ausência de passos vira retorno nulo — é
 * assim que este renderizador escreve as três coisas. Os nomes das entradas, o
 * que cada uma carrega e o momento em que a lista se redesenha são os mesmos
...
function AgentPlan({
  ...
  labels,
  className,
}: AgentPlanProps): React.ReactElement | null {
  ...
  return (
    // `<ol>` e não `<ul>`: aqui a ordem É a informação (decisão 1 da folha).
    // Quem ouve quer saber que está no terceiro de cinco, e uma lista não
    // ordenada anuncia quantos itens há sem dizer em que lugar cada um está.
    <ol
      data-slot="agent-plan"
      className={cn("nds-agent-plan", className)}
      aria-label={labels.plan}
    >
      {steps.map((step, index) => (
        <li
          key={step.id ?? `${step.label}-${index}`}
          className="nds-agent-plan-step"
          data-slot="agent-plan-step"
          data-state={step.state}
          data-step-id={step.id}
          // `aria-current="step"` é o padrão que o leitor de tela já anuncia
          // como "atual" (decisão 1 da folha). Ele responde "onde estamos" sem
          // depender da cor do marcador, que é justamente o que não chega a
          // quem não a vê.
        ...
        >
          {/* O MARCADOR É DECORATIVO (decisão 3). Ele é a leitura rápida para
              ...
              ouve. */}
          <span
            className="nds-agent-plan-marker"
            data-slot="agent-plan-marker"
            aria-hidden="true"
          />

          {/* O rótulo inteiro, sem corte. A folha resolve a quebra com
              `overflow-wrap` (decisão 5), e cortar aqui em JavaScript seria
              decidir por ela: um passo pela metade é uma instrução pela
              metade. */}
          <span className="nds-agent-plan-label" data-slot="agent-plan-label">
            {step.label}
          </span>
          ...
          {/* A palavra do estado, em etiqueta curta. É `Badge` de propósito: a
              ...
              O `data-slot` da etiqueta é sobrescrito para o do plano, como a
              fila de envio já faz. */}
          <Badge data-slot="agent-plan-state" variant={STATE_VARIANT[step.state]}>
            {labels.state[step.state]}
          </Badge>

          {/* O detalhe é o que o rótulo não diz: por que pulou, o que produziu,
              o que falhou. Ele é texto corrido em container de estado, então
              fica na cor neutra e nunca na semântica (regra do CLAUDE.md
              raiz). */}
          {step.detail ? (
            <p className="nds-agent-plan-detail" data-slot="agent-plan-detail">
              {step.detail}
            </p>
          ) : null}
        </li>

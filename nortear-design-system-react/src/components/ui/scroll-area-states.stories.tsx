...
function List() {
  return (
    <div className="nds-p-4" data-spacing="sm">
      {items.map((n) => (
        <div key={n} className="nds-text-body nds-border-b nds-last-border-0 nds-pb-2">
          Item {n}
        </div>
      ))}
    </div>
 ...
 *
 * A barra da lib nasce escondida até ser medida (`visibility: hidden`) e ainda
 * atravessa uma transição. `toBeVisible()` do jest-dom só reprova em opacidade
...
export const Idle: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Estado padrão — a barra existe enquanto houver transbordo e o viewport rola sem mover a página.",
      },
    },
  },
  render: () => (
...
export const AlwaysVisible: Story = {
  parameters: {
    ...
    docs: {
      description: {
        story:
          'A barra permanece montada enquanto houver transbordo — nesta stack isso não depende de propriedade nenhuma, e é a condição para arrastar o pegador e para medir o contraste dele.',
      },
    },
  },
  render: () => (
...
export const Hover: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Ponteiro sobre a área — a lib publica o estado na própria barra (`data-hovering`), para o CSS decidir a aparência. Não há propriedade de tempo de espera nesta stack.',
      },
    },
  },
  render: () => (
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("O ponteiro sobre a área marca a barra como sob o cursor", async () => {
      ...
      // rodada, mesmo quando o painel Interactions reexecuta no mesmo DOM.
      //
...
export const NoLimit: Story = {
  ...
  render: () => (
    <div className="nds-stack" data-spacing="lg" style={{ width: "300px" }}>

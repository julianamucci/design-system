...
function playgroundSource(
  _gerado: string,
  ctx: { args?: Partial<{ enableRowSelection: boolean; enablePagination: boolean; pageSize: number }> }
): string {
  const {
  ...
  return `import { DataTable, type DataTableColumn } from "@/components/ui/data-table"
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("O checkbox do cabeçalho tem nome diferente do das linhas", async () => {
      ...
      await expect(tudo.getAttribute("aria-label")).not.toBe(
        daLinha.getAttribute("aria-label")
      )
    })

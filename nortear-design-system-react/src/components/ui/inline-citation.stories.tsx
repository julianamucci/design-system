...
import { InlineCitationDocs } from "@/components/docs/InlineCitationDocs"
...
function PlaygroundExample(args: PlaygroundArgs) {
  ...
  return (
    // SEM ESPAÇO ANTES DA MARCA: entre duas expressões a quebra de linha do JSX
    // não vira espaço, e é assim que a marca não se separa da palavra que a
    // antecede quando a linha quebra.
    <p>
      {parts[0]}
      <InlineCitation
        // `defaultOpen` é um PADRÃO, e padrão só vale na montagem. A chave
        // remonta a marca quando o control muda, que é o que faz o eixo
        // continuar fotografável sem inventar uma propriedade controlada.
        key={`${args.shape}-${args.defaultOpen}`}
        citation={citation}
        index={1}
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(InlineCitationDocs),
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    // Espera por RELÓGIO, e com leitura pura: espera por observador de mutação
    // que tocasse o DOM reagendaria a si mesma e penduraria a aba sem reprovar.
    ...
    await step("O alvo de toque é maior que a marca, e vive FORA do fluxo", async () => {
      // Aumentar a marca em si esticaria a entrelinha do parágrafo em toda
      // linha que trouxesse uma citação (decisão 7 da folha). Leitura pura de
      // estilo calculado, uma vez — nada aqui espera por nada.
    ...
    await step("Nada aqui é região viva, e o estado aberto não é só cor", async () => {
      ...
      await expect(
        root.querySelector('[role="status"], [role="alert"], [role="log"]'),
      ).toBeNull()

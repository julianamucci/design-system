...
const indents = (piece: HTMLElement) =>
  [...piece.querySelectorAll<HTMLElement>('[data-slot="trace-waterfall-row"]')].map((row) =>
    ...
    // declarada dentro do próprio seletor vence a herança, e ler o estilo do
    // elemento provaria só que alguém escreveu alguma coisa ali.
 ...
 * convenção nenhuma sobre onde começa a contagem, e a cascata inteira encosta.
 * O desenho é idêntico ao da story em andamento, e é esse o assunto.
 */
...
export const LongLabels: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Ele não quebra: a linha continua de uma linha só', async () => {
      ...
      // `font-size`, que computa em pixel em qualquer motor — entrelinha
 ...
 * folha de quem monta, e nunca por largura em estilo embutido.
 */
export const TightColumns: Story = {
  ...
  // folha a declara em `.nds-trace-waterfall`, e propriedade personalizada
  // declarada dentro do próprio seletor vence a herança — o invólucro nunca a
  // alcançaria.
  render: () => ({
    props: {
    ...
    template: `
      <div class="nds-stack nds-max-w-md" data-spacing="lg">
        ...
        <div data-testid="trace-waterfall-tight-columns">
          <div
            ndsTraceWaterfall
            [spans]="spans"
            [totalMs]="totalMs"
            status="running"
            [labels]="labels"
            [style.--trace-waterfall-name-min]="tightNameMin"
          ></div>
        </div>
      </div>
    `,
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A coluna do nome aperta, e a caixa encolhe com ela', async () => {
      ...
      // chegou ao elemento — ler o estilo do elemento provaria só que alguém
      // escreveu ali.

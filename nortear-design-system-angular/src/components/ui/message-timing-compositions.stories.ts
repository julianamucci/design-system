...
//
// DIVERGÊNCIA DE FORMA, e é de framework: a linha entra na dica como
// `<ng-template ndsTooltipContent>`, e não como elemento pronto passado numa
// opção. É o que permite ao portal criar e destruir o conteúdo junto com o
...
export const InTightSpace: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E o termo nunca se separa do seu valor ao quebrar', async () => {
      // O elemento de agrupamento de cada par é o que faz o PAR quebrar inteiro.
      // Sem ele, o termo terminaria uma linha e o valor abriria a seguinte.
...
export const InsideTooltip: Story = {
  ...
  render: () => ({
    props: {
      stats: statsOf('settled'),
      labels: messageTimingLabels(),
      triggerLabel: messageTimingTriggerLabel(),
    },
    template: `
      <div ndsTooltipProvider [delay]="0">
        <span ndsTooltip>
          <button
            ndsTooltipTrigger
            ndsButton
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cleanup', async () => { trigger.blur(); });
  },
};

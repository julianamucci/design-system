...
import { NdsTerminalBlockDocs } from '@/components/docs/TerminalBlockDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsTerminalBlockDocs),
      ...
      // real: um componente que declara os rótulos e a saída que recebeu.
  ...
  argTypes: {
    ...
    withOutput: {
      control: 'boolean',
      description:
        'Houve saída? Sem linha nenhuma não há caixa que rola, e é o comando que terminou sem escrever nada.',
...
export const Playground: Story = {
  ...
  // O comando e a saída vêm do módulo compartilhado, e os rótulos do andaime:
  ...
  render: (args) => ({
    props: {
      ...
      status: args.status,
      // Controle numérico vazio é ausência de código de saída, e não zero: zero
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A monoespaçada está nos dois lugares que dependem dela', async () => {
      // Ela vem de uma utilitária que mora na marcação, não na folha desta peça
      // — e por isso some em silêncio quando alguém copia a árvore pela metade.
      // Esta asserção é o que impede o silêncio: saída de terminal se alinha em
    ...
    await step('A caixa de saída existe quando há o que mostrar, e tem o comando por nome', async () => {
      const output = root.querySelector<HTMLElement>('[data-slot="terminal-block-output"]');
      const shouldExist =
        (args.withOutput && linesFor(args.status).length > 0) || args.status === 'running';
      if (!shouldExist) {
        // Caixa vazia com parada de tabulação dentro seria dar foco a lugar
        // nenhum — e um comando que não escreveu nada acontece de verdade.

...
import { NdsContextDisplayDocs } from '@/components/docs/ContextDisplayDocs';
...
// primitivo já responde, e é o único jeito de alcançar essa medição por control.
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsContextDisplayDocs),
      ...
      // real: um componente que declara os rótulos e passa a medição.
  ...
  argTypes: {
    ...
    form: {
      ...
      description:
        'Como desenhar o mesmo número. A escolha é de espaço, e não de significado.',
      table: {
        type: { summary: CONTEXT_DISPLAY_FORMS.map((f) => `'${f}'`).join(' | ') },
...
export const Playground: Story = {
  ...
  render: (args) => ({
    props: {
      ...
      usage: { input: args.input, output: args.output, limit: args.limit || undefined },
      form: args.form,
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('E a palavra do nível está na linha, ao lado da cor', async () => {
      // Pela CLASSE, e não pelo `data-slot`: o `ndsBadge` liga
      // `data-slot="badge"` por host binding, e o atributo estático do template
      // disputa com ele (§8 do RULES.md). A classe é o que a folha estiliza e
      // não disputa com ninguém.
      const level = budgetLevel(usage);
      const badge = root.querySelector<HTMLElement>('.nds-context-display-level')!;

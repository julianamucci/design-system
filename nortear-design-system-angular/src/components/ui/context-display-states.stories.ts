...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// O que a medição diz conforme quanto já foi gasto — e o que acontece quando não
...
const levelOf = (block: HTMLElement) =>
  block.querySelector<HTMLElement>('.nds-context-display-level');
...
export const AllLevels: Story = {
  ...
  render: () => ({
    props: {
      readings: BUDGET_LEVELS.map((level) => usageOf(level)),
      labels: contextDisplayLabels(),
    },
    template: `
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Há um bloco por nível, na ordem do primitivo', async () => {
      ...
      await expect(blocks.map((b) => b.dataset.level)).toEqual([...BUDGET_LEVELS]);
    });
    ...
    await step('Cada um traz a PALAVRA daquele nível', async () => {
      ...
      for (const [i, level] of BUDGET_LEVELS.entries()) {
        await expect(levelOf(blocks[i]!)!.textContent).toBe(labels.level[level]);
      }
    });
    ...
    await step('E as três palavras são DIFERENTES entre si', async () => {
      ...
      const words = blocks.map((b) => levelOf(b)!.textContent);
    ...
    await step('A palavra aparece inclusive com folga', async () => {
      ...
      await expect(levelOf(blocks[0]!)).not.toBeNull();
    });
  },
};
...
export const Unbounded: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E não há palavra de nível, porque não há nível', async () => {
      ...
      await expect(levelOf(block)).toBeNull();
    });

...
const { t } = useTranslation(costTranslations as Record<string, unknown>);
...
/** As situações que a peça desenha diferente. */
...
/**
 * Um gasto por situação, todos contra o mesmo teto de um dólar.
 *
 * O teto é o MESMO em cinco dos seis para que a diferença entre as fotos seja o
 * gasto, e não a escala. O sexto não tem teto — é a situação em que ele não foi
 * declarado, e é justamente a que não pode parecer "não gastou nada".
 *
...
/** O gasto daquele exemplo, já escrito. */
export function amountOf(name: CostMeterCase): string {
  return costAmount(COST_METER_SPEND[name].spent);
}

/**
 * O teto daquele exemplo — a quantia escrita e a fração já gasta —, ou nada.
 *
 ...
 * esse `null` que vira "sem teto declarado" na tela, e é por ele que a peça sem

...
import { SAMPLE_DRAFT, SAMPLE_TIMESTAMP, draftLabels } from './draft-restore.fixtures';
import { draftRestoreSource } from './draft-restore.source';
import { NdsDraftRestoreDocs } from '@/components/docs/DraftRestoreDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsDraftRestoreDocs),
      ...
      // real: um componente que guarda o rascunho e trata a escolha.
...
export const Playground: Story = {
  ...
  render: (args) => ({
    props: {
      labels: draftLabels(),
      draft: args.draft,
      // Campo de texto vazio é ausência de carimbo, e não um carimbo em branco:
      // uma string vazia desenharia o separador sem nada depois dele.
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A faixa se anuncia como ESTADO, e não como alerta', async () => {
      // A faixa nasce junto com a tela e quem lê está começando a ler: um alerta
      // interromperia a leitura em curso por algo que não é urgente (decisão 1
      // da folha). `status` espera a primeira pausa.
    ...
    await step('A prévia carrega o rascunho INTEIRO', async () => {
      // O corte é da folha, por `line-clamp`. Comparar o texto com o que entrou
      // é o que separa um corte visual de um corte no texto — e só o visual
      // mantém o rascunho achável pela busca do navegador.
    ...
    await step('Cada controle NOMEIA o rascunho, e não traz só o verbo', async () => {
      // Dois verbos sozinhos são dois destinos sem assunto para quem chega neles
      // por tabulação vindo de outro lugar da tela (decisão 3).
    ...
    await step('E a faixa não rouba o foco ao aparecer', async () => {
      // Quem entrou para escrever continua onde estava; a faixa é alcançável por
      // estar ANTES na ordem de leitura, e não por puxar ninguém para si.

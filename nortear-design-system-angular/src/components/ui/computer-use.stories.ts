...
import { NdsComputerUseDocs } from '@/components/docs/ComputerUseDocs';
...
// que a legenda diz; e a ausência de passo decide se existe rastro e legenda. A
// grade dos cinco estados mora em `States`; aqui o assunto é o que muda quando
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsComputerUseDocs),
      ...
      // real: um componente que declara os passos, os rótulos e a tela.
...
export const Playground: Story = {
  ...
  // O endereço e os passos vêm do módulo compartilhado, e os rótulos do andaime:
  // eles têm três idiomas, e uma palavra escrita à mão aqui congelaria um deles.
  render: (args) => ({
    props: {
      url: COMPUTER_URL,
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A tela entra no quadro, e a peça não toca no que ela traz', async () => {
      // A tela é ESPAÇO de quem consome: a peça instancia o template inteiro e
      // não mexe em nada do que veio com ele — nem no texto alternativo, nem no
      // `inert` que faz dela uma foto em vez de um formulário para preencher.
    ...
    await step('A marca e o rastro ficam fora do que é lido em voz', async () => {
      // Posição numa imagem não chega a quem não a vê — o que chega é a legenda,
      // e é por isso que ela existe.
    ...
    await step('Nada na peça é região viva', async () => {
      ...
      // cada um tornaria a tela impossível de ouvir (regra 1 da §8 da guideline
      // 17). O que existe no lugar é a peça se declarar ocupada.

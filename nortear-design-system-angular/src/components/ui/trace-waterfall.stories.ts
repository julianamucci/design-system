...
import { NdsTraceWaterfallDocs } from '@/components/docs/TraceWaterfallDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsTraceWaterfallDocs),
      ...
      // real: um componente que declara os trechos, o eixo e os rótulos.
...
export const Playground: Story = {
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    if (args.revealed === 0) {
      // Sem trecho não há cascata, e a peça não desenha nada — nem régua, nem
      // camada que rola. Nesta stack o host continua no documento, porque
      // quem o escreve é quem consome; o que a peça controla é tudo o que
      // estaria dentro dele.
      await step('Sem trecho nenhum, nada é desenhado', async () => {
        await expect(root.children.length).toBe(0);
        await expect(root.querySelector('[data-slot="trace-waterfall-viewport"]')).toBeNull();
    ...
    await step('Cada barra cai na fração do eixo que o dado manda', async () => {
      ...
      // declarada dentro do próprio seletor vence a herança, e ler o estilo
      // do elemento provaria só que alguém escreveu alguma coisa ali.

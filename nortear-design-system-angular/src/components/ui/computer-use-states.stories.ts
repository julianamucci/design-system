...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// Os cinco momentos de uma sessão, mais os dois casos em que o rastro não está
// cheio: o começo, quando ainda não há três marcas, e a moldura antes do
// primeiro toque. Não há eixo de forma nesta peça — a estrutura é sempre a
// mesma, endereço, quadro e legenda — e o que muda é quanto cada parte tem para
// dizer.

const meta: Meta = {
  title: 'Primitives/Conversational/ComputerUse/States',
  tags: ['conversational'],
  decorators: [moduleMetadata({ imports: [NdsComputerUse, NdsComputerUseDemoScreen] })],
...
/**
 * Uma peça, na configuração que a story desenha.
 *
 * A tela é um `<ng-template>` DECLARADO UMA VEZ e instanciado por cada figura:
 * um template pode ser instanciado quantas vezes for preciso, e cada
 * instanciação monta a sua própria tela — com o próprio escopo de id.
 */
 ...
 *
 * A lista sai de `RUN_STATUSES`, e não de cinco figuras escritas à mão: estado
 * novo no vocabulário compartilhado entra nesta story sozinho, e ninguém precisa
...
export const EveryStatus: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Há uma peça por estado, e cada uma diz qual é o seu', async () => {
      await expect(pieces.length).toBe(RUN_STATUSES.length);
...
export const Running: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A peça se declara ocupada e a marca em curso é a última do rastro', async () => {
      ...
      await expect(marks.slice(0, -1).every((m) => m.dataset.active === undefined)).toBe(true);
    });
    ...
    await step('O ponto de cada marca chega em porcentagem do quadro', async () => {
      ...
      // porque não existe token de "42%". Cadeia, e não número — `[style.--x]`
      // com valor numérico faria o Angular anexar "px" e a marca cairia fora do
 ...
 *
 * Com o índice no primeiro passo há uma marca só. É o que toda sessão atravessa,
 * e o que mais escapa de quem só fotografa o meio.
 */
 ...
 *
 * Sem passo nenhum não há rastro nem legenda: sobra o endereço e a tela, que é o
 * que existe antes de o agente tocar em qualquer coisa. Uma legenda vazia daria
 * à figura um nome em branco.
 */
...
export const ClampedIndex: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A legenda aponta para o último passo, e não para lugar nenhum', async () => {
      ...
      await expect(marksIn(piece).length).toBe(3);
    });
  },
};

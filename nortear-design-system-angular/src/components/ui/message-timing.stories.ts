...
import { NdsMessageTiming } from './message-timing';
import { messageTimingLabels, statsOf } from './message-timing.fixtures';
import { messageTimingSource } from './message-timing.source';
import { NdsMessageTimingDocs } from '@/components/docs/MessageTimingDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsMessageTimingDocs),
      ...
      // real: um componente que escreve as medidas e declara os rótulos.
...
export const Playground: Story = {
  ...
  // Os rótulos e as medidas vêm do andaime compartilhado, e não de literais:
  // eles têm três idiomas, e um número escrito à mão aqui congelaria um deles.
  render: (args) => ({
    props: {
      // O corte sai da MESMA lista, e não de quatro listas diferentes: é assim
      // que a story prova que a ordem é preservada em qualquer contagem.
      stats: statsOf('settled').slice(0, args.measures),
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('Termo e valor são um par de lista de definição', async () => {
      // Com a lista de definição, o par sobrevive a quem navega de item em item
      // (decisão 1 da folha), e é o elemento de agrupamento de cada par que o
      // faz quebrar de linha inteiro.

...
import { NdsQuotaBannerDocs } from '@/components/docs/QuotaBannerDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsQuotaBannerDocs),
      ...
      // real: um componente que escreve o horizonte e declara os rótulos.
...
export const Playground: Story = {
  ...
  render: (args) => ({
    props: {
      ...
      horizon: args.renews ? renewalIn(RENEWAL_MINUTES) : undefined,
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('O nível chega em PALAVRA, e a cor apenas acompanha', async () => {
      ...
      // disputa com ele (§8 do RULES.md).
      await expect(root.dataset.level).toBe(level);
      await expect(root.querySelector('.nds-quota-banner-level')).not.toBeNull();

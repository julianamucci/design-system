...
export const Streaming: Story = {
  ...
  render: () => ({
    props: { content: MARKDOWN_STREAMING },
    template: '<nds-markdown [content]="content" [streaming]="true" />',
  }),
...
export const RefusedContent: Story = {
  ...
  render: () => ({
    props: { content: MARKDOWN_UNSAFE },
    template: '<nds-markdown [content]="content" />',
  }),
  play: async ({ canvasElement, step }) => {
...
export const Empty: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    //
    // `:empty` ignora comentário, que é o que a raiz guarda aqui: as âncoras
    // de `ng-template` são nós de comentário.

...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// Os cinco momentos de um comando, e o comando que não escreveu nada. Não há
  ...
  render: () => ({
    ...
    template: `
      <div class="nds-stack" data-spacing="lg">
        @for (run of runs; track run.status) {
          <div
            ndsTerminalBlock
            [command]="command"
            [lines]="run.lines"
...
export const Complete: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A saída chega com o espaçamento intacto', async () => {
      ...
      // alinhava os números viraria um parágrafo. E o texto sai exatamente como
      // a peça o recebeu: recuo de template dentro do bloco pré-formatado seria
      // saída visível que ninguém escreveu.
      const output = outputOf(piece)!;
      await expect(output.textContent).toContain(TERMINAL_LINES_COMPLETE[3]);
      await expect(output.textContent).toContain(TERMINAL_LINES_COMPLETE.join('\n'));
 ...
 *
 * É o que a decisão 7 existe para não errar: a linha rola na horizontal DENTRO
 * do próprio bloco, e o bloco não cresce para acomodá-la.
 */
 ...
 *
 * Acontece de verdade, e não é borda — é o que um terminal mostra. A ausência
 * da caixa é o assunto: caixa vazia com parada de tabulação dentro seria dar
 * foco a lugar nenhum.
 */
export const WithoutOutput: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Nenhuma das duas desenha caixa de saída', async () => {
      // Uma terminou sem escrever nada, a outra ainda não rodou. As duas dizem
      // a mesma coisa sobre a caixa: não há o que mostrar.

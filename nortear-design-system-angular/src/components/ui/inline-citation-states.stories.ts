...
// leitura pura: nesta stack a renderização é agendada, e ler logo depois do
// clique pega a árvore de antes.
...
const closeMarker = async (el: HTMLElement, root: HTMLElement) => {
  if (el.getAttribute('aria-expanded') !== 'false') await userEvent.click(el);
  await awaitNoPanel(root);
};

/** A frase daquele formato de citação, com a marca já aberta. */
const sentenceOf = (name: InlineCitationCase) => {
  ...
  return {
    ...
    // SEM ESPAÇO ANTES DA MARCA: as emendas encostam de propósito.
    template: `
      <p>{{ antes }}<span
          ndsInlineCitation
          [citation]="citation"
 ...
 *
 * É o formato que mostra as quatro linhas de uma vez, e é nele que se vê por que
 * a caixa não precisa de nome próprio: quem chegou até ela veio da marca, e a
 ...
 * lugar, a prévia mostra o endereço e o título e para aí — sem traço, sem espaço
 * reservado e sem linha vazia.
 */

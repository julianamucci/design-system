...
import { fractionLevel, fractionPercent, spentFraction } from '@shared/primitives/token-budget';
import { NdsCostMeter, type CostBudget } from './cost-meter';
import { costAmount, costMeterLabels } from './cost-meter.fixtures';
import { costMeterSource } from './cost-meter.source';
import { NdsCostMeterDocs } from '@/components/docs/CostMeterDocs';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsCostMeterDocs),
      ...
      // real: um componente que escreve o dinheiro e declara os rótulos.
...
const budgetFrom = (args: PlaygroundArgs): CostBudget | undefined => {
  const fraction = spentFraction(args.spent, args.budget);
  if (fraction === null) return undefined;
  return { amount: costAmount(args.budget), fraction };
};
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    if (budget && fraction !== null) {
      ...
      await step('O nível chega em PALAVRA, e a cor apenas acompanha', async () => {
        ...
        // `data-slot="badge"` por host binding, e o atributo estático do
        // template disputa com ele (§8 do RULES.md).
        const canvas = within(canvasElement);
        await expect(root.dataset.level).toBe(level);
        await expect(root.querySelector('.nds-cost-meter-level')).not.toBeNull();

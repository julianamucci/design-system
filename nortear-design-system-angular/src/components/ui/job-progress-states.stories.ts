...
// ─── Meta ─────────────────────────────────────────────────────────────────────
//
// Os cinco momentos de um trabalho longo, e o que acontece quando ninguém sabe
// de quantas. Não há eixo de forma nesta peça: a grade é sempre a mesma, e o
...
const actionOf = (piece: HTMLElement) =>
  piece.querySelector<HTMLButtonElement>('.nds-job-progress-action');
...
export const EveryStatus: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E os rótulos de ação são TRÊS nomes diferentes', async () => {
      ...
      const names = [
        ...canvasElement.querySelectorAll<HTMLElement>('.nds-job-progress-action'),
      ].map((b) => b.textContent);
...
export const UnknownTotal: Story = {
  ...
  render: () => ({
    props: {
      label: jobLabel(),
      omitted: JOB_COUNT_WITHOUT_TOTAL,
      zeroed: { done: JOB_COUNT_WITHOUT_TOTAL.done, total: 0 } as JobCount,
      labels: jobProgressLabels(),
      onAction,
    },
    template: `

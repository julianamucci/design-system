...
const gridLines = (piece: HTMLElement, axis: 'column' | 'row') =>
  [...piece.querySelectorAll<HTMLElement>('[data-slot="flow-graph-node"]')].map((node) =>
    ...
    // dentro do próprio seletor vence a herança, e ler o estilo do elemento
...
export const LongLabels: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O rótulo inteiro está no DOM, sem reticências', async () => {
      ...
      // vezes o tamanho da fonte, o que uma linha só nunca seria. Medido contra
...
export const Rejoin: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A ligação que volta é desenhada como as outras', async () => {
      // Fila não se ramifica nem se reencontra, e é este desenho que ela não
    ...
    await step('O nó de reencontro diz que depende dos dois lados', async () => {
      // O endereço é o do dado, e não o do identificador que o declara: quem
      // procura no DOM procura por `data-node-id`, que é o que a peça escreve.
 ...
 * monta, e nunca por largura em estilo embutido.
 */
export const TightColumns: Story = {
  ...
  // token declarado por conta ele volta como a expressão, não como o pixel. O
  // que compara de verdade é a largura da caixa, e para comparar é preciso haver
  // duas.
  //
  // A propriedade é declarada NO PRÓPRIO elemento, e não no invólucro: a folha a
  // declara em `.nds-flow-graph`, e propriedade personalizada declarada dentro
  // do próprio seletor vence a herança — o invólucro nunca a alcançaria.
  render: () => ({
    props: {
      nodes: wideFlowNodes(),
      edges: wideFlowEdges(),
      labels: flowGraphLabels(),
      tightColumnMin: 'var(--spacing-16)',
    },
    template: `
      <div class="nds-stack nds-max-w-md" data-spacing="lg">
        ...
        <div data-testid="flow-graph-tight-columns">
          <div
            ndsFlowGraph
            [nodes]="nodes"
            [edges]="edges"
            status="running"
            [labels]="labels"
            [style.--flow-graph-column-min]="tightColumnMin"
          ></div>
        </div>
      </div>
    `,

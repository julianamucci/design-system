...
const render = (allow: readonly string[]) => () => ({
  props: { content: MARKDOWN_COMMENT, allow },
  template: '<nds-markdown [content]="content" [allow]="allow" />',
});
...
export const Full: Story = {
  ...
  render: render(ALLOW_PRESETS.full),
...
export const Chat: Story = {
  ...
  render: render(ALLOW_PRESETS.chat),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
export const Comment: Story = {
  ...
  render: render(ALLOW_PRESETS.comment),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);

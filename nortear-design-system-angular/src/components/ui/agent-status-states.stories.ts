...
export const EveryState: Story = {
  ...
  render: () => ({
    props: {
      rows: RUN_STATUSES.map((status) => ({ status, elapsed: elapsedOf(status) })),
      labels: agentStatusLabels(),
      onAction,
    },
    template: `
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Há uma linha por estado, na ordem do vocabulário', async () => {
      ...
      await expect(lines.map((line) => line.dataset.status)).toEqual([...RUN_STATUSES]);
    });
...
export const Failed: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Depois do fim, a MESMA ação passa a pedir para começar de novo', async () => {
      ...
      const button = actionOf(line)!;
      await expect(button).toHaveAccessibleName(labels.action!.failed!);
      onAction.mockClear();
      await userEvent.click(button);

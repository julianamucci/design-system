...
import { generatingLabel } from './thinking-indicator.fixtures';
import { thinkingIndicatorSource } from './thinking-indicator.source';
import { NdsThinkingIndicatorDocs } from '@/components/docs/ThinkingIndicatorDocs';
...
// é o que a espera DIZ, e é isso que o controle move. Por isso não há arquivo de
// variantes: a peça não tem eixo de forma.
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsThinkingIndicatorDocs),
      ...
      // real, com a frase que os controls estão mostrando.
  ...
  args: {
    // O rótulo vem do andaime compartilhado, e não de um literal: ele tem três
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    template: `<p ndsThinkingIndicator [label]="label"></p>`,
  }),

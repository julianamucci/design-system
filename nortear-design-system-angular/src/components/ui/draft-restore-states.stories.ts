...
// que ela encontrou — se há carimbo, se o texto cabe nas duas linhas. Um arquivo
// de variantes com estados dentro diria que há uma escolha de forma onde não há,
// e a próxima pessoa procuraria a diferença que não existe.
//
// O quarto estado da tabela — SEM RASCUNHO — não tem story, e também é decisão:
// ele é a faixa que não existe. Quem consome não a monta, porque faixa vazia é
// pergunta sem assunto. Fotografar o nada provaria só que o Chromatic sabe
// fotografar o nada.
...
export const Dated: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E ele é LIDO junto com o título, ao contrário de um cronômetro', async () => {
      ...
      // não protegeria ninguém, só tiraria de quem ouve a informação de quando o
      // rascunho é.
      const stamp = titleOf(canvasElement)
        .querySelector<HTMLElement>('[data-slot="composer-draft-timestamp"]')!;
...
export const LongDraft: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    await step('O rascunho INTEIRO continua no documento', async () => {
      // O texto que entrou é o texto que está lá, letra por letra. É isso que o
      // mantém achável pela busca do navegador e audível do começo ao fim.

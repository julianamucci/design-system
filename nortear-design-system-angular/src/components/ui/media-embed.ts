...
// o protocolo inteiro é verificável sem sair da máquina — e sem o compilador de
// template no caminho, o que o mantém no projeto `unit` do vitest. O que sobra
// sem cobertura é só o aperto de mão real, declarado na story.
...
export function embedCommand(provider: EmbedProvider, command: EmbedCommand): string {
  if (provider === 'youtube') {
    ...
    return JSON.stringify({ event: 'command', ...call });
  }
...
export function parseEmbedMessage(provider: EmbedProvider, data: unknown): EmbedEvent | null {
  ...
  if (provider === 'youtube') {
    if (payload['event'] === 'onStateChange') {
      const kind = YOUTUBE_STATE[Number(payload['info'])];
      return kind ? ({ type: kind } as EmbedEvent) : null;
    }
  ...
  const details = payload['data'] as { seconds?: number; duration?: number } | undefined;
  if (name === 'play') return { type: 'playing' };
  if (name === 'pause') return { type: 'paused' };
  if (name === 'ended') return { type: 'ended' };
  if (name === 'timeupdate' && details && typeof details.seconds === 'number') {
    return { type: 'time', currentTime: details.seconds, duration: details.duration ?? 0 };
  }
  return null;
}

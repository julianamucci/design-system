...
/**
 * A lista, atrás da guarda que quem consome escreve.
 *
 * A raiz é o próprio `<ol>`, então a peça não tem como se apagar sozinha quando
 * não há passo nenhum: sem a guarda no template, a story da lista vazia
 * desenharia uma lista de zero itens em vez de nada.
 */
const mount = (steps: PlanStep[]) => ({
  props: { steps, labels: agentPlanLabels() },
  template: `
...
export const Empty: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    await step('Sem passo nenhum, o plano não existe no documento', async () => {
      ...
      // Aqui a raiz é a própria lista, então quem faz a ausência acontecer é a
      // guarda do template — e é ela que esta story mede.

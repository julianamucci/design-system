...
export const InRail: Story = {
  ...
  // O seletor é AUTÔNOMO: ele não é uma entrada do campo. Quem consome o monta
  // e o põe no trilho, pelo mesmo espaço de qualquer outro controle.
  render: () => ({
    props: {
      labels: composerLabels(),
      pickerLabels: modelLabels(),
      models: everyModel(),
      onValueChange,
    },
    template: `
      ...
      <nds-composer class="nds-max-w-lg" [labels]="labels" [railStart]="rail" />
    `,
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('O seletor vive no INÍCIO do trilho', async () => {
      ...
      // ela. Escolher quem responde é do início.
...
export const Keyboard: Story = {
  ...
  render: () => ({
    props: { labels: modelLabels(), models: availableModels(), onValueChange },
    template: `
      <nds-composer-model-picker
        [labels]="labels"
        [models]="models"
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Acionar o gatilho abre a lista e leva o foco para dentro dela', async () => {
      // Aqui não há texto em curso: a escolha é o único assunto enquanto a
      // lista está aberta, e o teclado tem de estar onde ela está.
      onValueChange.mockClear();
      await userEvent.click(trigger);
      // Leitura pura dentro do laço: a condição não toca no documento, então
      // ela não provoca a própria reagendagem.
      await waitFor(async () => {
        await expect(panelOf(canvasElement)).not.toBeNull();
        await expect(doc.activeElement).toBe(panelOf(canvasElement));
      });
...
export const OutsideClick: Story = {
  ...
  // Um alvo qualquer FORA do seletor. Ele existe só para a play ter onde
  // acionar, e por isso é texto inerte: um botão de verdade acrescentaria um
  // controle sem assunto à foto e ao percurso do teclado.
  render: () => ({
    props: { labels: modelLabels(), models: availableModels(), onValueChange },
    template: `
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Acionar algo fora do seletor fecha a lista', async () => {
      // Sem isto a lista ficaria aberta por cima do que a pessoa foi fazer, e
      // o único jeito de fechá-la seria voltar ao gatilho.
      await userEvent.click(trigger);
      await waitFor(async () => {
        await expect(panelOf(canvasElement)).not.toBeNull();
      });

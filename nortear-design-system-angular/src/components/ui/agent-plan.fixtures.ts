/**
 * Andaime das demonstrações do plano — um construtor por configuração.
 *
 * Os RÓTULOS DE INTERFACE saem da `translations.json`: o nome da lista e a
 * palavra de cada estado são texto de tela. O TEXTO DOS PASSOS é dado de exemplo
 * e fica igual nos três idiomas — traduzi-lo faria as cinco stories fotografarem
 * listas de larguras diferentes conforme o idioma da foto, e o assunto da peça
 * não é a fala.
 *
...
const { t } = useTranslation(planTranslations as Record<string, unknown>);
...
/** A lista a partir dos estados, na ordem dos passos de exemplo. */

...
// não há palavra. As quatro são situações em que o desenho sozinho falharia — e
...
export const Sliver: Story = {
  ...
  render: () => ({
    props: { parts: partsOf('sliver'), labels: contextBreakdownLabels() },
    template: TEMPLATE,
  }),
...
export const SingleOrigin: Story = {
  ...
  render: () => ({
    props: { parts: partsOf('single'), labels: contextBreakdownLabels() },
    template: TEMPLATE,
  }),
  play: async ({ canvasElement, step }) => {
    const block = blockOf(canvasElement);
...
export const Empty: Story = {
  ...
  render: () => ({
    props: { parts: partsOf('empty'), labels: contextBreakdownLabels() },
    template: TEMPLATE,
  }),
  play: async ({ canvasElement, step }) => {
    const block = blockOf(canvasElement);
 ...
 *
 * O que falta é o que se sabe dizer sobre a repartição, e não a repartição:
 * a story tira um rótulo, e nunca inventa uma parcela. Sem nome, a cor ficaria
 * sozinha dizendo de qual parcela se trata — e é justamente isso que a peça não
 * aceita.
 */
export const UnlabeledOrigin: Story = {
  ...
  render: () => ({
    props: {
      ...
      labels: contextBreakdownLabelsWithout('tools'),
    },
    template: TEMPLATE,
  }),
  play: async ({ canvasElement, step }) => {

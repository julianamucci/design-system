...
export const JobQueue: Story = {
  ...
  render: () => ({
    props: {
      label: jobLabel(),
      withTotal: JOB_COUNT,
      withoutTotal: JOB_COUNT_WITHOUT_TOTAL,
    ...
    template: `
      <div class="nds-stack nds-max-w-lg" data-spacing="lg">
        ...
        <div
          ...
          (action)="onAction($event)"
        ></div>
        <!-- O que espera não recebe conta nenhuma: nada começou, e não há o
             que contar. -->
        <div
          ndsJobProgress
          [label]="label"
...
export const Requesting: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const action = piece.querySelector<HTMLButtonElement>('.nds-job-progress-action')!;

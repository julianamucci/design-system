 ...
 * é a entrada hostil que o componente precisa aguentar. As cinco stacks montam a
 * mesma lista, para a story fotografar a mesma tela em todas.
 */
function withTools(): ChatMessage[] {
...
export const WithReasoningAndTools: Story = {
  ...
  render: () => ({
    props: { messages: withTools(), labels: chatLabels() },
    template: '<nds-chat-thread [messages]="messages" [labels]="labels" size="lg" />',
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('O controle é alcançável por teclado, e abre', async () => {
      ...
      // reproduz — medido, `userEvent.keyboard('{Enter}')` deixa o `<details>`
      // fechado. Asserção com a tecla aqui não provaria o componente, provaria
      // o simulador.
    ...
    await step('A fonte de endereço recusado é legível, e NÃO é clicável', async () => {
      ...
      const sources = root.querySelector<HTMLElement>('.nds-chat-sources')!;
      const refused = sources.querySelector<HTMLElement>('[data-unsafe]')!;
      await expect(refused.tagName).toBe('SPAN');
      await expect(refused).toHaveTextContent(/recusado/i);
...
// ── As ações do turno ─────────────────────────────────────────────────────────
//
// O andaime é um COMPONENTE porque as ações são `TemplateRef` nesta stack: um
// `<ng-template>` só existe depois que a vista foi criada, e um objeto de
// `props` é montado antes.

@Component({
  selector: 'nds-chat-thread-actions-demo',
  standalone: true,
  imports: [NdsChatThread, NdsButton],
  template: `
    <ng-template #actionsTpl>
      <button ndsButton variant="ghost" size="sm" (click)="copy()">Copiar</button>
      <button ndsButton variant="ghost" size="sm">Refazer</button>
    </ng-template>

    <nds-chat-thread [messages]="messages()" [labels]="labels" size="lg" />
  `,
})
class ActionsDemo {
  readonly labels = chatLabels();

  private readonly actionsTpl = viewChild.required<TemplateRef<unknown>>('actionsTpl');

  readonly messages = computed<ChatMessage[]>(() => {
    const messages = toMessages(CHAT_COM_FERRAMENTAS);
    messages[messages.length - 1].actions = this.actionsTpl();
    return messages;
  });
...
export const WithActions: Story = {
  ...
  decorators: [moduleMetadata({ imports: [ActionsDemo] })],
  render: () => ({ template: '<nds-chat-thread-actions-demo />' }),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    const root = canvasElement.querySelector<HTMLElement>('[data-slot="chat-thread"]')!;
    const actions = root.querySelector<HTMLElement>('.nds-chat-message-actions')!;
    const copyButton = canvas.getByRole('button', { name: 'Copiar' });
    ...
    await step('Em repouso as ações estão invisíveis — mas NO percurso do teclado', async () => {
      ...
      // `toBeVisible`: para a testing-library, `opacity: 0` já é invisível — e é
      // justamente essa a diferença que este componente explora. O que
    ...
    await step('Ao receber foco, elas aparecem', async () => {
      ...
      // ainda perto de zero.
      //
      // Leitura PURA dentro do `waitFor`: condição que mexe no DOM reagenda a si
      // mesma por observador de mutação e pendura o arquivo inteiro.

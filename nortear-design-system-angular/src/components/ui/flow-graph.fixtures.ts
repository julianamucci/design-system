 ...
 *
 * Diferente da tela do computador, aqui NADA do andaime é marcação: o grafo não
 * tem espaço de quem consome, então tudo o que a demonstração precisa é dado, e
 * dado inteiro cabe no compartilhado. Por isso este arquivo não declara nenhum
 * componente de demonstração — só dado e o molde de story que os três arquivos
 * de story compartilham.
 *
...
import type { FlowGraphLabels } from './flow-graph';

const { t } = useTranslation(flowGraphTranslations as Record<string, unknown>);
...
/**
 * Um grafo largo, para quando ele não cabe na conversa.
 *
...
export interface FlowGraphStory {
  props: {
 ...
 *
 * MORA AQUI, e não em cada arquivo de story, porque os três arquivos ligariam o
 * mesmo template. Duas cópias do mesmo nome com corpos diferentes é o defeito
 * que `fixture_duplicada_entre_stories` existe para pegar: corrigir uma não
 * corrige a outra.
 *
 * O invólucro também é o que dá à asserção um elemento a que apontar quando não
 * há nó nenhum — a peça, ali, não desenha nada por dentro.
 */

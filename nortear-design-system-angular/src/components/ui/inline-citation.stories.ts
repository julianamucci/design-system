...
import { NdsInlineCitationDocs } from '@/components/docs/InlineCitationDocs';
...
//
// O control da citação é uma ESCOLHA entre os três formatos que a peça desenha
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(NdsInlineCitationDocs),
      ...
      // real: um componente que declara a citação, os rótulos e a frase.
  ...
  argTypes: {
    shape: {
      ...
      table: {
        type: { summary: "'full' | 'minimal' | 'unsafe'" },
...
export const Playground: Story = {
  ...
  // A frase é montada AQUI, e não pela peça: é a demonstração do contrato —
  // quem escreve a frase decide onde a afirmação precisa de apoio. Os pedaços
  // saem do módulo compartilhado, e nenhum termina em espaço.
  render: (args) => {
    ...
    return {
      props: {
        antes: parts[0],
        depois: parts[1]! + parts[2]!,
        citation,
        labels: inlineCitationLabels(1, citation),
        defaultOpen: args.defaultOpen,
      },
      // SEM ESPAÇO ANTES DA MARCA: as emendas encostam de propósito, e é assim
      // que ela não se separa da palavra que a antecede quando a linha quebra.
      template: `
        <p>{{ antes }}<span
            ndsInlineCitation
            [citation]="citation"
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    // `defaultOpen` abre DEPOIS da primeira renderização, e nesta stack a
    // renderização é agendada. Esperar por relógio antes de ler é o que mantém a
    // asserção honesta nos dois valores do control, e o que a mantém fora da
    // armadilha da espera por observador.
    ...
    await step('Nada aqui é região viva, e o estado aberto não é só cor', async () => {
      ...
      await expect(marker.getAttribute('aria-expanded')).toBe(

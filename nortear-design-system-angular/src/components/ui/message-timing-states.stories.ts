...
import {
  messageTimingEmptySource,
  messageTimingEveryCaseSource,
  messageTimingMeasuringSource,
  messageTimingPartialSource,
  messageTimingSettledSource,
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      source: { transform: messageTimingEveryCaseSource },
      description: {
        component:
...
const mount = (name: MessageTimingCase) => ({
  props: {
    stats: statsOf(name),
...
export const Measuring: Story = {
  parameters: {
    ...
    docs: { source: { transform: messageTimingMeasuringSource } },
  },
  render: () => mount('measuring'),
  play: async ({ canvasElement, step }) => {
    ...
    await step('O estado chega em PALAVRA, e ela abre a linha', async () => {
      ...
      await expect(state).toHaveClass('nds-badge');
    ...
    await step('E a palavra vem ANTES dos números que ela ressalva', async () => {
      ...
      const position = state.compareDocumentPosition(list);
...
export const Settled: Story = {
  parameters: {
    ...
    docs: { source: { transform: messageTimingSettledSource } },
  },
  render: () => mount('settled'),
  play: async ({ canvasElement, step }) => {
    const root = rootOf(canvasElement);
    const labels = messageTimingLabels();
    ...
    await step('E a palavra do estado não é montada', async () => {
      ...
      await expect(root.querySelector('.nds-badge')).toBeNull();
...
export const PartialMeasures: Story = {
  parameters: {
    ...
    docs: { source: { transform: messageTimingPartialSource } },
  },
  render: () => mount('partial'),
  play: async ({ canvasElement, step }) => {
    const root = rootOf(canvasElement);
...
export const NoMeasures: Story = {
  parameters: {
    ...
    docs: { source: { transform: messageTimingEmptySource } },
  },
  render: () => mount('none'),
  play: async ({ canvasElement, step }) => {
    const root = rootOf(canvasElement);

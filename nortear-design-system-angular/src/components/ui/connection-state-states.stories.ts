...
const actionOf = (line: HTMLElement) =>
  line.querySelector<HTMLButtonElement>('.nds-connection-state-action');
...
export const EveryState: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    const lines = [
      ...canvasElement.querySelectorAll<HTMLElement>('[data-slot="connection-state"]'),
    ];
    ...
    await step('E os rótulos de ação são DOIS nomes diferentes', async () => {
      ...
      const names = [
        ...canvasElement.querySelectorAll<HTMLElement>('.nds-connection-state-action'),
...
export const Disconnected: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E a ação oferece COMEÇAR uma, com nome próprio', async () => {
      const button = actionOf(line)!;
      await expect(button).toHaveAccessibleName(labels.action!.disconnected!);

 ...
 * `composer.fixtures.ts`, e a fila vive dentro do campo. Copiar o construtor
...
const { t } = useTranslation(attachmentTranslations as Record<string, unknown>);
...
export function one(state: AttachmentState, extra: Partial<Attachment> = {}): Attachment[] {
  return [{ id: 'a1', name: 'planta.pdf', size: SIZE_MB, state, ...extra }];
}

/**
 * A fila em que o último item não tem tamanho.
 *
 ...
 */
export function queueWithoutSize(): Attachment[] {

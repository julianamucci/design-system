...
import { NdsQuotaBanner } from './quota-banner';
...
// casos ela é AUTÔNOMA: recebe o controle sem saber o que ele faz, e divide a
// tela com a medição da janela sem que nenhuma das duas saiba da outra (§4.2 da
// guideline 17).
 ...
 * que a resposta significa fica do lado de fora. Nesta stack o controle chega
 * como `TemplateRef` — quem consome declara o `<ng-template>` e a faixa o
 * instancia —, e é a mesma divergência já registrada no cartão de autorização.
 *
 * O controle é o último tanto na marcação quanto no foco, e nenhum `order`
 * desfaz isso.
 */
export const WithAction: Story = {
  ...
  render: () => ({
    props: {
      ...
      horizon: renewalOf('warning'),
      labels: quotaBannerLabels(),
      actionLabel: quotaBannerActionLabel(),
    },
    template: `

...
import { NdsDocsCompositionsStory } from './DocsCompositionsStory';
 ...
 *
 * O preview é `TemplateRef`, que só nasce de `<ng-template>` recolhido por
 * `viewChild` e não cabe em `props` de story — daí a hospedeira
 * `DocsCompositionsStory`.
 */
...
const meta: Meta<DocsCompositionsArgs> = {
  ...
  tags: ['autodocs'],
  decorators: [moduleMetadata({ imports: [NdsDocsCompositionsStory] })],
  parameters: {
    layout: 'padded',
    docs: {
      description: {

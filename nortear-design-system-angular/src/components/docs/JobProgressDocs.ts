...
import { NdsJobProgress, type JobProgressLabels } from '@/components/ui/job-progress';
import {
  jobLabel,
  jobProgressLabels,
  JOB_COUNT,
...
import { NdsProgress, NdsProgressIndicator, NdsProgressTrack } from '@/components/ui/progress';
...
const { t, dict } = useTranslation(jobTranslations as Record<string, unknown>, {
  '*': {
    'props.table.onAction.name': 'action',
    'props.table.onAction.type': 'OutputEmitterRef<JobProgressIntent>',
  },
});
...
const INTERFACE_CODE = `// As quatro entradas e a saída da peça, no <div ndsJobProgress>
export class NdsJobProgress {
  ...
  // stack. Parar, retomar e repetir continuam sendo de quem consome.
...
@Component({
  selector: 'nds-job-progress-docs',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  imports: [
    NdsJobProgress, NdsButton, NdsSeparator,
  ...
  template: `
    <!-- A MESMA conta vai para as fotos, de propósito: quem decide o que a
         barra mostra em cada estado é o vocabulário compartilhado, e não esta
         página. -->

    <!-- O primeiro par é o do total desconhecido, que é o que a peça existe
         para não errar. -->
    <ng-template #tplDoDont1Do>
      <div class="nds-stack nds-w-full" data-spacing="lg">
        <div
          ...
          [label]="job"
          status="running"
          [count]="countWithoutTotal"
          [labels]="labels()"
        ></div>
      </div>
    </ng-template>
    <ng-template #tplDoDont1Dont>
      <!-- O contraexemplo é escrito À MÃO, e tem de ser: a peça trata total
           zero como ausência, então não há entrada que produza o erro. Aqui a
           barra determinada em zero vai no lugar da indeterminada, para que se
           veja a trilha vazia dizendo "acabou de começar" para algo que já
           andou muito. -->
      <div class="nds-stack nds-w-full" data-spacing="lg">
        <div class="nds-job-progress" data-status="running" aria-busy="true">
          <span class="nds-job-progress-label">{{ job }}</span>
          <span class="nds-job-progress-count" aria-hidden="true">{{ tallyWithoutTotal() }}</span>
          <div
            ndsProgress
          ...
          <span class="nds-job-progress-status">{{ runningWord() }}</span>
          <button
            ndsButton
            type="button"
            variant="outline"
        ...
        >
          <div class="nds-stack nds-w-full" data-spacing="lg">
            <!-- A legenda diz QUAL exemplo está desenhado — sem ela, quatro
                 peças empilhadas viram uma só, e o assunto da demonstração é
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.unknownTotal') }}</p>
              <div
                ndsJobProgress
                [label]="job"
                status="running"
...
export class NdsJobProgressDocs implements AfterViewInit, OnDestroy {
  protected readonly t = t;
  protected readonly tNav = tNav;
  protected readonly interfaceCode = INTERFACE_CODE;

  /** O que está sendo feito nas fotos. É também o nome acessível da barra. */
  protected readonly job = jobLabel();

  /** As duas contas de exemplo. Dado, e por isso as mesmas nos três idiomas. */
  protected readonly count = JOB_COUNT;
  protected readonly countWithoutTotal = JOB_COUNT_WITHOUT_TOTAL;

  /** Os rótulos são texto de interface, então acompanham a troca de idioma. */
  protected readonly labels = computed<JobProgressLabels>(() => {
    dict();
    return jobProgressLabels();
  });

  /**
   * O contraexemplo do segundo par: o estado que só existe em barra e cor.
   *
   * As palavras saem de `RUN_STATUSES`, e não de cinco linhas escritas à mão —
   * mesma razão da tabela de estados logo abaixo.
   */
  protected readonly wordless = computed<JobProgressLabels>(() => ({
    ...this.labels(),
  ...
  /** A conta, a palavra e o rótulo que o contraexemplo do primeiro par escreve à mão. */
  protected readonly tallyWithoutTotal = computed(() =>
    this.labels().countWithoutTotal.replace(
      '{done}',
      JOB_COUNT_WITHOUT_TOTAL.done.toLocaleString(),
    ),
  );
  protected readonly runningWord = computed(() => this.labels().status.running);
  protected readonly stopAction = computed(() => this.labels().action?.running ?? '');
  ...
  protected readonly uxWriting = computed(() => {
    ...
    return {
      ...
      items: ['jobLabel', 'count', 'countWithoutTotal', 'status', 'action'].map((key) => ({
  ...
  protected readonly propTables = computed(() => {
    ...
    return [
      {
        title: 'NdsJobProgress',
        cols,
        items: rowsOf(['label', 'status', 'count', 'labels', 'onAction']),
      },
      {
        title: 'JobProgressLabels',
        cols,
        items: rowsOf([
          'labelsStatus', 'labelsCount', 'labelsCountWithoutTotal', 'labelsAction',
        ]),
      },
    ];
  });

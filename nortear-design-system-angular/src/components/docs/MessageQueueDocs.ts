...
import { NdsMessageQueue, type MessageQueueLabels } from '@/components/ui/message-queue';
import {
  composerLabels,
  longQueue,
  queueLabels,
  sending,
  waiting,
} from '@/components/ui/message-queue.fixtures';
...
// existe.
const { t, dict } = useTranslation(queueTranslations as Record<string, unknown>, {
  '*': {
    'props.table.onWithdraw.name': 'withdraw',
    'props.table.onWithdraw.type': 'OutputEmitterRef<QueuedMessage>',
  },
});
...
const INTERFACE_CODE = `// A fila é peça própria, e fica ACIMA do campo.
export class NdsMessageQueue {
  ...
  // stack. Retirar de verdade continua sendo de quem envia.
  readonly withdraw = output<QueuedMessage>();
}

export interface MessageQueueLabels {
  list: string;                              // o nome da fila
  withdraw: string;                          // {text} vira o texto da mensagem
  state: Record<QueuedMessageState, string>; // a palavra de cada estado
}

// A mensagem vem de @shared/primitives/chat-protocol:
export interface QueuedMessage {
  id?: string;                  // endereço, quando duas trazem o mesmo texto
  text: string;                 // o que foi escrito
  state: 'waiting' | 'sending'; // a que espera se retira; a que já vai, não
}`;
...
@Component({
  selector: 'nds-message-queue-docs',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  imports: [
    NdsComposer, NdsMessageQueue, NdsSeparator,
  ...
  template: `
    <!-- O primeiro par é a MESMA fila: o que muda é se cada botão diz qual
         mensagem retira. -->
    <ng-template #tplDoDont1Do>
      <nds-message-queue [labels]="queueText()" [messages]="waitingQueue" />
    </ng-template>
    <ng-template #tplDoDont1Dont>
      <!-- O contraexemplo: o rótulo do botão sem o texto da mensagem, e os três
           passam a se chamar a mesma coisa. -->
      <nds-message-queue [labels]="withdrawWithoutText()" [messages]="waitingQueue" />
    </ng-template>

    <ng-template #tplDoDont2Do>
      <nds-message-queue [labels]="queueText()" [messages]="sendingQueue" />
    </ng-template>
    <ng-template #tplDoDont2Dont>
      <!-- O contraexemplo: a mesma palavra nos dois estados, e a única
           diferença que sobra é a transparência. -->
      <nds-message-queue [labels]="stateWithoutWords()" [messages]="sendingQueue" />
    </ng-template>
    ...
    >
      ...
      <ng-container docsMain>
        ...
        >
          <div class="nds-stack nds-w-full" data-spacing="lg">
            <!-- A legenda diz QUAL exemplo está desenhado — sem ela, quatro
                 filas empilhadas viram uma só, e o assunto da demonstração é
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.sending') }}</p>
              <nds-message-queue [labels]="queueText()" [messages]="sendingQueue" />
            </div>
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.long') }}</p>
              <nds-message-queue [labels]="queueText()" [messages]="longestQueue" />
            </div>

            <div ndsSeparator></div>

            <!-- A fila é peça própria, e o campo não sabe que ela existe: quem
                 consome empilha as duas, nesta ordem. -->
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.aboveField') }}</p>
              <div class="nds-stack nds-w-full" data-spacing="xs">
                <nds-message-queue [labels]="queueText()" [messages]="sendingQueue" />
        ...
        <nds-docs-import
          ...
          [secondaryCode]="t('import.aboveFieldCode')"
          componentSlug="message-queue"
          language="html"
        />
...
export class NdsMessageQueueDocs implements AfterViewInit, OnDestroy {
  protected readonly t = t;
  protected readonly tNav = tNav;
  protected readonly interfaceCode = INTERFACE_CODE;

  protected readonly waitingQueue = WAITING;
  protected readonly sendingQueue = SENDING;
  protected readonly longestQueue = LONG;
  ...
  protected readonly queueText = computed<MessageQueueLabels>(() => {
    dict();
    return queueLabels();
  });

  /** O contraexemplo do primeiro par: o botão que não diz qual mensagem retira. */
  protected readonly withdrawWithoutText = computed<MessageQueueLabels>(() => ({
    ...this.queueText(),
    withdraw: t('usage.uxWriting.table.withdraw.bad'),
  }));

  /** O contraexemplo do segundo par: a mesma palavra nos dois estados. */
  protected readonly stateWithoutWords = computed<MessageQueueLabels>(() => ({
    ...this.queueText(),
    state: {
      waiting: t('labels.state.waiting'),
      sending: t('labels.state.waiting'),
    },
  }));
  ...
  protected readonly uxWriting = computed(() => {
    ...
    return {
      ...
      items: ['position', 'text', 'state', 'withdraw', 'list'].map((key) => ({
  ...
  protected readonly stateItems = computed(() => {
    ...
    return ['waiting', 'sending', 'empty'].map((k) => ({
  ...
  protected readonly propTables = computed(() => {
    ...
    return [
      {
        title: 'NdsMessageQueue',
        cols,
        items: rowsOf(['messages', 'labels', 'onWithdraw']),
      },
      {
        title: 'QueuedMessage',
        cols,
        items: rowsOf(['text', 'state', 'id']),
      },
    ];
  });
  ...
  protected readonly tokenItems = computed(() => {
    dict();
    return [
      'muted', 'radiusSm', 'textLabel', 'mutedForeground',
      'spacing1', 'spacing2', 'spacing4', 'spacing6',
    ].map((k) => ({

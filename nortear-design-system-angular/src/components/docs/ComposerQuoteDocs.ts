...
import { attachmentLabels, queue } from '@/components/ui/composer-attachments.fixtures';
import type { ComposerAttachmentLabels } from '@/components/ui/composer-attachments';
import {
  longQuote,
  quoteLabels,
  shortQuote,
} from '@/components/ui/composer-quote.fixtures';
import type {
  ComposerQuote,
  ComposerQuoteLabels,
...
// existe.
const { t, dict } = useTranslation(quoteTranslations as Record<string, unknown>, {
  '*': {
    'props.table.onDismissQuote.name': 'dismissQuote',
    'props.table.onDismissQuote.type': 'OutputEmitterRef<ComposerQuote>',
  },
});
...
const INTERFACE_CODE = `// As três entradas da citação, no <nds-composer>
export class NdsComposer {
  ...
  // stack. Tirar a citação continua sendo de quem consome.
...
/** As citações das demonstrações. Dado de exemplo não muda com o idioma. */
const QUOTE_SHORT = shortQuote();
const QUOTE_LONG = longQuote();
const ATTACHMENT_PAIR = queue().slice(0, 2);

/**
 * O contraexemplo do primeiro par: o trecho já cortado em código.
 *
 * Na tela as duas molduras parecem iguais — a diferença só existe para quem
 * ouve, e para a busca do navegador.
 */
const QUOTE_TRUNCATED: ComposerQuote = {
  ...QUOTE_LONG,
  excerpt: `${QUOTE_LONG.excerpt.slice(0, 60)}…`,
};

@Component({
  selector: 'nds-composer-quote-docs',
  standalone: true,
  ...
  template: `
    <!-- O primeiro par é a MESMA citação longa: o que muda é se o texto inteiro
         chegou ao documento ou foi cortado antes. -->
    <ng-template #tplDoDont1Do>
      <nds-composer [labels]="labels()" [quoteLabels]="citationLabels()" [quote]="long" />
    </ng-template>
    <ng-template #tplDoDont1Dont>
      <nds-composer [labels]="labels()" [quoteLabels]="citationLabels()" [quote]="truncated" />
    </ng-template>

    <ng-template #tplDoDont2Do>
      <nds-composer
        [labels]="labels()"
        [quoteLabels]="citationLabels()"
        [quote]="short"
        [attachmentLabels]="fileLabels()"
        [attachments]="pair"
      />
    </ng-template>
    <ng-template #tplDoDont2Dont>
      <!-- O contraexemplo: o botão SEM o nome de quem escreveu, ao lado dos
           botões de anexo — três "×" indistinguíveis por audição. O texto sai do
           próprio rótulo traduzido, com o lugar do nome removido, para o
           contraexemplo continuar nos três idiomas. -->
      <nds-composer
        [labels]="labels()"
        [quoteLabels]="dismissWithoutAuthor()"
        [quote]="short"
        [attachmentLabels]="fileLabels()"
        [attachments]="pair"
      />
    ...
    >
      ...
      <ng-container docsMain>
        ...
        >
          <div class="nds-stack nds-w-full" data-spacing="lg">
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.short') }}</p>
              <nds-composer [labels]="labels()" [quoteLabels]="citationLabels()" [quote]="short" />
            </div>
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.withAttachments') }}</p>
              <nds-composer
                [labels]="labels()"
                [quoteLabels]="citationLabels()"
                [quote]="short"
            ...
            <div class="nds-stack nds-w-full" data-spacing="xs">
              <p class="nds-text-caption nds-text-muted-foreground">{{ t('demonstration.labels.absent') }}</p>
        ...
        <nds-docs-import
          ...
          [secondaryCode]="t('import.fromThreadCode')"
          componentSlug="composer-quote"
          language="html"
        />
...
export class NdsComposerQuoteDocs implements AfterViewInit, OnDestroy {
  protected readonly t = t;
  protected readonly tNav = tNav;
  protected readonly interfaceCode = INTERFACE_CODE;

  protected readonly short = QUOTE_SHORT;
  protected readonly long = QUOTE_LONG;
  protected readonly truncated = QUOTE_TRUNCATED;
  protected readonly pair = ATTACHMENT_PAIR;
  ...
  protected readonly citationLabels = computed<ComposerQuoteLabels>(() => {
    dict();
    return quoteLabels();
  });

  protected readonly fileLabels = computed<ComposerAttachmentLabels>(() => {
    dict();
    return attachmentLabels();
  });

  /** O contraexemplo do segundo par: o botão que não diz o que dispensa. */
  protected readonly dismissWithoutAuthor = computed<ComposerQuoteLabels>(() => ({
    ...this.citationLabels(),
    dismiss: this.citationLabels().dismiss.replace('{author}', '').trim(),
  }));
  ...
  protected readonly uxWriting = computed(() => {
    ...
    return {
      ...
      items: ['author', 'prefix', 'dismiss', 'excerpt'].map((key) => ({
  ...
  protected readonly stateItems = computed(() => {
    ...
    return ['absent', 'short', 'long'].map((k) => ({
    ...
    return [
      {
        title: 'NdsComposer',
        cols,
        items: rowsOf(['quote', 'quoteLabels', 'onDismissQuote']),
      },
      {
        title: 'ComposerQuote',
        cols,
        items: rowsOf(['quoteId', 'author', 'role', 'excerpt']),
      },
    ];
  });
  ...
  protected readonly tokenItems = computed(() => {
    dict();
    return [
      'primary', 'muted', 'radiusSm', 'foreground', 'mutedForeground', 'textLabel',
    ].map((k) => ({

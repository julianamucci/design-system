...
- **Não commite com agente escrevendo na árvore.** Mesma campanha, mesma raiz. Rodei os cinco builds, vi verde, commitei — e o commit levou junto um `failureCallsPlantedDefect` que uma agente tinha acabado de plantar para provar que o portão tinha dentes. Aquele commit não compilava. A frase que resume: **commit tirado durante a medição de outra agente captura a medição, não o trabalho.** Ou se espera a árvore assentar, ou se commita `-- <caminhos>` que só você tocou.
  ...
  **Corolário, medido em 2026-09-08: plantar defeito para provar portão QUEBRA A ÁRVORE PARA TODO MUNDO enquanto dura.** Uma agente plantou um `[structureCode]="defeitoPlantado"` para verificar que a regra tinha dentes — procedimento correto, e ela desfez em seguida. Só que **duas outras agentes mediram dentro daquela janela**, as duas relataram "o build do Angular está vermelho" como achado próprio, e uma delas montou um `tsconfig` temporário para contornar. Ninguém errou; o custo foi de coordenação.
  ...
  Isto está em tensão direta com a regra de provar todo portão replantando o defeito, e as duas continuam valendo. O que muda é a forma: **planta, mede e restaura na MESMA chamada de ferramenta**, sem nada entre uma coisa e outra — não em passos separados, não com leitura no meio. E ao receber de uma agente um relato de árvore quebrada em arquivo que não é o escopo dela, **confira antes de reagir**: `grep` na árvore, `git status` e o conteúdo do commit. Vermelho transitório de defeito plantado some sozinho; reagir a ele é reagir ao passado, e foi o que quase me fez caçar um defeito que não existia mais.

...
**Building a conversational component**: follow `docs/shared/guidelines/17-componentes-conversacionais.md` — the port of the assistant-ui Elements catalogue (120 pieces, MIT) into the five stacks. It carries the triage — part of the 120 are compositions of primitives that already exist, the rest are new and grouped into families with one sheet each. **The split is not quoted here on purpose**: building corrects the reading, entries keep crossing from new to composition — one whole family has already dissolved that way — and every number written outside §5.1/§5.2 went stale within a couple of corrections without anyone noticing. Those two headers carry the current counts; §5.3 records each move and why. It also carries the runtime-free rule that is what makes runtime-bound React elements portable at all, the shared foundation that has to exist before the SECOND piece, and the ten accessibility rules the whole family inherits. `chat-thread` is the first member and the model for the rest.
...
- **Componente com PRD não tem seção de catálogo na guideline** — `docs/shared/prd/<slug>.md` descreve o que o componente É hoje (contrato, decisões com data e medição, tokens, peças das cinco stacks); a guideline fica com a regra da CATEGORIA, que atravessa componentes e não tem casa no PRD. Os dois são eixos transpostos da mesma matriz: PRD é um componente × cinco stacks, guideline é uma stack × muitos componentes. Enquanto os dois descrevem o mesmo componente há duas fontes, e nesta casa a segunda é a que deixa de ser corrigida — medido em 2026-09-07, quando as cinco cópias da guideline de overlay cobriam a categoria com 6 a 13 seções cada e o vanilla, que é a referência, documentava 6 dos 10. Gate: `catalogo_duplicado_com_prd`. A migração é por componente, quando o PRD nasce — nunca antes, porque hoje são 9 PRDs para 85 componentes.
...
- **Agente que delega em segundo plano é SUSPENSA, e não volta para conferir.** Medido três vezes na campanha da guideline 17, sempre igual: a agente dispara as quatro portas, relata "as portas estão rodando em paralelo, vou verificar quando voltarem", e o harness a encerra ali — porque ela não tem mais nada a fazer. A verificação prometida nunca acontece, e as filhas reportam soltas, cada uma sabendo só da sua stack. Instruir "não encerre com as portas pela metade" NÃO resolve: não é desatenção, é o mecanismo. Ou a agente delega em **primeiro plano** (bloqueante), ou quem orquestra assume a conferência das cinco.
  ...
  **E antes de lançar reforço, LISTE as agentes vivas.** O corolário custou caro: vendo uma medição da árvore com três stacks incompletas, lancei duas agentes de reforço — e as filhas da primeira fecharam aquelas stacks segundos depois. Duas agentes passaram a escrever os mesmos sete arquivos do Angular: uma plantou defeito de teste que reprovou o build da outra, uma apagou o override da outra, e uma sobrescreveu destrutivamente um arquivo de story que estava fora do git e não voltou. Medição de árvore com agente viva dentro é estado em movimento, e reagir a ela é reagir ao passado.

- **Não commite com agente escrevendo na árvore.** Mesma campanha, mesma raiz. Rodei os cinco builds, vi verde, commitei — e o commit levou junto um `failureCallsPlantedDefect` que uma agente tinha acabado de plantar para provar que o portão tinha dentes. Aquele commit não compilava. A frase que resume: **commit tirado durante a medição de outra agente captura a medição, não o trabalho.** Ou se espera a árvore assentar, ou se commita `-- <caminhos>` que só você tocou.

  **Corolário, medido em 2026-09-08: plantar defeito para provar portão QUEBRA A ÁRVORE PARA TODO MUNDO enquanto dura.** Uma agente plantou um `[structureCode]="defeitoPlantado"` para verificar que a regra tinha dentes — procedimento correto, e ela desfez em seguida. Só que **duas outras agentes mediram dentro daquela janela**, as duas relataram "o build do Angular está vermelho" como achado próprio, e uma delas montou um `tsconfig` temporário para contornar. Ninguém errou; o custo foi de coordenação.

  Isto está em tensão direta com a regra de provar todo portão replantando o defeito, e as duas continuam valendo. O que muda é a forma: **planta, mede e restaura na MESMA chamada de ferramenta**, sem nada entre uma coisa e outra — não em passos separados, não com leitura no meio.

  **E dentro dessa chamada, encadeie com `;`, nunca com `&&`.** Medido em 2026-09-08: o `audit.mjs` sai com código NÃO-ZERO quando acha violação — que é justamente o que se quer ao plantar —, então `plantar && medir && restaurar` **pula a restauração** e o defeito fica na árvore. Vale para todo portão que reprova por código de saída, que é a maioria: `npm run build`, `eslint`, `vitest`. A forma que mede E restaura é `plantar; medir; restaurar`, com a conferência do arquivo restaurado depois. E ao receber de uma agente um relato de árvore quebrada em arquivo que não é o escopo dela, **confira antes de reagir**: `grep` na árvore, `git status` e o conteúdo do commit. Vermelho transitório de defeito plantado some sozinho; reagir a ele é reagir ao passado, e foi o que quase me fez caçar um defeito que não existia mais.
...
- **Portão só vale depois de saber o que ele cobre.** Declarei "build limpo nas cinco" com base no `npm run build` sem saber o que ele alcançava. Três arquivos do Svelte não compilavam (`import "./rotulos"` órfão, dois `Identifier already declared`) e a dona descobriu pelo CI, não por mim.
  ...
  **E o `svelte-check` não vê expressão de template que é STRING LITERAL.** Medido em 2026-09-08: uma gravação perdeu o `$tStore` no escape do shell e plantou `{('demonstration.labels.cancel')}` em **35 pontos** de uma docs page. O `svelte-check` fechou **0 ERRORS** — `{('string')}` é Svelte perfeitamente válido —, e a página renderizaria o NOME DA CHAVE no lugar do texto, nos três idiomas. Quem viu foi o `eslint`, pela regra `svelte/no-useless-mustaches`. Não é caso de threshold: é o compilador não tendo o que reprovar. Onde a mudança é de fiação de i18n no Svelte, o `eslint` não é complemento do build — é o portão que enxerga.
...
- **Bump de dependência invalida patch, e o patch não avisa.** `nortear-design-system-svelte/patches/` e `nortear-design-system-vue/patches/` existem e o `postinstall` roda `patch-package`. O nome do arquivo carrega a versão (`svelte-sonner+1.2.0.patch`): subiu a versão, o patch para de aplicar. Depois de qualquer atualização, rode `npx patch-package` e confirme a linha `pacote@versão ✔`. Se o upstream tiver corrigido, apague o patch; se não, refaça com `npx patch-package <pacote>` e remova o arquivo antigo.
  ...
  **E `npm update` NÃO roda o `postinstall`** — medido em 2026-09-03, subindo o Storybook para 10.6.0. As cinco stacks fecharam em exit 0 sem uma linha sobre patch na saída, e o `reka-ui` do Vue estava DESAPLICADO: tinha subido de 2.10.3 para 2.10.4 dentro do `^2.10.1`, o npm reinstalou o pacote limpo, e nada reaplicou o patch. Ler o código de saída teria declarado sucesso com o comportamento revertido — aqui, item de menu desabilitado voltando a ser inalcançável pelo teclado. O `✔` do `patch-package` não é conferência opcional depois do update: é a única coisa que prova que o patch está de pé.

  **Ao decidir o que sobe, compare `current` com `wanted`, nunca com `latest`.** No mesmo dia o `npm outdated` mostrou Angular `22.1.3 → 21.2.22`, `v-calendar` `3.1.2 → 2.4.2` e `vaul-svelte` `1.0.0-next.7 → 0.3.2`, e os três são o projeto estando À FRENTE da tag `latest`, não atualizações pendentes. Tratar `latest` como alvo rebaixa três pacotes de uma vez. Quem manda é o `wanted`, que é o que o range declarado de fato permite.
...
- **No Angular, host binding de diretiva APAGA o atributo estático do template, e `[attr.x]` no template também perde.** Medido em 2026-09-01, em 19 pontos de uma vez: `NdsButton` e `NdsBadge` ligam `data-slot` a um valor fixo, então toda peça que se nomeava (`data-slot="composer-voice-toggle"`) renderizava `"button"`. Nenhum compilador alcança isso — só a suíte de navegador —, e várias stories já contornavam consultando por classe, com o motivo no comentário.

  **A correção óbvia não funciona**: `[attr.data-slot]="'…'"` no template perde para o host binding do mesmo jeito. O conserto vai nas duas diretivas, lendo o atributo estático uma vez na construção — 19 pontos, 2 arquivos. É a mesma raiz da regra de estilizar por CLASSE e nunca por `[data-slot]`: no Angular o atributo é disputado, e quem escreve por último vence.

- **Arquivo com `(0 test)` na suíte de navegador não é arquivo vazio — é arquivo que MORREU.** Medido em 2026-09-01, na primeira rodada autorizada do Angular: o `preview.ts` importa `storybook/internal/core-events` (o ouvinte de canal da barra de temas), o otimizador do Vite só descobriu esse subcaminho **depois** de a suíte começar, o pré-empacotamento mudou no meio e a página **recarregou**. Os onze arquivos em voo naquele instante morreram com `Vitest failed to find the current suite` e foram contados como `(0 test)` — e um deles era o `docs-smoke.stories.ts`, ou seja **as 98 docs pages inteiras**.

  O que torna isto pior que uma falha: a rodada só fechou vermelha por outro motivo. **Com os outros vinte testes verdes, ela teria fechado VERDE sem medir a fumaça** — a suíte reportando sucesso sem ter rodado um terço de si mesma. Some junto o sintoma: o Angular era a única das cinco sem `optimizeDeps.exclude: ['vitest/browser']`.

  Regra: ao ler saída de suíte de navegador, **conte arquivos, não só falhas**, e trate `(0 test)` como vermelho até prova em contrário. Import descoberto tarde pelo otimizador se resolve declarando-o em `optimizeDeps.include`; módulo virtual do plugin se resolve em `exclude`.

- **Erro de SINTAXE aborta o `ngc` antes da fase semântica, e a rodada parece ter medido o que não mediu.** Medido em 2026-09-01: cinco `TS1005` por crase dentro de um literal `template:` derrubaram o `npm run build` do Angular — e quando ele foi consertado e a rodada refeita, foi a **primeira vez** que os `input()` com alias e os host bindings daquele arquivo passaram por type-check. A rodada anterior não os aprovou; ela nem chegou neles.

  A leitura que isso corrige é sutil e vale para as cinco stacks, porque é do compilador e não do Angular: **build vermelho por sintaxe não é "quase verde com um detalhe"** — é uma rodada em que tudo que depende de análise semântica ficou sem medição. Consertar o `TS1005` e ver exit 0 está certo; o erro é lembrar da rodada anterior como se ela tivesse coberto o arquivo. Ao ler saída de build, separe `TS1002`–`TS1999` (sintaxe, interrompe) de `TS2xxx` (semântica, acumula): se apareceu o primeiro grupo, o segundo não foi procurado.

- **Portão que casa palavra solta mede PROSA.** Mesma rodada: a guarda nova de nome acessível procurava `group` e `region` no arquivo, e reprovou num COMENTÁRIO que explicava por que `region` tinha sido recusado. As duas expressões passaram a exigir `role` na mesma linha do valor. Corolário do que já está escrito sobre portão sem dentes: **portão alterado é portão não provado** — replante o defeito depois de apertar o regex, porque foi o aperto que virou a coisa nova.

- **Exclusão de portão se DECLARA, com a premissa verificada.** A mesma guarda excluía dois pares stack×componente por comentário — a forma exata do `source-snippets.test.ts`, que encolheu em silêncio quando 28 exports saíram da varredura. Agora cada exclusão é nomeada, um caso cobra que todo par tenha caso ou exceção declarada, e outro **checa a premissa de cada exceção**: se o painel do `resizable` no Vue ou no Svelte ganhar um `tabindex`, a exceção vence e o portão reprova, em vez de continuar quieto.

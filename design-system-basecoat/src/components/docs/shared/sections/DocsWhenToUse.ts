...
export function createDocsWhenToUse(props: DocsWhenToUseProps): HTMLElement {
  ...
  // ── Guidelines ───────────────────────────────────────────────────────────────
  const guidelinesBlock = createCard({ className: 'bg-muted/50 border border-border/40 p-4 space-y-3' });
  ...
  if (props.uxWriting) {
    ...
    uxHeaderRow.appendChild(createTableHead(ux.cols.element, 'p-3 font-semibold'));
    if (ux.cols.rules) {
      uxHeaderRow.appendChild(createTableHead(ux.cols.rules, 'p-3 font-semibold'));
    }

    const doHead = document.createElement('th');
    doHead.className = 'text-muted-foreground p-3 font-semibold text-green-700';
    ...
    ux.items.forEach(row => {
      ...
      tr.appendChild(createTableCell(row.element, 'p-3 font-medium'));
      if (ux.cols.rules) {
        tr.appendChild(createTableCell(row.rules ?? '', 'p-3 text-muted-foreground'));
      }
      tr.appendChild(createTableCell(row.do, 'p-3 font-medium text-green-600'));

...
type EstadosArgs = Record<string, never>;
...
export const ItemDisabled: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Item individual desabilitado. O trigger fica inativo enquanto os demais itens permanecem funcionais.',
      },
    },
  },
};
...
export const DefaultOpen: Story = {
  ...
  render: () => createAccordion({
    ...
    defaultValue: 'item-2',
    items: [
      { value: 'item-1', trigger: 'Primeira seção', content: 'Conteúdo da primeira seção.' },
      { value: 'item-2', trigger: 'Segunda seção (aberta por padrão)', content: 'Este item está aberto por padrão via defaultValue.' },
      { value: 'item-3', trigger: 'Terceira seção', content: 'Conteúdo da terceira seção.' },
    ],
    class: 'w-full max-w-md',
  }),
  play: async ({ canvasElement, step }) => {

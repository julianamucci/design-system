...
- [ ] **O `toggle-group` do Vanilla ganhou três capacidades que as outras quatro stacks não têm.** (Aberto em 2026-08-27, ao montar a barra do protótipo de editor.) Vanilla é a referência, então a divergência é dívida de porte, não decisão:
  ...
  |---|---|
  | `items[].children` aceita ELEMENTO, não só `string` | item só de ícone é o caso mais comum do grupo; antes o ícone era injetado depois de construir, casando ícone com posição — cada story de composição carrega uma cópia desse laço, e a fixture `injectIcons` só existe por isso |
  | `role: 'toolbar' | 'group'` | grupo ANINHADO numa barra maior: `toolbar` dentro de `toolbar` é papel dentro do mesmo papel, e duas rovings disputando o mesmo Tab prendem quem navega no primeiro grupo. Em `group` o grupo abre mão do teclado |
...
- [ ] **`createToggle` (solto) continua sem setter público de `pressed`.** (Aberto em 2026-08-27; o grupo foi resolvido, o toggle avulso não.) A fábrica guarda `pressed` num fecho e só o troca no próprio clique, então o toggle avulso não serve a nada que espelhe estado externo. Dentro do grupo isso não morde — o grupo é a autoridade e repinta os botões a cada mudança —, mas quem usar `createToggle` direto numa barra dirigida por fora esbarra na mesma parede.
  ...
  O que destrava: um `setPressed(next: boolean)` no nó devolvido, na mesma forma do `setValue` do grupo.
...
- [x] **Falta a story que exercita o nível do título.** **Resolvido em 2026-09-09**, no dia seguinte ao de abertura: a story `HeadingH3` nasceu nas **cinco** stacks, nos **quatro** painéis — 20 no total, todas em `<slug>-variants.stories.*`, com o painel aberto na montagem e o título pedido em `h3`. O grupo da barra lateral é o mesmo nas cinco (o nível é escolha de quem MONTA o componente, então é forma, e forma mora em `-variants`). Cada uma declara `covers` no item de `testes.accessibility` que já falava do `aria-labelledby` (`item3` no dialog e no drawer, `item4` no sheet, `item2` no alert-dialog), tem construtor próprio no painel Code (`<camelSlug>HeadingH3Source`) com caso no `<slug>.source.test.ts` — o do alert-dialog do Angular teve de ser CRIADO, não existia —, e afirma cinco coisas na mesma ordem nas cinco: o `aria-labelledby` existe, resolve num elemento, esse elemento é `H3`, carrega a classe do título e é dele que sai o nome acessível do painel.

  **Os dois defeitos que ela existe para pegar foram plantados e reprovaram, nas cinco**: trocar a tag mantendo o vínculo (`expected 'H2' to be 'H3'`) e manter a tag rompendo o vínculo (`expected null not to be null`, ou `expected null to be truthy` onde a lib deixa de escrever o atributo). Portão: build, lint, `audit.mjs` nos quatro slugs, projeto `unit` inteiro e a suíte de navegador dos quatro arquivos de story — **4 arquivos / 27 testes por stack**, sem `(0 test)`.

  **A story corrigiu a premissa do item anterior, e essa é a parte que importa.** A entrada de 2026-09-08 dizia que as quatro stacks com lib já aceitavam qualquer nível "pelo mecanismo da própria lib", citando `level` no svelte. **Medido em 2026-09-09: `level` sozinho NÃO troca a tag** — o título daquela lib renderiza `<div role="heading">` e o `level` só alimenta o `aria-level`; o mesmo componente serve os quatro painéis, então não havia atalho por slug. A entrega vai pelo snippet `child`, que é a delegação de elemento daquela lib — irmã do `render`, do `as` e do `asChild` —, com `level` junto para a tag e o ARIA concordarem. A tabela de mecanismos nos quatro PRDs foi corrigida. A afirmação errada foi escrita no dia em que o item anterior fechou e sobreviveu porque nenhuma superfície a exercitava — é exatamente a demonstração do que este item afirmava.

  **Duas armadilhas de medição encontradas no caminho, ambas do tipo "verde sem medir":**

  · `-t "Heading H3"` **não casa nada e fecha exit 0.** O Storybook separa a letra do dígito ao nomear o teste, e a story se chama **"Heading H 3"**. Duas agentes plantaram defeito, viram `8 skipped` e teriam concluído que o portão tinha dentes. Filtro seguro: `-t Heading`, ou o arquivo inteiro sem filtro.

  · **`story_name_not_english` reprova `HeadingLevel`** — `SUFIXO_PT` casa `vel` no fim de `Level`, e a regra 1 de `auditSidebarVocab` é a única da família que não consulta lista de dispensa. Daí o nome `HeadingH3`. É a mesma família de "portão que casa palavra solta mede PROSA": `Level` é inglês. Fica registrado para quem for apertar a heurística — hoje o único caminho de exceção é renomear.

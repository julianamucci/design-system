...
//
// Nenhuma linha sobrescrita aqui, e é o esperado: `stats`, `streaming` e
// `labels` têm o mesmo nome em todas as stacks, e a peça é só leitura — sem
// retorno para avisar e sem ação para oferecer, não há evento, que é o ponto em
// que a API costuma divergir. A única divergência é a AUSÊNCIA de uma prop
// `class`, e ela não tem linha na tabela para sobrescrever.

const { t: tNav } = useTranslation(uiTranslations);
const { t: tContent, locale } = useTranslation(messageTimingTranslations);

const labels = useMessageTimingLabels();
const stats = useMessageTimingStats();

// O contraexemplo do PRIMEIRO par não precisa de nada aqui: são as mesmas
// medidas parciais do lado certo, com a ressalva retirada no template. A
// medição ainda corre e nada diz isso — o total parcial se lê como final, e
// nada na tela parece errado, que é o pior defeito possível aqui.

/**
 * O contraexemplo do segundo par: as medidas escritas à mão.
 *
 * Ponto decimal e unidade colada, iguais em todos os idiomas — que é exatamente
 * o que a peça não tem como consertar, porque ela recebe a cadeia pronta.
 */
const handWrittenStats = computed(() => [
  { label: tContent('labels.stats.firstToken'), value: '420ms' },
  { label: tContent('labels.stats.total'), value: '1.24s' },
]);
...
const stateItems = computed(() =>
  ['settled', 'measuring', 'partial', 'empty'].map(k => ({
...
const propsTables = computed(() => [
  {
    ...
    items: propsRows(['stats', 'streaming', 'labels']),
  },
  {
    title: 'MessageTimingStat',
    cols: propsCols.value,
    items: propsRows(['statLabel', 'statValue']),
  },
  {
    title: 'MessageTimingLabels',
    cols: propsCols.value,
    items: propsRows(['labelsTitle', 'labelsMeasuring']),
  },
]);
...
<template>
  ...
  >
    ...
    <!--
      ...
      números empilhadas viram uma só, e o assunto da demonstração é justamente
      ...
      >
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.settled') }}
          </p>
          <MessageTiming
            :stats="stats.settled"
            :labels="labels"
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.measuring') }}
          </p>
          <MessageTiming
            :stats="stats.measuring"
            :streaming="true"
            :labels="labels"
          />
        </div>
        ...
        <!-- O quarto exemplo é o mesmo dado do primeiro num container estreito:
             é assim que se vê que a forma é do container, e não um argumento da
             peça. -->
        <div
          class="nds-stack nds-w-full"
          data-spacing="xs"
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.tight') }}
          </p>
    ...
    <DocsDoDont
      ...
      :pairs="doDontPairs"
    >
      <!-- As MESMAS medidas parciais nos dois lados: o que muda é a ressalva
           chegar. -->
      ...
      <!-- E aqui as MESMAS duas medidas: o que muda é se elas chegaram escritas
           no idioma de quem lê, ou coladas à mão. -->

...
//
// As DUAS linhas sobrescritas são as de `railStart`, e por um motivo de API: o
// conteúdo compartilhado descreve o trilho na API da referência, onde os
// controles chegam como lista de elementos. Aqui eles chegam como slot com
// escopo, e o nome muda junto com o tipo. Deixar a tabela anunciando uma prop
// que não existe seria documentar outra stack.
...
const wordlessStop = computed<ComposerLabels>(() => ({
  ...labels.value,
  stop: labels.value.submit,
}));

/** O contraexemplo do segundo par: o limite que fica só para os olhos. */
const silentLimit = computed<ComposerLabels>(() => ({ ...labels.value, limit: '' }));
...
const variantItems = computed(() =>
  SUBMIT_MODES.map(mode => ({
    name: mode,
    description: tContent(`variants.items.${mode}.description`),
    code: tContent(`variants.items.${mode}.code`),
  })),
);

const stateItems = computed(() =>
  ['empty', 'filled', 'running', 'nearLimit', 'disabled'].map(k => ({
...
const interfaceCode = `// Composer.vue
...
// O TEXTO é um vínculo, e não um método: \`v-model:value\`. Sem ninguém do lado
// de fora ele é estado local; com o vínculo, é por ali que um rascunho volta.
// O estado de geração é a prop \`running\` — quem sabe é quem consome.

// O que sai do componente:
//   @submit — o texto pedido, sem espaços nas pontas
//   @stop   — alguém pediu para interromper a geração

// A marcação que quem consome fornece entra por SLOT:
//   <template #railStart> — os controles do início do trilho`;
    ...
    >
      ...
      >
        ...
        >
          ...
          <Composer
            :labels="labels"
            :value="tContent('labels.placeholder')"
            running
          />
        </div>
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.limit') }}
          </p>
          <Composer
            :labels="labels"
            :max-length="DEMO_LIMIT"
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.rail') }}
          </p>
          <Composer :labels="labels">
            <template #railStart>
              <Button

...
//
// A ÚNICA linha sobrescrita é o NOME do aviso de retirada. O conteúdo
// compartilhado o descreve como callback, que é a forma da raiz imperativa de
// referência; aqui ele é um evento, e quem consome o escuta. Divergência de API
// de framework se registra, não se "alinha".

const { t: tNav } = useTranslation(uiTranslations);
const { t: tContent, locale } = useTranslation(queueTranslations, {
  '*': { 'props.table.onWithdraw.name': '@withdraw' },
});

const queueLabels = useQueueLabels();
// `computed` e não o objeto pronto: o `setup` roda uma vez, e a barra de idioma
// do Storybook troca o idioma com a página montada.
const fieldLabels = computed(composerLabels);

const waitingQueue = waiting();
const sendingQueue = sending();
const longWaitingQueue = longQueue();

/**
 * O contraexemplo do primeiro par: o rótulo do botão sem o texto da mensagem.
 *
 * Os três passam a se chamar a mesma coisa, e uma fila de três botões iguais é
 * um botão só para quem navega por audição.
 */
const namelessWithdraw = computed<MessageQueueLabels>(() => ({
  ...queueLabels.value,
  withdraw: tContent('usage.uxWriting.table.withdraw.bad'),
}));

/**
 * O contraexemplo do segundo par: a mesma palavra nos dois estados.
 *
 * A única diferença que sobra é a transparência, e transparência sozinha não
 * descreve estado (WCAG 1.4.1).
 */
const sameStateWord = computed<MessageQueueLabels>(() => ({
  ...queueLabels.value,
  state: {
    waiting: tContent('labels.state.waiting'),
    sending: tContent('labels.state.waiting'),
  },
}));
...
// ─── Navigation groups ────────────────────────────────────────────────────────
//
// Não há seção de variantes: a fila tem um desenho só, e tudo o que muda nela é
// situação — o que mora em `Estados`.
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.sending') }}
          </p>
          <MessageQueue
            :labels="queueLabels"
            :messages="sendingQueue"
          />
        </div>
        ...
        >
          ...
          <MessageQueue
            :labels="queueLabels"
            :messages="longWaitingQueue"
          />
        </div>

        <Separator />

        <!-- A fila é peça própria: quem consome a empilha ACIMA do campo, e o
             campo não sabe que ela existe. -->
        <div
          class="nds-stack nds-w-full"
          data-spacing="xs"
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.aboveField') }}
          </p>
          <div
            class="nds-stack nds-w-full"
            data-spacing="xs"

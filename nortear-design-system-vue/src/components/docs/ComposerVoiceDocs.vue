...
import { Composer, ComposerVoice, type ComposerVoiceLabels } from '@/components/ui/composer';
...
//
// A ÚNICA linha sobrescrita é o NOME do aviso de que alguém pediu para começar
// ou parar. O conteúdo compartilhado o descreve como callback, que é a forma do
 ...
 *
 * A diferença entre uma captura de pé e uma parada passa a existir só no
 * desenho do medidor — e pista que só existe em desenho não chega a quem ouve.
 */
const wordlessStatus = computed<ComposerVoiceLabels>(() => ({
  ...voiceLabels.value,
  status: { idle: '', recording: '', transcribing: '' },
}));

const sampleLevel = SAMPLE_LEVEL;
const sampleElapsed = SAMPLE_ELAPSED;
const sampleElapsedDone = SAMPLE_ELAPSED_DONE;
...
// stories: a peça tem um desenho só, e tudo o que muda nela é situação.
...
const stateItems = computed(() =>
  ['idle', 'recording', 'transcribing', 'disabled'].map(k => ({
...
<template>
  ...
  >
    ...
    <!-- ── Demonstração ───────────────────────────────────────────── -->
    <!--
      A legenda diz QUAL ponto está desenhado — sem ela, três controles
      ...
      >
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.idle') }}
          </p>
          <ComposerVoice
            :labels="voiceLabels"
            state="idle"
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.recording') }}
          </p>
          <ComposerVoice
            :labels="voiceLabels"
            state="recording"
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.transcribing') }}
          </p>
          <ComposerVoice
            :labels="voiceLabels"
            state="transcribing"
            :elapsed="sampleElapsedDone"
          />
        </div>
        ...
        >
          <p class="nds-text-caption nds-text-muted-foreground">
            {{ tContent('demonstration.labels.inRail') }}
          </p>

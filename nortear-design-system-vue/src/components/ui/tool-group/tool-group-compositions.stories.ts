...
export const BeforeAnswer: Story = {
  render: () => ({
    ...
    setup() {
      return { calls: TOOL_CALLS_WITH_FAILURE, labels: useToolGroupLabels() };
    },
...
export const Toggling: Story = {
  ...
  render: () => ({
    components: { ToolGroup },
    setup() {
      return { calls: TOOL_CALLS_WITH_FAILURE, labels: useToolGroupLabels(), onOpenChange };
    },
    template: `<ToolGroup
      :calls="calls"
      :labels="labels"
  ...
  render: () => ({
    ...
    setup() {
      const split = computed(() =>
        splitWaitingCalls([TOOL_CALL_WAITING, ...TOOL_CALLS_WITH_FAILURE]),
      );
      return {
        grouped: computed(() => split.value.grouped),
        waiting: computed(() => split.value.waiting),

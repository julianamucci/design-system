...
export function panelOpen(): HTMLElement | null {
  const panel = document.body.querySelector<HTMLElement>(SELECTOR_PANEL);
  if (!panel) return null;
  return panel.querySelector(SELECTOR_CONTENT) ? panel : null;
}
...
export async function waitForPanel(timeout = 4000): Promise<HTMLElement> {
  return await waitFor(
    () => {
      const panel = document.body.querySelector<HTMLElement>(SELECTOR_PANEL);
      if (!panel) throw new Error('painel: ainda não montou');
      ...
      const content = panel.querySelector<HTMLElement>(SELECTOR_CONTENT);
      if (!content) throw new Error('painel: o miolo ainda não foi instanciado');
      if (content.getBoundingClientRect().height < 1) {

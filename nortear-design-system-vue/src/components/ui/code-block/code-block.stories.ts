...
<template>
  <CodeBlock :code="source" language="ts" title="exemplo.ts" />
</template>`;
...
const meta = {
  ...
  argTypes: {
    ...
    footer: {
      control: 'text',
      description: 'Observações abaixo do código. Também aceita o slot footer.',
      table: { type: { summary: 'string' } },
    },
    copyLabel: {
      control: false,
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('Copiar coloca no clipboard só o código, sem os números de linha', async () => {
      ...
      try {
        ...
        await expect(writeText).not.toHaveBeenCalledWith(expect.stringMatching(/^1<script/));
      } finally {

...
import { flowGraphLabels, useFlowGraphLabels } from './flow-graph.fixtures';
 ...
 * pontas; e a presença das ligações decide se o desenho é um grafo ou uma
 * grade de caixas soltas.
 *
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(FlowGraphDocs),
  ...
  argTypes: {
    status: {
      ...
      table: {
        type: { summary: RUN_STATUSES.map((item) => `'${item}'`).join(' | ') },
...
export const Playground: Story = {
  ...
  // O INVÓLUCRO existe para a asserção ter a que apontar quando não há grafo:
  // sem nó a peça não desenha nada, nem moldura, nem camada que rola.
  render: (args) => ({
    ...
    setup() {
      return {
        ...
        nodes: computed(() => FLOW_NODES_ORDER.slice(0, args.revealed)),
        edges: computed(() => (args.withEdges ? FLOW_EDGES_ORDER : [])),
      };
    },
    template: `<div data-testid="flow-graph-host">
      <FlowGraph
        :nodes="nodes"
        :edges="edges"
        :status="args.status"
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    if (args.revealed === 0) {
      // Sem nó não há grafo, e a peça não desenha nada — nem moldura, nem
    ...
    await step('Há um nó por nó, na ordem em que foram declarados', async () => {
      ...
      await expect(nodes.map((node) => node.dataset.nodeId)).toEqual(
        visible.map((node) => node.id),
      );
      await expect(nodes.map((node) => node.dataset.state)).toEqual(
        visible.map((node) => node.state),
      );
    });
    ...
    await step('Só a ligação com as duas pontas é desenhada', async () => {
      ...
      const expected = args.withEdges
        ? FLOW_EDGES_ORDER.filter((edge) => ids.has(edge.from) && ids.has(edge.to))
        : [];
      ...
      // "aresta adiante", que era a única razão do segundo tom de traço.
      const distinct = new Set([...paths].map((path) => path.getAttribute('vector-effect')));
    ...
    await step('Cada nó diz o estado em palavra, e de quem depende', async () => {
      ...
      for (const [index, element] of nodes.entries()) {
        ...
        const sources = args.withEdges
          ? FLOW_EDGES_ORDER.filter((edge) => edge.to === declared.id && ids.has(edge.from))
          : [];
        if (sources.length === 0) {
          await expect(reading.textContent).not.toContain(
            labels.dependsOn.split('{')[0].trim(),
          );
          continue;
        }
        for (const source of sources) {
          const sourceLabel = FLOW_NODES_ORDER.find((node) => node.id === source.from)!.label;

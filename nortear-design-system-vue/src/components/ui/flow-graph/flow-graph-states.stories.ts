...
import { FlowGraph, type FlowGraphLabels } from './index';
 ...
 */
function everyStateNodes(labels: FlowGraphLabels): FlowNode[] {
...
export const EveryState: Story = {
  ...
  render: () => ({
    ...
    setup() {
      ...
      return {
        labels,
        nodes: computed(() => everyStateNodes(labels.value)),
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Há um nó por estado, e cada um diz qual é o seu', async () => {
      ...
      await expect(nodes.map((node) => node.dataset.state)).toEqual([...TOOL_CALL_STATES]);
    });
...
export const Empty: Story = {
  ...
  // O INVÓLUCRO é o que permite afirmar que NADA foi desenhado: sem ele, a
  // story não teria elemento nenhum a que a asserção pudesse apontar.
  render: () => ({
    components: { FlowGraph },
    setup() {
      return {
    ...
    template: `<div data-testid="flow-graph-empty-host">
      <FlowGraph
        :nodes="nodes"
        :edges="edges"
        status="idle"

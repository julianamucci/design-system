...
export const WholeYear: Story = {
  ...
  render: () => ({
    ...
    template: `<div class="nds-max-w-md">
      <ActivityGraph
        :days="days"
        start="${WIDE_START}"
        end="${WIDE_END}"
  ...
  render: () => ({
    ...
    setup() {
      return {
        ...
        days: ACTIVITY_DAYS,
        thresholds: ACTIVITY_THRESHOLDS,
        // Declarada NO PRÓPRIO elemento, e não no invólucro: a folha a declara
        // em `.nds-activity-graph`, e propriedade personalizada declarada
        // dentro do próprio seletor vence a herança — o invólucro nunca a
        // alcançaria. Nesta stack o caminho é o repasse de `style`, que o Vue
        // funde na raiz do componente.
        tight: { '--activity-graph-cell': 'var(--spacing-2)' },
      };
    },

...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(QuotaBannerDocs),
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      // O horizonte e os rótulos saem de composables, então o render passa por
      // um `setup`. A cota entra por `computed`: os controls trocam com a story
      // montada, e um objeto lido uma vez congelaria a foto.
      return {
        labels: useQuotaBannerLabels(),
        quota: computed(() => ({ used: args.used, limit: args.limit })),
        renewsIn: useQuotaRenewal(() => (args.renews ? RENEWAL_MINUTES : undefined)),
      };
    },

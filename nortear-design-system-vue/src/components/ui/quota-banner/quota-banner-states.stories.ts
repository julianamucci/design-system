...
/** Do mais folgado ao mais apertado — a ordem em que a conta os separa. */
const LEVEL_CASES = ['normal', 'warning', 'critical'] as const;
...
const mount = (name: QuotaBannerCase) => ({
  ...
  setup() {
    // O horizonte sai do `computed` e é indexado NO template, e não aqui: lido
    // no `setup`, ele congelaria a foto no idioma em que a story abriu.
    return {
      name,
      quota: quotaOf(name),
...
export const AllLevels: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        names: LEVEL_CASES,
        quotas: Object.fromEntries(LEVEL_CASES.map((name) => [name, quotaOf(name)])),
        renewals: useQuotaRenewals(),
        labels: useQuotaBannerLabels(),
      };
    },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada uso traz a palavra do SEU nível', async () => {
      for (const [index, name] of LEVEL_CASES.entries()) {
        const level = fractionLevel(fractionOf(name)!);
        await expect(banners[index]!.dataset.level).toBe(level);
    ...
    await step('E a razão de cada um está em TEXTO, e não só no desenho', async () => {
      ...
      for (const [index, name] of LEVEL_CASES.entries()) {
        ...
        await expect(textOf(banners[index]!, 'detail'))
    ...
    await step('As três palavras são diferentes entre si', async () => {
      ...
      const words = LEVEL_CASES.map((name) => labels.level[fractionLevel(fractionOf(name)!)]);
      await expect(new Set(words).size).toBe(LEVEL_CASES.length);
    });
  },
};
 ...
 *
 * Crédito comprado uma vez acontece de verdade, e a linha some em vez de dizer
 * "renova em nunca". Os números são os mesmos do exemplo de aviso de propósito:
 * assim a única diferença entre as duas fotos é a linha que falta.
 */

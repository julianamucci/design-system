...
export function embedCommand(provider: EmbedProvider, command: EmbedCommand): string {
  if (provider === 'youtube') {
    ...
    return JSON.stringify({ event: 'command', ...dialect });
  }
...
export function parseEmbedMessage(provider: EmbedProvider, data: unknown): EmbedEvent | null {
  ...
  if (provider === 'youtube') {
    if (payload.event === 'onStateChange') {
      const eventType = YOUTUBE_STATE[Number(payload.info)];
      return eventType ? ({ type: eventType } as EmbedEvent) : null;
    }
  ...
  const detail = payload.data as { seconds?: number; duration?: number } | undefined;
  if (name === 'play') return { type: 'playing' };
  if (name === 'pause') return { type: 'paused' };
  if (name === 'ended') return { type: 'ended' };
  if (name === 'timeupdate' && detail && typeof detail.seconds === 'number') {
    return { type: 'time', currentTime: detail.seconds, duration: detail.duration ?? 0 };
  }
  return null;
}

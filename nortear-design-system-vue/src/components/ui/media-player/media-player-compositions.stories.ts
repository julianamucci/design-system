...
import { LABELS, YOUTUBE_VIDEO_ID, canvasStream, captionTrack } from './media-player.fixtures';
import { clockText, messageFromFrame, playerBridge, until } from './media-player.play-helpers';
import { mediaPlayerCaptionsSource, mediaPlayerTwoPlayersSource } from './media-player.source';

/**
 * As instâncias da story corrente.
 *
 * É uma LISTA porque `TwoPlayers` precisa dos dois quadros ao mesmo tempo: o
 * ponto da composição é que uma mensagem chega à `window` inteira e só um dos
 * dois a aceita, e isso só se prova comparando um com o outro.
 */
let players: MediaPlayerApi[] = [];

/** Guarda a instância recém-montada na posição que ela ocupa na composição. */
function keepPlayer(index: number) {
  return (instance: MediaPlayerApi | null): void => {
    if (instance) players[index] = instance;
  };
}
...
const meta = {
  title: 'Components/Display/MediaPlayer/Compositions',
...
export const WithCaptions: Story = {
  parameters: { docs: { source: { transform: mediaPlayerCaptionsSource } } },
  render: () => ({
    components: { MediaPlayer },
    setup() {
      players = [];
      return {
        playerRef: playerBridge(keepPlayer(0)),
        labels: LABELS,
        stream: canvasStream(),
        tracks: [captionTrack()],
      };
  ...
  play: async ({ step }) => {
    const video = players[0].media as HTMLVideoElement;
...
export const TwoPlayers: Story = {
  parameters: { docs: { source: { transform: mediaPlayerTwoPlayersSource } } },
  render: () => ({
    components: { MediaPlayer },
    setup() {
      players = [];
      return {
        firstRef: playerBridge(keepPlayer(0)),
        secondRef: playerBridge(keepPlayer(1)),
        labels: LABELS,
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const [first, second] = Array.from(
      canvasElement.querySelectorAll<HTMLElement>('[data-slot="media-player"]'),
    );

    await step('São dois quadros, cada um com o seu', async () => {
      await expect(canvas.getAllByRole('group', { name: LABELS.player })).toHaveLength(2);
      await expect(players[0].frame).not.toBeNull();
      await expect(players[1].frame).not.toBeNull();
      await expect(players[0].frame).not.toBe(players[1].frame);
    });
    ...
    await step('A mensagem do provedor move a barra DELE', async () => {
      ...
      messageFromFrame(players[0].frame!, JSON.stringify({ event: 'onStateChange', info: 1 }));
    ...
    await step('E não move a do outro', async () => {
      ...
      messageFromFrame(players[0].frame!, JSON.stringify({ event: 'onStateChange', info: 2 }));
    ...
    await step('O que esta suíte NÃO prova', async () => {
      ...
      await expect(players[0].frame!.src.startsWith('https://')).toBe(true);
    });
  },
};

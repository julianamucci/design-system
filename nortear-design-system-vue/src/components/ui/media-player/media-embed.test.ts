...
describe('embedHandshake', () => {
  ...
  it('Vimeo assina um evento por vez, e assina os quatro que a barra usa', () => {
    const subscribed = embedHandshake('vimeo').map((message) => JSON.parse(message).value);
...
describe('parseEmbedMessage', () => {
  it('lê o estado numérico do YouTube', () => {
    const parse = (info: number) => parseEmbedMessage('youtube', { event: 'onStateChange', info });
    expect(parse(1)).toEqual({ type: 'playing' });
    expect(parse(2)).toEqual({ type: 'paused' });
    expect(parse(0)).toEqual({ type: 'ended' });

...
import { elapsedOf, useAgentStatusLabels } from '@/components/ui/agent-status/agent-status.fixtures';
import {
  jobProgressLabels,
  useJobLabel,
...
export const JobQueue: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        count: JOB_COUNT,
        countWithoutTotal: JOB_COUNT_WITHOUT_TOTAL,
        label: useJobLabel(),
        labels: useJobProgressLabels(),
        onAction,
      };
    },
    // O que espera não recebe conta nenhuma: nada começou, e não há o que
    // escrever.
    template: `<div class="nds-stack nds-max-w-lg" data-spacing="lg">
      <JobProgress
        :label="label"
        status="running"
        :count="count"
...
export const BesideRunStatus: Story = {
  ...
  render: () => ({
    components: { JobProgress, AgentStatus },
    setup() {
      return {
        count: JOB_COUNT,
        label: useJobLabel(),
        labels: useJobProgressLabels(),
        runLabels: useAgentStatusLabels(),
        elapsed: elapsedOf('complete'),
        onAction,
      };
    },
    // As duas são IRMÃS num invólucro, e não pai e filha: cada uma responde a
    // uma pergunta, e aninhá-las faria a segunda parecer detalhe da primeira.
    template: `<div class="nds-stack nds-max-w-lg" data-spacing="lg">
      <AgentStatus
        status="complete"
        :elapsed="elapsed"
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E nenhuma das duas carrega região viva', async () => {
      // A exceção da folha é do estado da ligação e do cartão de autorização,
      // e nenhuma das duas é isto: aqui nada está bloqueado.
...
export const Requesting: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O alvo de toque tem pelo menos vinte e quatro pixels', async () => {
      ...
      // tentação de encolher é maior — e a coluna da direita é compartilhada
      // com a conta, que é larga.

 ...
 * não este andaime.
 */
const mount = (status: RunStatus, count: JobCount = JOB_COUNT) => ({
  ...
  setup() {
    return {
      ...
      count,
      label: useJobLabel(),
      labels: useJobProgressLabels(),
      onAction,
    };
  },
  template: `<JobProgress
    :label="label"
    :status="status"
    :count="count"
    :labels="labels"
...
export const EveryStatus: Story = {
  ...
  render: () => ({
    ...
    template: `<div class="nds-stack" data-spacing="lg">
      <JobProgress
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada uma traz a PALAVRA daquele estado', async () => {
      for (const [index, status] of RUN_STATUSES.entries()) {
        await expect(wordOf(pieces[index]!).textContent).toBe(labels.status[status]);
      }
    });
    ...
    await step('A barra guarda o papel e o NOME do trabalho', async () => {
      ...
      // quanto, nunca de quê. E ela não é escondida — "não se anuncia" e "não
      // se lê" são coisas diferentes (decisão 4 da folha).
    ...
    await step('Só o que corre se declara ocupado', async () => {
      for (const [index, status] of RUN_STATUSES.entries()) {
        const busy = pieces[index]!.getAttribute('aria-busy');
    ...
    await step('E os rótulos de ação são TRÊS nomes diferentes', async () => {
      ...
      for (const [index, status] of RUN_STATUSES.entries()) {
        ...
        const button = actionOf(pieces[index]!);
...
/**
 * Sem total conhecido — o caso que a peça existe para não errar.
 *
...
export const UnknownTotal: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        counts: [
          JOB_COUNT_WITHOUT_TOTAL,
          { done: JOB_COUNT_WITHOUT_TOTAL.done, total: 0 },
        ] as JobCount[],
        label: useJobLabel(),
        labels: useJobProgressLabels(),
        onAction,
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Nenhuma das duas mostra um número', async () => {
      ...
      await expect(pieces).toHaveLength(2);
      for (const piece of pieces) {
        const bar = barOf(piece);
...
export const Stopped: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E a ação oferece RETOMAR, com nome próprio', async () => {
      // Interromper não é erro, e o que a pessoa escolheu fazer se retoma —
      // não se tenta de novo.

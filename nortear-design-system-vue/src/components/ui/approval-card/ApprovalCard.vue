 ...
 * — a numeração é a DA FOLHA, que é a origem citada aqui e a única que vale nas
 * cinco; o docblock do grupo nesta stack ordena as mesmas decisões de outro
 * jeito. A decisão diz que uma execução de ferramenta que espera por uma PESSOA
 * não fica dentro da caixa recolhida, porque pedir autorização sem mostrar é
 * não pedir. Este é o lugar
 ...
 *
 * DIVERGÊNCIA DE API DE FRAMEWORK, registrada e não "alinhada", em dois pontos.
 * O espaço dos controles é um SLOT (`#actions`), e não uma lista de nós passada
 * por propriedade: é a forma desta stack para "quem desenha é quem consome", a
 * mesma que a autorização dentro da conversa já usa. E o aviso de qual foi a
 * escolha é um EVENTO (`@choose`), e não um retorno passado por propriedade. O
 * conceito é o mesmo dos dois lados; o que muda é por onde a resposta entra e
...
// nasceu para exatamente este caso — o slot está sempre declarado, e quem
// decide se ele desenha alguma coisa é o `v-if` de quem consome, lá dentro.
// Copiá-la produziria duas versões que divergem sem nenhum sinal.
...
defineProps<{
   ...
   * do alcance, porque juntar o nome de uma ferramenta com "?" produz uma
...
/**
 * DELEGAÇÃO, e não um ouvinte por controle: os controles chegam prontos de quem
 * consome, e pendurar um ouvinte em cada um obrigaria a peça a conhecer a forma
 * de cada um deles. O que ela conhece é o atributo.
 *
 * `closest` porque o alvo do clique pode ser um ícone ou um `<span>` dentro do
 * botão; a guarda de contenção existe porque `closest` sobe sem limite, e sem
 * ela um atributo posto na raiz por engano viraria resposta.
 */
...
<template>
  ...
  >
    <!-- A CAIXA QUE SE ANUNCIA (decisão 1), e ela existe como elemento
         ...
         único lugar onde o papel caberia seria a raiz — e a raiz contém os
         botões.
         ...
         prazo. -->
    <div
      class="nds-approval-card-ask"
      data-slot="approval-card-ask"
      role="status"
    >
      <p
      ...
      <!-- O ALCANCE É UMA LISTA DE DEFINIÇÃO (decisão 2). Com `<dl>`, o termo e
      ...
      <dl
        v-if="scope?.length"
        class="nds-approval-card-scope"
        data-slot="approval-card-scope"
      >
        <template

...
/** Um cartão daquele exemplo, com ou sem a lista do alcance. */
const mountCard = (name: ApprovalExampleName, withScope = true) => ({
  components: { ApprovalCard, Button },
  setup() {
    const labels = useApprovalCardLabels();
    return {
      question: computed(() => labels.value.question[name]),
      scope: computed(() => (withScope ? approvalScopeOf(labels.value, name) : undefined)),
...
/** O mesmo cartão sem o slot dos controles — e não com o slot vazio. */
const mountWithoutActions = (name: ApprovalExampleName) => ({
  components: { ApprovalCard },
  setup() {
    const labels = useApprovalCardLabels();
    return {
      question: computed(() => labels.value.question[name]),
      scope: computed(() => approvalScopeOf(labels.value, name)),
    };
  },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('E cada termo é seguido pelo SEU valor, alternando', async () => {
      ...
      for (const [index, item] of expected.entries()) {
        ...
        await expect(term.textContent?.trim()).toBe(item.term);
        await expect(detail.tagName).toBe('DD');
        await expect(detail.textContent?.trim()).toBe(item.detail);
      }
    });
  },
};
...
export const WithoutScope: Story = {
  ...
  render: () => mountCard('publish', false),
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement);
    ...
    await step('E a pergunta continua à vista, dentro da região que se anuncia', async () => {
      ...
      await expect(question.textContent?.trim()).toBe(approvalCardLabels().question.publish);
...
export const LongDetail: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('O valor chega INTEIRO ao documento', async () => {
      await expect(details.map((el) => el.textContent?.trim())).toEqual(
...
export const WithoutActions: Story = {
  ...
  render: () => mountWithoutActions('publish'),
  play: async ({ canvasElement, step }) => {
    const card = cardOf(canvasElement);
    const ask = card.querySelector<HTMLElement>('[data-slot="approval-card-ask"]')!;

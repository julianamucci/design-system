...
const meta: Meta<PlaygroundArgs> = {
  ...
  args: {
    // O idioma vem escrito, e não da store: este módulo é avaliado ANTES de a
    // aplicação existir, e ler a store aqui derrubaria o arquivo na carga. O
    // control também não acompanha a barra de idioma — o que ele guarda é o
    // texto que alguém digitou, e trocá-lo por baixo apagaria a edição.
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      ...
      return {
        args,
        // "Sem alcance" é ausência de lista, e não uma lista vazia: um arranjo
        // sem itens desenharia uma caixa com afastamento e nada dentro.
        scope: computed(() =>
          args.scope === 'none' ? undefined : approvalScopeOf(labels.value, args.scope),
        ),
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A pergunta abre o cartão', async () => {
      ...
      await expect(question.textContent?.trim()).toBe(args.question);
    });
    ...
    await step('E cada termo do alcance chega pareado com o seu valor', async () => {
      ...
      await expect(terms.map((el) => el.textContent?.trim())).toEqual(
        expected.map((item) => item.term),
      );

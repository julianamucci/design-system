 ...
 *
 * A peça não desenha nada quando não há passo nenhum, e a story da lista vazia
 * precisa de um elemento para devolver mesmo assim.
 */
const mount = (steps: PlanStep[]) => ({
  components: { AgentPlan },
  setup() {
    return { steps, labels: useAgentPlanLabels() };
  },
...
export const Finished: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Com tudo terminado, nenhum passo é o atual', async () => {
      ...
      await expect(
        steps.every((el) => isStepFinished(el.dataset.state as PlanStepState)),
      ).toBe(true);

 ...
 *
 * DIVERGÊNCIA DE API DE FRAMEWORK, registrada e não "alinhada": a peça não
 * declara prop de classe. A raiz imperativa da referência recebe a classe extra
 * por opção, porque lá não existe outro caminho; aqui a classe que quem consome
 * escreve na tag CHEGA SOZINHA à `<ol>` pelo repasse de atributos, e declará-la
 * criaria uma segunda porta para a mesma coisa. O conceito é o mesmo dos dois
 * lados — o que muda é por onde a classe entra.
 *
 ...
 */
const STATE_VARIANT: Record<PlanStepState, BadgeVariants['variant']> = {
  pending: 'info',
  running: 'default',
  done: 'success',
  failed: 'destructive',
 ...
 * discordaria sobre `skipped`, que é o estado em que a resposta é menos óbvia:
 * pulado não aconteceu, e ainda assim é fim.
 *
...
const currentIndex = computed(() =>
  props.steps.findIndex((step) => !isStepFinished(step.state)),
)
</script>

<template>
  <!-- Sem passo nenhum, o plano NÃO EXISTE no documento. Não é uma lista vazia
       escondida: é ausência. Uma `<ol>` sem item é anunciada como "lista com
       zero itens", que promete algo que não há — a mesma decisão da fila de
       envio e da lista de contexto do campo.
       ...
       Nada de `aria-live`, `role="log"` ou `role="status"`: o plano anda passo
       a passo enquanto a resposta é gerada logo ao lado, e narrar cada troca é
       a mesma armadilha do relógio (decisão 4). -->
  <ol
    v-if="steps.length > 0"
  ...
  >
    ...
    >
      ...
      <!-- O rótulo inteiro, sem corte. A folha resolve a quebra com
           `overflow-wrap` (decisão 5), e cortar aqui seria decidir por ela: um
           passo pela metade é uma instrução pela metade. -->
      <span
      ...
      <!-- O detalhe é o que o rótulo não diz: por que pulou, o que produziu, o
           ...
           cor neutra e nunca na semântica. -->
      <p
        v-if="step.detail"

...
export const SubmitOnEnter: Story = {
  ...
  render: () => ({
    components: { Composer },
    setup() {
      return { labels: useComposerLabels(), onSubmit };
    },
    template:
      '<Composer :labels="labels" submit-on="enter" class="nds-max-w-lg" @submit="onSubmit" />',
  }),
...
export const SubmitOnModifier: Story = {
  ...
  render: () => ({
    ...
    template:
      '<Composer :labels="labels" submit-on="modifier" class="nds-max-w-lg" @submit="onSubmit" />',
  }),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    const input = canvas.getByRole('textbox');

...
/**
 * Os rótulos da citação fora de um componente — `play` não é render.
 *
 * Lê a MESMA store de locale que o composable abaixo, então o rótulo que a play
 * procura é sempre o que a citação desenha.
 */
export function quoteLabels(): ComposerQuoteLabels {
  return quoteLabelsFor(useI18nStore().locale);
}
 ...
 * um objeto congelaria a citação no idioma em que a story abriu — e a barra de
 * idioma do Storybook troca o idioma com a story montada.
 */
export function useQuoteLabels(): ComputedRef<ComposerQuoteLabels> {
  const { locale } = useTranslation(quoteTranslations);
  return computed(() => quoteLabelsFor(locale.value as Locale));
}

...
function sizeText(attachment: Attachment): string | null {
  ...
  return `${value.toLocaleString()} ${props.labels.unit[unit]}`
}
...
function metaText(attachment: Attachment): string {
  return [sizeText(attachment), props.labels.state[attachment.state]]
    .filter(Boolean)
    .join(' · ')
}
...
function removeLabel(attachment: Attachment): string {
  return props.labels.remove.replace('{name}', attachment.name)
}
...
function requestRemove(attachment: Attachment): void {
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- Decorativa: o estado já está escrito no texto de apoio, e uma barra
           que se anuncia a cada quadro é o defeito que esta peça existe para
           evitar. -->
      <span
        v-if="attachment.state === 'uploading'"
        class="nds-composer-attachment-bar"
        aria-hidden="true"
      >
        <!-- Custom property, e não largura em `style`: o valor é dado de
             runtime, e a folha é quem decide como ele vira desenho. -->
        <span
      ...
      <!-- Uma fila de três botões chamados "Remover" é indistinguível por
           audição: o nome acessível leva o nome do arquivo. -->
      <Button
        data-slot="composer-attachment-remove"
        variant="ghost"
        size="icon-sm"
        :aria-label="removeLabel(attachment)"
        @click="requestRemove(attachment)"
      >
        ×
      </Button>
    </li>

 ...
 *
 * Existe pelo mesmo motivo do `composer-context.fixtures.ts`: num `*.stories.ts`
 ...
 * Traduzi-los faria as stories fotografarem listas diferentes conforme o idioma
 * da foto.
 *
 ...
 * gatilho sem nome acessível seria um botão anunciado só pelo valor.
 */
...
/**
 * Os rótulos do seletor fora de um componente — `play` não é render.
 *
 * Lê a MESMA store de locale que o composable abaixo, então o rótulo que a play
 * procura é sempre o que o seletor desenha.
 */
export function modelLabels(): ComposerModelPickerLabels {
  return modelLabelsFor(useI18nStore().locale);
}
 ...
 * um objeto congelaria o seletor no idioma em que a story abriu — e a barra de
 * idioma do Storybook troca o idioma com a story montada.
 */
export function useModelLabels(): ComputedRef<ComposerModelPickerLabels> {
  const { locale } = useTranslation(pickerTranslations);
  return computed(() => modelLabelsFor(locale.value as Locale));
}

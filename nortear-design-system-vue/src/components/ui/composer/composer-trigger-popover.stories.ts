...
const meta: Meta<Args> = {
  ...
  parameters: {
    ...
    docs: {
      ...
      // Sem a transform o gerador imprime a tag sozinha, sem os rótulos e sem
      // os gatilhos que fazem o painel existir. O painel mostra o uso real.
...
export const Playground: Story = {
  ...
  render: (args) => ({
    components: { Composer },
    setup() {
      // Os gatilhos saem de composables, então o render passa por um `setup`.
      const mention = useMentionSource();
      const command = useCommandSource();
      const triggers = computed(() => [
        ...(args.mention ? [mention.value] : []),

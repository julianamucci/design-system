...
export const Filled: Story = {
  ...
  render: () => ({
    components: { Composer },
    setup() {
      return { labels: useComposerLabels(), sample: SAMPLE, onSubmit };
    },
    template:
      '<Composer :labels="labels" :value="sample" class="nds-max-w-lg" @submit="onSubmit" />',
  }),
...
export const Running: Story = {
   ...
   * a play chamar. Quem o liga é um controle na tela, que é justamente o
   * caminho real: quem sabe se a resposta está vindo é quem consome.
   */
  render: () => ({
  ...
  render: () => ({
    ...
    setup() {
      return {
        ...
        filled: textOfLength(Math.ceil(LIMIT * 0.95)),
        onSubmit,
      };
    },
    template:
      '<Composer :labels="labels" :max-length="limit" :value="filled" class="nds-max-w-lg" @submit="onSubmit" />',
  }),
  play: async ({ canvasElement, step }) => {
    const root = canvasElement.querySelector<HTMLElement>('[data-slot="composer"]')!;
...
export const Disabled: Story = {
  ...
  render: () => ({
    ...
    template:
      '<Composer :labels="labels" :value="sample" disabled class="nds-max-w-lg" @submit="onSubmit" />',
  }),
  play: async ({ canvasElement, step }) => {
    const canvas = within(canvasElement);
    const input = canvas.getByRole('textbox');

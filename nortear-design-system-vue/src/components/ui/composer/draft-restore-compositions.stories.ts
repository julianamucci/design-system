...
import DraftRestore from './DraftRestore.vue';
...
const aboveComposer = (timestamp?: string) => ({
  components: { Composer, DraftRestore },
  setup() {
    return {
      labels: useComposerLabels(),
      draftBandLabels: useDraftLabels(),
      draft: SAMPLE_DRAFT,
      timestamp,
      onAction,
    };
  },
  // Nenhum utilitário de pilha no invólucro, e é de propósito: a distância até
  // o campo já é do desenho da faixa (`margin-block-end`), e uma pilha por cima
  // somaria o gap dela ao respiro que a folha já declarou.
  template: `<div class="nds-max-w-lg">
    <DraftRestore

...
import {
  ...
  triggerPopoverCommandsSource,
  triggerPopoverMentionsSource,
...
export const Mentions: Story = {
  parameters: {
    ...
    docs: { source: { transform: triggerPopoverMentionsSource } },
  },
  render: () => ({
    components: { Composer },
    setup() {
      ...
      return {
        ...
        triggers: computed(() => [mention.value]),
        onSubmit,
      };
    },
    template: TEMPLATE,
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Digitar filtra, e quem COMEÇA pelo termo vem antes', async () => {
      ...
      const rotulos = [...panel().querySelectorAll('.nds-composer-trigger-option-label')]
        .map((el) => el.textContent);
      await expect(rotulos).toEqual(['Ana Souza', 'Ângela Reis', 'Joana Lima']);
    });
...
export const Commands: Story = {
  parameters: {
    ...
    docs: { source: { transform: triggerPopoverCommandsSource } },
  },
  render: () => ({
    components: { Composer },
    setup() {
      ...
      return {
        labels: useComposerLabels(),
        triggerLabels: useTriggerLabels(),
        triggers: computed(() => [command.value]),
        onSubmit,
      };
    },
    template: TEMPLATE,

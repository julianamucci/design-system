...
import { DEFAULT_LOCALE, indicatorLabelsFor } from './thinking-indicator.fixtures';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  args: {
    // O valor inicial do control sai do idioma padrão da documentação, e não do
    // idioma corrente: aqui é escopo de módulo, e a store de locale só existe
    // depois que a aplicação da story é criada. Quem troca o idioma na barra
    // troca junto o que este controle mostra.
    label: indicatorLabelsFor(DEFAULT_LOCALE).generating,
  },
};
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    template: '<ThinkingIndicator :label="args.label" />',
  }),

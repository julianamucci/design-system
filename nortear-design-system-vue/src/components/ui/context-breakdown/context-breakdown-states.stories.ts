...
const mount = (name: ContextBreakdownCase) => ({
  components: { ContextBreakdown },
  setup() {
    return {
      parts: partsOf(name),
 ...
 *
 * A situação se produz TIRANDO um rótulo, e nunca inventando uma parcela: o que
...
export const UnlabeledOrigin: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        ...
        labels: useContextBreakdownLabelsWithout('tools'),
      };
    },
    template: `<ContextBreakdown
      :parts="parts"
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('A linha continua na lista, e na posição dela', async () => {
      await expect(rows).toHaveLength(parts.length);
      await expect(contextBreakdownLabelsWithout('tools').parts.tools).toBeUndefined();
    });

 ...
 */
const aboveField = (messages: QueuedMessage[]) => ({
  components: { Composer, MessageQueue },
  setup() {
    return {
      // `computed` e não o objeto pronto: o `setup` roda uma vez, e a barra de
      // idioma do Storybook troca o idioma com a story montada.
      labels: computed(composerLabels),
      queueLabels: useQueueLabels(),
      messages,
      onWithdraw,
    };
  },
  template: `<div class="nds-stack nds-max-w-lg" data-spacing="xs">
    <MessageQueue
      :labels="queueLabels"
      :messages="messages"
      @withdraw="onWithdraw"

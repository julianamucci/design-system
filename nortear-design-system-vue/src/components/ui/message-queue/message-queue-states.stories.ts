...
import { queueLabels, sending, useQueueLabels, waiting } from './message-queue.fixtures';
import {
  queueEmptySource,
  queueSendingSource,
  queueWaitingSource,
...
export const Empty: Story = {
  ...
  // A peça não desenha nada quando não há o que esperar, então a story precisa
  // de um lugar onde a ausência caiba.
  render: () => ({
    components: { MessageQueue },
    setup() {
      return { labels: useQueueLabels(), messages: [] as QueuedMessage[] };
    },

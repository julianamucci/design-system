...
import { SAMPLE_TEXT, queueLabels, useQueueLabels } from './message-queue.fixtures';
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      ...
      // A mensagem é um `computed` sobre os args para trocar de control refazer
      // a fila, e não só o snippet do painel.
      const messages = computed<QueuedMessage[]>(() => [
        { id: 'm1', text: args.text, state: args.state },
      ]);
      return { labels: useQueueLabels(), messages, onWithdraw };
    },
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('O texto escolhido chega inteiro, e o estado chega em PALAVRA', async () => {
      ...
      const label = item.querySelector<HTMLElement>('[data-slot="composer-queue-text"]')!;
      await expect(label.textContent).toBe(args.text);

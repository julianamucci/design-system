 ...
 */
function withRefusedSource(): ChatMessage[] {
...
export const WithReasoningAndTools: Story = {
  ...
  render: () => ({
    ...
    setup: () => ({ labels: useChatLabels(), messages: withRefusedSource() }),
    template: '<ChatThread :messages="messages" :labels="labels" size="lg" />',
  }),
  play: async ({ canvasElement, step }) => {
    ...
    await step('A fonte de endereço recusado é legível, e NÃO é clicável', async () => {
      // O endereço de uma fonte vem de quem gerou a resposta. `javascript:`
      // num `href` executa ao clique, então ele não vira link — e a fonte não
      // some, porque o texto dela continua sendo informação.
      const sources = root.querySelector<HTMLElement>('.nds-chat-sources')!;
...
export const WithActions: Story = {
  ...
  render: () => {
    ...
    return {
      ...
      setup: () => ({
        ...
        messages,
        // As ações são do último turno. Nesta stack elas entram por slot com
        // escopo, então quem escolhe o turno é o `v-if` de quem consome.
        actionsFor: messages[messages.length - 1].id,
        onCopy,
      }),
      template: `
        <ChatThread :messages="messages" :labels="labels" size="lg">
          <template #actions="{ message }">
            <template v-if="message.id === actionsFor">
              <Button variant="ghost" size="sm" @click="onCopy">Copiar</Button>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const copy = canvas.getByRole('button', { name: /copiar/i });

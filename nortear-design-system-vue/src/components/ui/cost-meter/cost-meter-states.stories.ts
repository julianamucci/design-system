...
const mount = (name: CostMeterCase) => ({
  ...
  setup() {
    return { name, views: useCostViews(), labels: useCostMeterLabels() };
  },
  template: `<CostMeter
    :amount="views[name].amount"
    :budget="views[name].budget"
    :labels="labels"
  />`,
});
...
export const AllLevels: Story = {
  ...
  render: () => ({
    components: { CostMeter },
    setup() {
      return { names: LEVEL_CASES, views: useCostViews(), labels: useCostMeterLabels() };
    },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada gasto traz a palavra do SEU nível', async () => {
      for (const [index, name] of LEVEL_CASES.entries()) {
        ...
        await expect(lines[index]!.dataset.level).toBe(level);
    ...
    await step('E a fração de cada um está em TEXTO, e não só no desenho', async () => {
      ...
      for (const [index, name] of LEVEL_CASES.entries()) {
        ...
        await expect(textOf(lines[index]!, 'detail')).toContain(`${percent}%`);
      }
    });

...
import { agentStatusLabels, useAgentStatusLabels } from './agent-status.fixtures';
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      ...
      return {
        ...
        // Campo de texto vazio é ausência de relógio, e não um relógio em
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A linha NÃO é região viva', async () => {
      ...
      await expect(line.hasAttribute('aria-live')).toBe(false);
      await expect(line.hasAttribute('role')).toBe(false);
      await expect(line.querySelector('[aria-live]')).toBeNull();
      await expect(line.querySelector('[role="status"], [role="alert"], [role="log"]')).toBeNull();
    });
    ...
    await step('A palavra do estado escolhido está na linha', async () => {
      ...
      await expect(line.dataset.status).toBe(args.status);

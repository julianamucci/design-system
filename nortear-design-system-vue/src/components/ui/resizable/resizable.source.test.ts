...
<template>
  <div class="nds-w-full nds-max-w-lg nds-aspect-video nds-rounded-md nds-border-default nds-overflow-hidden">
...
describe('a moldura de tamanho definido', () => {
  it('toda transform embrulha o grupo num contêiner com tamanho', () => {
    ...
    for (const saida of TODAS) {
      expect(saida).toMatch(/<div class="nds-w-full nds-max-w-\w+ nds-aspect-[\w-]+ /);
    }
  });

...
import { ComputerUseDemoScreen, useComputerUseLabels } from './computer-use.fixtures';
import {
...
export const BesideRunStatus: Story = {
  ...
  render: () => ({
    components: { ComputerUse, ComputerUseDemoScreen, AgentStatus },
    setup() {
      return {
        url: COMPUTER_URL,
    ...
    template: `<div class="nds-stack nds-max-w-md" data-spacing="sm">
      <AgentStatus
        status="running"
        :elapsed="elapsed"
        :labels="runLabels"
...
export const LongText: Story = {
  ...
  render: () => ({
    ...
    template: `<div class="nds-stack nds-max-w-2xs">
      <ComputerUse
        :url="url"
        :steps="steps"
        :active-index="3"
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Nada dentro da peça entra na ordem de foco', async () => {
      ...
      await expect([...focusable].every((element) => element.closest('[inert]') !== null)).toBe(true);
    });
  },
};
...
export const Portrait: Story = {
  ...
  render: () => ({
    ...
    // fotografar o retrato. Fosse `aspect-ratio` direto, a declaração venceria
    // a folha e sairia do tema.
    template: `<div class="nds-stack nds-max-w-2xs">
      <ComputerUse
        :url="url"
        :steps="steps"
        ...
        status="running"
        :labels="labels"
        style="--computer-use-aspect: 9 / 16"
      >
        <template #screen><ComputerUseDemoScreen /></template>
      </ComputerUse>
    </div>`,

...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      // O nome acessível sai de um composable, então o render passa por um
      // `setup`: ele depende do idioma corrente, do número e da citação.
      return {
        args,
        parts: sentenceParts(),
        citation: computed(() => citationOf(args.shape)),
        labelsOf: useInlineCitationLabels(),
      };
    },
    // A FRASE É DE QUEM ESCREVE, e nunca da peça: a marca entra onde a
    // afirmação precisa de apoio. SEM ESPAÇO ANTES DELA — é assim que ela não
    // se separa da palavra que a antecede quando a linha quebra.
    //
    // O `key` refunda a marca quando o control troca: `default-open` é lido na
    // montagem, e sem isso o control mudaria a propriedade sem reabrir nada.
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    // `default-open` abre DEPOIS da montagem. Esperar por relógio antes de ler
    // é o que mantém a asserção honesta nos dois valores do control, e o que a
    // mantém fora da armadilha da espera por observador de mutação.

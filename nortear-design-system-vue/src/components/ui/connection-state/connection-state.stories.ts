...
import { CONNECTION_STATES, type ConnectionState as ConnectionStateValue } from '@shared/primitives/chat-protocol';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  argTypes: {
    state: {
      ...
      table: {
        type: { summary: CONNECTION_STATES.map((item) => `'${item}'`).join(' | ') },
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      ...
      return {
        ...
        countdown: computed(() => args.countdown || undefined),
        onRetry,
      };
    },
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('A região que se anuncia é SÓ a palavra', async () => {
      ...
      const liveRegions = [...root.querySelectorAll('[role="status"], [role="alert"], [role="log"], [aria-live]')];
      await expect(liveRegions).toEqual([label]);
    });

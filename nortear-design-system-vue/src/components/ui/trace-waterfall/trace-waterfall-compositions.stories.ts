...
/**
 * Uma cascata só, com um teto de largura opcional.
 *
 * O teto é parte do assunto quando a story fotografa a rolagem: o canvas do
 * Storybook é largo, e sem ele a cascata larga ficaria folgada — o guarda da
...
const mount = (
  ...
  setup() {
    return { labels: useTraceWaterfallLabels(), spans, totalMs, hostClass };
  },
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Nenhuma barra passa do fim do eixo', async () => {
      ...
      for (const bar of bars) {
        ...
        const start = Number(style.getPropertyValue('--trace-waterfall-bar-start').trim());
  ...
  render: () => ({
    ...
    setup() {
      return {
        ...
        // Declarada NO PRÓPRIO elemento, e não num invólucro: a folha a
        // declara em `.nds-trace-waterfall`, e propriedade personalizada
        // declarada dentro do próprio seletor vence a herança — o invólucro
        // nunca a alcançaria. Nesta stack o caminho é o repasse de `style`,
        // que o Vue funde na raiz do componente.
        tight: { '--trace-waterfall-name-min': 'var(--spacing-8)' },
      };
    },
    template: `<div class="nds-stack nds-max-w-md" data-spacing="lg">
      <TraceWaterfall
        data-testid="trace-waterfall-default-columns"
        :spans="spans"
        :total-ms="totalMs"
        status="running"
        :labels="labels"
      />
      <TraceWaterfall
        data-testid="trace-waterfall-tight-columns"
        :style="tight"
        :spans="spans"
        :total-ms="totalMs"
        status="running"

...
import { traceWaterfallLabels, useTraceWaterfallLabels } from './trace-waterfall.fixtures';
...
export const Playground: Story = {
  ...
  // O INVÓLUCRO existe para a asserção ter a que apontar quando não há
  // cascata: sem trecho a peça não desenha nada, nem moldura, nem camada que
  // rola.
  render: (args) => ({
    components: { TraceWaterfall },
    setup() {
      return {
        args,
        labels: useTraceWaterfallLabels(),
        spans: computed(() => TRACE_SPANS_ORDER.slice(0, args.revealed)),
      };
    },
    template: `<div data-testid="trace-waterfall-host">
      <TraceWaterfall
        :spans="spans"
        :total-ms="args.totalMs"
        :status="args.status"
        :labels="labels"
      />
    </div>`,
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('Cada barra cai na fração do eixo que o dado manda', async () => {
      ...
      for (const [index, bar] of bars.entries()) {
        ...
        const start = Number(
          style.getPropertyValue('--trace-waterfall-bar-start').trim(),
        );
        const size = Number(style.getPropertyValue('--trace-waterfall-bar-size').trim());

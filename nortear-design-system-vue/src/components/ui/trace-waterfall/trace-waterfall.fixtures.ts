 ...
 * nome da camada que rola, a frase da régua, o molde da leitura e a palavra de
 * cada estado. Os TRECHOS e o EIXO saem de
 ...
 *
 * Diferente da tela do computador, aqui NADA do andaime é interface: a cascata
 * não tem espaço de quem consome, então tudo o que a demonstração precisa é
 * dado. O que mora aqui, e não no compartilhado, são os rastros que existem só
 * para as fotos de borda desta stack — o largo, o de rótulos longos, o de
 * rótulos curtos e o da janela que recorta.
 *
...
import type { TraceWaterfallLabels } from './TraceWaterfall.vue';

/**
 * A anotação de tipo é o PORTÃO: a seção `labels` é lida como
 * `TraceWaterfallLabels` em CADA idioma, então rótulo que sumir do JSON — ou
...
export function traceWaterfallLabels(): TraceWaterfallLabels {
  return traceWaterfallLabelsFor(useI18nStore().locale);
}
 ...
 */
export function useTraceWaterfallLabels(): ComputedRef<TraceWaterfallLabels> {
  const { locale } = useTranslation(traceWaterfallTranslations);
  return computed(() => traceWaterfallLabelsFor(locale.value as Locale));
}
 ...
 * asserção e o dado que a produz não podem divergir: escrito à mão, o seletor
 * já apontou para um id inexistente depois de uma varredura de renomeação, e a
 * story passou a LANÇAR em vez de reprovar — defeito que nenhum build alcança,
 * porque mora dentro de uma string.
 */

  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada uma traz a PALAVRA daquele estado', async () => {
      ...
      for (const [index, status] of RUN_STATUSES.entries()) {
        await expect(words[index]).toBe(labels.status[status]);
      }
    ...
    await step('O código de saída só aparece onde a execução já acabou', async () => {
      for (const [index, status] of RUN_STATUSES.entries()) {
        ...
        const exit = exitOf(pieces[index]!);
...
export const WithoutOutput: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        command: TERMINAL_COMMAND,
        idleLines: linesFor('idle'),
        labels: useTerminalBlockLabels(),
      };
    },
    // Uma terminou sem escrever nada, a outra ainda não rodou.
    template: `<div class="nds-stack" data-spacing="lg">
      <TerminalBlock
        :command="command"
        status="complete"

...
import { linesFor, terminalBlockLabels, useTerminalBlockLabels } from './terminal-block.fixtures';
...
const meta: Meta<PlaygroundArgs> = {
  ...
  parameters: {
    ...
    docs: {
      page: withAutoDocsTab(TerminalBlockDocs),
...
export const Playground: Story = {
  ...
  render: (args) => ({
    ...
    setup() {
      ...
      return {
        ...
        lines: computed(() => (args.withOutput ? linesFor(args.status) : [])),
        // Campo numérico vazio é AUSÊNCIA de código de saída, e não zero: zero
        // é um resultado, e ausência é não haver resultado ainda.
        exitCode: computed(() =>
          Number.isFinite(args.exitCode) ? args.exitCode : undefined,
        ),
      };
    },
  ...
  play: async ({ canvasElement, step, args }) => {
    ...
    await step('O comando aparece como escrito, e o sinal fica fora do que é lido', async () => {
      ...
      const commandText = root.querySelector<HTMLElement>(
        '[data-slot="terminal-block-command-text"]',
      )!;
      await expect(commandText.textContent).toBe(TERMINAL_COMMAND);
      await expect(commandText.getAttribute('lang')).toBe('en');

...
import { exitCodeFor, linesFor, useTerminalBlockLabels } from './terminal-block.fixtures';
import {
...
export const CommandSequence: Story = {
  ...
  render: () => ({
    ...
    setup() {
      return {
        statuses: ['complete', 'failed', 'idle'] as RunStatus[],
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Cada peça é autônoma, e nenhuma contém a outra', async () => {
      ...
      for (const piece of pieces) {
        for (const other of pieces.filter((item) => item !== piece)) {
...
export const BesideRunStatus: Story = {
  ...
  render: () => ({
    components: { TerminalBlock, AgentStatus },
    setup() {
      return {
        command: TERMINAL_COMMAND,

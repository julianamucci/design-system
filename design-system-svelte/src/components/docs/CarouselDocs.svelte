    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          ...
          track('docs_section_viewed', { section_id: entry.target.id, component_name: 'carousel', locale: $locale });
          break;
        }
      }
...
<DocsPageLayout navGroups={NAV_GROUPS} {activeSection} componentSlug="carousel">
  {#snippet header()}
    <DocsHeader
      title={$tStore('title')}
  ...
  <DocsTokens
    ...
    items={[
      ...
      { token: '--ring',          value: 'focus-visible:ring',   description: $tStore('tokens.table.ring')         },
      { token: '--radius-button', value: 'rounded-full',         description: $tStore('tokens.table.radiusButton') },

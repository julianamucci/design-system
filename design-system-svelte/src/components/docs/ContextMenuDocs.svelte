    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          ...
          track('docs_section_viewed', { section_id: entry.target.id, component_name: 'context-menu', locale: $locale });
          break;
        }
      }
...
<DocsPageLayout navGroups={NAV_GROUPS} {activeSection} componentSlug="context-menu">
  {#snippet header()}
    <DocsHeader
      title={$tStore('title')}

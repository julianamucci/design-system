    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          ...
          track('docs_section_viewed', { section_id: entry.target.id, component_name: 'toggle', locale: $locale });
          break;
        }
      }
...
<DocsPageLayout navGroups={NAV_GROUPS} {activeSection} componentSlug="toggle">
  {#snippet header()}
    <DocsHeader
      title={$tStore('title')}
